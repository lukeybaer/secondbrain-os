...
//
// 2026-05-08 ExampleCo dispatch (PM revision): the dashboard tile must show
...
const DATA_DIR = process.env.SECONDBRAIN_DATA_DIR || path.join(REPO_ROOT, 'data');
const OUT_DIR = path.join(DATA_DIR, 'agent', 'mortgage-rates');
...
function parseFhlmcPmmsRate(html) {
  // PMMS HTML ExampleCos a "30-Year Fixed Rate Mortgage" section followed by a
...
async function main() {
  ...
  try {
    ...
    if (deltas.today != null) {
      ...
      // PMMS is weekly, so a non-publication day ExampleCos the latest official
