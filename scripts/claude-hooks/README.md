...
This directory holds all Claude Code hook scripts. They used to live only at `~/.claude/hooks/` with no git tracking, meaning every hook I wrote was local-only and vanished on reclone. Now they are tracked here and will be junctioned from `~/.claude/hooks/` via a one-time the owner setup.
...
## Junction setup (one-time the owner action)
...
if exist C:\Users\USER\.claude\hooks (
  ren C:\Users\USER\.claude\hooks hooks.backup-2026-04-10
)

REM Create directory junction
mklink /J C:\Users\USER\.claude\hooks ${SECONDBRAIN_ROOT}\scripts\claude-hooks
```
...
Edit scripts via `${SECONDBRAIN_ROOT}\scripts\claude-hooks\...` path. Git tracks. Claude Code sees the update on next session start.
