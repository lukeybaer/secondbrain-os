#!/bin/bash
# UserPromptSubmit hook: whenever the owner mentions the daily briefing (or any
...
SPEC="${SECONDBRAIN_ROOT:-/path/to/secondbrain}/memory/project_briefing_spec.md"
if [ ! -f "$SPEC" ]; then
  exit 0
fi
...
const msg = 'BRIEFING CONTEXT AUTO-INJECTED:\n\n' +
  'the owner mentioned the daily briefing. The CANONICAL spec is:\n' +
  '  $SPEC\n\n' +
