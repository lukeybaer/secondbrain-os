...
if echo "$COMMAND" | grep -qiE 'vapi\.ai/call|initiateCall|vapi'; then
  ...
  # Check for bridge-in language in the prompt/instructions , Amy should say "let me check with the owner and call back"
  if echo "$COMMAND" | grep -qiE 'bridge.*owner|transfer.*owner|connect.*to.*owner|patch.*owner'; then
    echo "WARNING: Detected bridge-in language in call instructions. Amy should NEVER offer to bridge the owner in. She should say 'let me check with the owner and call you back.' Review the prompt." >&2
  fi
fi
