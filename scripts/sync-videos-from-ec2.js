...
(async () => {
  ...
  for (const remotePath of ec2Files) {
    ...
    if (existingIds.has(videoId)) {
      ...
      // synced mp4 was the same file ExampleCo had just rejected.
      const entry = manifest.videos.find((v) => v.id === videoId);
      if (entry) {
        ...
        if (wasThumbRejected) {
          ...
          if (nowThumbMtime > rejectedMs && rejectedMs > 0) {
            ...
            // intact so this never reaches ExampleCo's review queue.
      ...
      // before this video ever reaches ExampleCo's review queue.
  ...
  // Without this, EC2 keeps a stale manifest and ExampleCo sees outdated items
