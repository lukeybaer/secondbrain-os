 ...
 *
 * Triggered by the 2026-05-01 incident: Luke asked for an immigration news
...
function extractDirectivesFromTranscript(transcript) {
  ...
  // Reject bodies that open with that filler shape (Luke 2026-05-18).
...
function handleEndOfCallReport(msg, callObj, deps = {}) {
  ...
  if (directives.length === 0) {
    return { archived: !!rawPath, rawPath, directives: [], skipped: 'no-directives', spineTaskId, sideEffectAudit };
  }
  ...
  try {
    ...
    return { archived: !!rawPath, rawPath, directives, directiveWriteError: e.message, spineTaskId, directiveTaskIds, sideEffectAudit };
  }

  // 4) Telegram ping so Luke sees what was captured.
