...
function walk(dir, out = [], includeAll = false) {
  ...
  for (const name of fs.readdirSync(dir)) {
    ...
    if (st.isDirectory()) walk(full, out, includeAll);
    else {
      const ext = path.extname(name).toLowerCase();
      if (includeAll || TEXT_EXTS.has(ext)) out.push(full);
    }
  }
  return out;
}
...
function phoneRegexSource(phone) {
  return String(phone || '')
    .replace(/\D/g, '')
    .split('')
    .map(escapeRegex)
...
function buildPatterns(denylist) {
  // Distinct token list: tokens + ALL-LOWERCASE single-token names. Tokens are
  // private context by definition, so they match even inside identifiers and
  // domains. This intentionally scrubs `formatPrivateCoSection` and
  // `@privateco.com`, not only standalone words.
  ...
  const firstNamesEscaped = (denylist.firstNames || []).map(escapeRegex);
  ...
  if (firstNamesEscaped.length) {
    out.push({ kind: 'first-name', re: new RegExp('\\b(' + firstNamesEscaped.join('|') + ')\\b', 'g') });
  }
  if (tokensEscaped.length) {
    out.push({
      kind: 'token',
      re: new RegExp('(' + tokensEscaped.join('|') + ')', 'gi'),
    });
  }
  ...
  if (phones.length) {
    const phonePatterns = phones.map(phoneRegexSource).filter(Boolean);
    out.push({ kind: 'phone', re: new RegExp('(' + phonePatterns.join('|') + ')', 'g') });
  }
  return out;
}
...
function firstPatternHit(value, patterns) {
  for (const p of patterns) {
    p.re.lastIndex = 0;
    const m = p.re.exec(value);
    if (m) {
      return { kind: p.kind, match: m[1] || m[0] };
    }
  }
  return null;
}

function scanContent(targetDir, repoRoot, denylist) {
...
function scanPathNames(targetDir, repoRoot, denylist) {
  const patterns = buildPatterns(denylist);
  const files = walk(targetDir, [], true);
  const hits = [];
  for (const abs of files) {
    const rel = relFromRepo(abs, repoRoot);
    const relFromTarget = path.relative(targetDir, abs).split(path.sep).join('/');
    if (SELF_SKIP.has(rel) || SELF_SKIP.has(relFromTarget)) continue;
    const hit = firstPatternHit(relFromTarget, patterns);
    if (hit) {
      hits.push({
        file: rel,
        line: 0,
        kind: `${hit.kind}-path`,
        match: hit.match,
        context: relFromTarget.slice(0, 160),
      });
    }
  }
  return hits;
...
function scan(targetDir, repoRoot, denylist) {
  return [
    ...scanContent(targetDir, repoRoot, denylist),
    ...scanPathNames(targetDir, repoRoot, denylist),
  ];
}
