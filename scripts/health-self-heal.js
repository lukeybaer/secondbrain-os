...
// overnight by Amy" lines instead of red flags the owner has to notice himself.
//
...
const REPO = '${SECONDBRAIN_ROOT:-/path/to/secondbrain}';
...
function healLlm(probe) {
  ...
  // The fix is local: start-claude-proxy.vbs on the owner's machine. Leave a telegram
  // alert instead so the owner can nudge it with one click.
  return {
    healed: false,
    action: 'no-op (proxy tunnel local-only)',
    note: 'launch start-claude-proxy.vbs on the owner machine',
  };
}
