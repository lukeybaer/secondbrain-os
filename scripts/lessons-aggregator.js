...
//
//   1. Per-skill outcome breakdown (ok / fail / partial / unknown)
...
//   4. Briefing-shaped tier1 + tier2 lines for Luke's CEO three-tier rule.
//
...
const PARTIAL_TOKENS = ['partial', 'amber', 'warn', 'warning', 'degraded', 'skipped', 'flaky', 'retry'];
...
function classifyOutcome(outcome) {
  if (!outcome || typeof outcome !== 'string') return 'unknown';
...
function summariseSkill(skillName, { window = 50 } = {}) {
  ...
  const counts = { ok: 0, fail: 0, partial: 0, unknown: 0 };
