 ...
 *
 * 2026-05-25 Luke flagged: my token-usage card was reporting "Claude Max
...
const OUT_PATH = path.join(REPO, 'data', 'agent', 'claude-plan-usage.json');
...
async function main() {
  ...
  let raw;
  try { raw = await fetchUsage(token); }
  catch (e) {
    console.error(`[collect-claude-plan-usage] ${e.message}`);
    process.exit(1);
  }
...
module.exports = { fetchUsage, flatten };
