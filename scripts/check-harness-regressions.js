...
function checkRegressions({ skillNames, currentMetricsBySkill }) {
  ...
  for (const skillName of skills) {
    ...
    if (current.insufficientData) {
      // Empty measurement window (no artifacts yet tonight). You cannot infer a
      // regression from absence of data, so never recommend a revert here.
      skipped.push({ skillName, reason: 'insufficient data in measurement window' });
      continue;
    }
    let rec;
    try {
