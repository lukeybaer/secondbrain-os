 ...
 * Luke called out that the heal loop only reported failures honestly,
 ...
 *       Repair: for each hit file that matches a known Luke-private path
 ...
 * well. The module never throws on unknown input; classification is
 * conservative.
 *
...
// 2026-05-27 Luke #gap: when the loop cannot repair a test, classify it as a
...
// briefing card surfaces the list so Luke can approve each skip in one click.
const DEFAULT_SKIP_RECOMMENDATIONS_PATH = path.join(REPO, 'data', 'agent', 'auto-heal-skip-recommendations.jsonl');

// Known Luke-private path patterns. Files matching any of these are safe
...
const LUKE_PRIVATE_PATH_PATTERNS = [
...
function isLukePrivateSurface(filepath) {
  const norm = String(filepath || '').replace(/\\/g, '/').replace(/^public-sync\//, '');
  return LUKE_PRIVATE_PATH_PATTERNS.some((re) => re.test(norm));
}

function classifyFailure(item) {
  if (!item || typeof item !== 'object') {
    return { category: 'unknown', canAutoRepair: false, queueForHuman: true };
  }
  ...
  // and a skip-with-reason recommendation is high-quality input for Luke.
  ...
  // Anything else: unknown, escalate. We still recommend a skip so the loop
  // does not requeue the same failure overnight after overnight without ever
  // producing an actionable next step for Luke.
  return { category: 'unknown', canAutoRepair: false, queueForHuman: true, recommendSkip: true };
}
...
// appends Luke-private paths to the STRIP array in
...
function repairPiiScreenLive(opts = {}) {
  ...
  for (const [file] of byFile) {
    if (isLukePrivateSurface(file)) stripCandidates.push(file);
  ...
  const insertion = '\n  // auto-heal ' + new Date().toISOString() + ': added by heal-tests-repair.js after detecting Luke-private surfaces in PII scan\n' + newEntries.join('\n') + '\n';
  const updated = src.slice(0, closeIdx) + insertion + src.slice(closeIdx);
  fs.writeFileSync(syncScriptPath, updated, 'utf8');
  return { ok: true, action: 'pii-screen auto-repair: added ' + newEntries.length + ' Luke-private file(s) to public-sync STRIP: ' + stripCandidates.join(', ') };
}
...
// 2026-05-27 Luke #gap: when an item cannot auto-repair, recommend a one-line
...
function reasonForSkip(item, classification) {
  ...
  return `no automatable handler for this failure category; classify as known skip pending Luke triage. ${err}`.slice(0, 220);
}
...
function repairItem(item, classification, opts = {}) {
  ...
  if (classification.category === 'pii-screen-public-sync-hits') {
    // Run the live scanner, classify each hit file, add Luke-private
...
function runRepairPass(blocked, opts = {}) {
  ...
  const actions = [];
  // 2026-05-27 Luke #gap: when a non-repairable failure has already produced
  ...
  for (const item of items) {
    ...
    if (cls.canAutoRepair) {
      ...
      if (r.ok) {
        ...
        // Luke has an explicit one-click triage entry instead of an opaque
        // re-queued blob.
