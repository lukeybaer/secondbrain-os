...
Public-figure mentions are fine (Sam Altman, Anthropic, Meta, etc.). Generic example names (PRIVATE_NAME Doe, Jane Doe, "the user", "the owner") are fine. Generic technology references (Claude, GPT, FFmpeg, Vapi, etc.) are fine.
...
// 2026-06-11 ladder: claude haiku -> codex read-only -> openai gpt-4o-mini.
// One askAI call covers rungs 2+3 (codex then the openai floor); claude
// haiku stays rung 1 because the model pin lives in callClaude.
// deps is a test seam: { claudeFn, askAIFn }.
async function runLadder(prompt, deps = {}) {
  const claude = deps.claudeFn || callClaude;
  const c = claude(prompt);
  if (c.ok) return { ok: true, text: c.text, provider: 'claude-haiku' };
  const ladder = deps.askAIFn || require('./lib/ask-ai.js').askAI;
  let out = null;
  try {
    out = await ladder(prompt, {
      surface: 'pii-llm-gate',
      rungOrder: ['codex', 'openai-api'],
  ...
  if (out && out.text) return { ok: true, text: out.text, provider: out.rung };
  return { ok: false, provider: null };
}

// FAIL-CLOSED policy when every rung is down: exit 2 blocks public-sync.
// The ONLY override is PII_GATE_OVERRIDE=1 WITH a non-empty
// PII_GATE_OVERRIDE_REASON (logged to the override ledger by main).
function failClosedDecision(env = process.env) {
  const reason = String(env.PII_GATE_OVERRIDE_REASON || '').trim();
  if (env.PII_GATE_OVERRIDE === '1' && reason) {
    return { exitCode: 0, overridden: true, reason };
  }
  return { exitCode: 2, overridden: false };
}

const OVERRIDE_LOG = path.join(REPO, 'data', 'agent', 'pii-gate-override-log.jsonl');
...
function recordOverride(reason) {
  try {
    ...
    /* the override still proceeds; the justification was printed loud */
  }
}
...
async function main() {
  ...
  const res = await runLadder(prompt);
  if (!res.ok) {
    const decision = failClosedDecision(process.env);
    if (decision.overridden) {
      recordOverride(decision.reason);
      console.warn(
        `[pii-llm-gate] OVERRIDDEN (PII_GATE_OVERRIDE=1): ${decision.reason}. Gate skipped; justification logged to ${OVERRIDE_LOG}`,
      );
      process.exit(0);
    }
    console.error(
      '[pii-llm-gate] FAIL-CLOSED: all LLM rungs failed (claude haiku -> codex read-only -> openai gpt-4o-mini). Public sync is BLOCKED until a rung recovers. Manual override: PII_GATE_OVERRIDE=1 with PII_GATE_OVERRIDE_REASON="why" (logged).',
    );
    process.exit(2);
  }
  ...
  if (blocked.length === 0) {
    console.log(`[pii-llm-gate] CLEAN: ${res.provider} passed all ${samples.length} lines`);
    process.exit(0);
  }

  console.log(`[pii-llm-gate] ${blocked.length} line(s) flagged by ${res.provider}:`);
...
if (require.main === module) {
  main().catch((e) => {
    console.error('[pii-llm-gate] runtime failure:', e && e.message);
    process.exit(2);
  });
}
