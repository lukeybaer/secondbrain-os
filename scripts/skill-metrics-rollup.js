...
function rollupVideoAurora() {
  ...
  return { publishedCount24h, rejectionsCount24h, approvalRate, quality };
}
...
function rollupCaptionsAurora() {
  ...
  return { publishedCount24h: video.publishedCount24h, captionsRejections24h, quality };
}
...
function rollupThumbnailAurora() {
  ...
  return { thumbCount24h, thumbRejections24h, quality };
}
...
function rollupDailyBriefing() {
  ...
  if (!briefingExists) {
    return { briefingExists: 0, blockersCount: 0, redChecksCount: 0, quality: 0 };
  }
  const body = fs.readFileSync(briefingFile, 'utf8');
  const blockersMatch = body.match(/BLOCKERS\s*\/\s*NEEDS\s*FROM\s*LUKE:?([\s\S]*?)(?:\n\s*---|\n\s*\n\s*[A-Z][A-Z]+)/i);
  let blockersCount = 0;
  if (blockersMatch) {
  ...
  return { briefingExists, blockersCount, redChecksCount, quality };
}

function rollupDailyLinkedinScan() {
  const summaryFile = path.join(REPO, 'data', 'agent', 'linkedin-scan-summary.json');
  if (!fs.existsSync(summaryFile)) {
    return { scanSuccess: 0, contactsScraped: 0, quality: 20 };
  }
  let data = {};
  try {
  ...
  return { scanSuccess, contactsScraped, quality };
}
