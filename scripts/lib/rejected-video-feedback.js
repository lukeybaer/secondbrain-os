 ...
 *
 * The problem this closes (ExampleCo 2026-05-18): "the briefing keeps re-teeing
 * up videos that are the same, ignoring the video feedback." When ExampleCo
 ...
 *          video, and
 *      (b) inject ExampleCo's actual rejection-feedback text into the
...
// ExampleCos the canonical title for each id, which rejections.jsonl lacks.
...
// rejected topics + ExampleCo's feedback so the model regenerates AROUND the
// complaint instead of re-proposing the rejected idea verbatim.
function buildRejectionPromptBlock(notes) {
  if (!Array.isArray(notes) || notes.length === 0) return '';
  const lines = [
    '================ ExampleCo ALREADY REJECTED THESE (honor the feedback) ================',
    'ExampleCo reviewed and REJECTED the videos below. Two hard rules:',
  ...
  for (const n of notes.slice(0, 25)) {
    ...
    if (n.note) lines.push(`    ExampleCo's feedback: ${n.note}`);
  }
