 ...
 *
 * 2026-05-25 Luke #learn: rebuild produces identical content across
 ...
 *       Defaults to a claude CLI invocation. Tests inject mocks.
 *
 * The default llmFn calls `claude -p <prompt>` via the system claude
 * binary. Max-plan only, no paid Anthropic API. If claude is missing
 * or returns non-zero, the interpreter surfaces an error object so the
 * caller can hard-block.
 */
'use strict';

const { spawnSync } = require('child_process');
const path = require('path');
// 2026-05-25 Luke #learn: the EC2 claude CLI is authenticated via the
// hourly-pushed ~/.claude-oauth-token file. Direct `claude -p` without
// CLAUDE_CODE_OAUTH_TOKEN in the env returns 401. Reuse the shared
// helper so the interpreter, ec2-server.js, and process-dispatches.js
// all build env the same way.
const { buildClaudeCliEnv } = require(path.join(__dirname, 'cli-output-guard.js'));
...
function buildPrompt(rejectionHistory, spec) {
  ...
  return [
    'You are reviewing the rejection history of a short-form vertical video that Luke has rejected multiple times. Your job is to translate the rejection text into STRUCTURED spec overrides that the build pipeline can consume on the next rebuild. Without this translation, the rebuild produces identical content.',
    '',
    'CURRENT SPEC:',
    specSummary,
    '',
    ...
    '  "opening_tag_override":   string | null,       // first kinetic-caption tag (1-3 words, ALLCAPS) when a prior tag was wrong',
...
function normalize(parsed) {
  return {
    banned_phrases: Array.isArray(parsed.banned_phrases) ? parsed.banned_phrases.filter((p) => typeof p === 'string' && p.trim()).map((p) => p.trim()) : [],
    hook_phrase: typeof parsed.hook_phrase === 'string' && parsed.hook_phrase.trim() ? parsed.hook_phrase.trim() : null,
    opening_tag_override: typeof parsed.opening_tag_override === 'string' && parsed.opening_tag_override.trim() ? parsed.opening_tag_override.trim() : null,
    voice_id_override: typeof parsed.voice_id_override === 'string' && parsed.voice_id_override.trim() ? parsed.voice_id_override.trim() : null,
    animation_density: typeof parsed.animation_density === 'string' && VALID_DENSITY.has(parsed.animation_density) ? parsed.animation_density : null,
    stock_query_override: typeof parsed.stock_query_override === 'string' && parsed.stock_query_override.trim() ? parsed.stock_query_override.trim() : null,
    required_directives: Array.isArray(parsed.required_directives) ? parsed.required_directives.filter((d) => typeof d === 'string' && d.trim()).map((d) => d.trim()) : [],
    rejection_summary: typeof parsed.rejection_summary === 'string' ? parsed.rejection_summary.trim() : '',
...
// Default LLM call via claude CLI (Max plan, free). Throws on failure
// so the interpreter wraps it in error handling and returns an error
// object instead of crashing the caller.
function defaultLlmFn(prompt) {
  // Find claude binary; allow override via CLAUDE_PATH for Windows .cmd.
  const claudeBin = process.env.CLAUDE_PATH
    || (process.platform === 'win32'
      ? (process.env.APPDATA ? `${process.env.APPDATA}\\npm\\claude.cmd` : 'claude.cmd')
      : 'claude');
  const r = spawnSync(claudeBin, ['-p', prompt, '--model', 'claude-sonnet-4-20250514'], {
    encoding: 'utf-8',
    timeout: 60_000,
    maxBuffer: 4 * 1024 * 1024,
    env: buildClaudeCliEnv(),
  });
  if (r.error) throw new Error(`claude CLI spawn failed: ${r.error.message}`);
  if (r.status !== 0) throw new Error(`claude CLI exit ${r.status}: ${(r.stderr || '').slice(0, 200)}`);
