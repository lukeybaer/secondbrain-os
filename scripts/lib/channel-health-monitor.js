...
// Luke on Telegram, attempts an in-process auto-fix, reprobes, then alerts
...
// Luke's laptop, not EC2. EC2 cannot restart them; they recover on their own
...
async function probeTelegram({ botToken, fetcher = httpGetJson } = {}) {
  ...
  return { status: 'red', detail: 'getMe failed: ' + (r.error || r.body || r.status || 'unknown') };
}
...
async function probeProxy({ proxyPort = 3456, fetcher = httpGetJson } = {}) {
  ...
  return { status: 'red', detail: 'proxy 127.0.0.1:' + proxyPort + ' not responding: ' + (r.error || r.status || 'unknown') };
}
...
async function probeBackend({ backendPort = 3001, fetcher = httpGetJson } = {}) {
  ...
  return { status: 'red', detail: 'backend :' + backendPort + ' not responding: ' + (r.error || r.status || 'unknown') };
}
...
async function probeVapi({
  ...
  // soft: zero webhooks in 24h is yellow-then-red because Luke may simply
  ...
  if (!apiReach.ok) {
    return { status: 'red', detail: 'vapi api unreachable: ' + (apiReach.error || apiReach.status || 'unknown') };
  }
...
async function probeClaudeMaxToken({
  ...
  try {
    ...
    return {
      status: 'red',
      detail: 'token file missing at ' + tokenPath + '. Run claude-token-refresh on Luke laptop.',
    };
  }
...
async function probeGmail({
  ...
  // hashtag workflow on Luke's laptop covers Gmail scanning in that case.
  ...
  if (!fsImpl.existsSync(heartbeatPath)) {
    ...
    if (!helperExists && !pm2Running) {
      return {
        status: 'green',
        detail: 'gmail-amy-scan not deployed on this host; #inbox runs from Luke laptop',
      };
    }
  ...
  if (ageMin > staleMin) {
    ...
    if (!helperExists && !pm2Running) {
      return {
        status: 'green',
        detail: 'gmail-amy-scan not deployed on this host; #inbox runs from Luke laptop (heartbeat ' + ageMin + 'm old, ignored)',
      };
    }
...
async function probeLinkedin({
  ...
  // app on Luke's laptop and relayed to EC2 via /sync. EC2 never crawls; it
  ...
  function probe({ uptimeMs, restarts, now = () => Date.now() } = {}) {
    ...
    if (resolvedUptime == null || resolvedRestarts == null) {
      try {
        ...
        return { status: 'red', detail: 'pm2 jlist failed: ' + (e.message || 'unknown').slice(0, 80) };
      }
    }
...
function healTelegram() {
  return {
    attempted: false,
    action: 'manual',
    detail: 'TELEGRAM_BOT_TOKEN is owner-managed; no safe in-process fix. Surface to Luke.',
  };
}
...
function healProxy() {
  return {
    attempted: false,
    action: 'manual',
    detail: 'Claude Max proxy runs on Luke laptop via start-claude-proxy.vbs. EC2 cannot restart it. Surface to Luke.',
  };
}
...
function healBackend({ exec = (cmd) => execSync(cmd, { encoding: 'utf-8', timeout: 10000 }) } = {}) {
  ...
  // any future crash loop is detected cleanly, and to surface to Luke that
...
function healVapi() {
  ...
  return { attempted: false, action: 'manual', detail: 'Vapi api outage is upstream; alert Luke and wait.' };
}
...
function healGmail({ exec = (cmd) => execSync(cmd, { encoding: 'utf-8', timeout: 15000 }) } = {}) {
  ...
  // (Windows task scheduler), surface to Luke.
  try {
...
function healLinkedin() {
  // The LinkedIn crawl runs in the Electron app on Luke's laptop, not EC2.
  ...
  return {
    attempted: false,
    action: 'manual',
    detail: 'LinkedIn crawl runs nightly in the Electron app on Luke laptop. EC2 cannot trigger it. Surface to Luke.',
  };
}
...
function healPm2({ exec = (cmd) => execSync(cmd, { encoding: 'utf-8', timeout: 5000 }) } = {}) {
  ...
  // plus a Luke alert so he can investigate the underlying cause.
  try {
...
function createMonitor({
  ...
  const defaultDefs = [
    ...
    {
      // Catches the 2026-05-23 outage where Luke's laptop was awake, the proxy
      // was reachable but the OAuth token push had stalled, so Amy went dark on
      // Telegram for ~2 hours before Luke noticed. With this probe the briefing
  ...
  async function checkOne(def) {
    ...
    try {
      ...
      result = { status: 'red', detail: 'probe threw: ' + (e.message || 'unknown') };
    }
  ...
  async function tick() {
    ...
    for (const t of transitions) {
      ...
      // Laptop-hosted channels run on Luke's laptop; EC2 cannot fix them.
