...
const ExampleCo_PRIORITY_ANCHORS = [
  'ExampleCo (stadium + security + AI ventures)',
...
function buildDraft(ev, cf) {
  ...
  if (cf && isActiveRelationship(cf)) {
    const lines = [];
    const postTopicFragment = sentenceFragment(postTopic);
    if (first) lines.push(`${first},`);
    if (headline) {
      lines.push(`solid post.${postTopicFragment ? ' The angle on ' + postTopicFragment + ' is the right frame.' : ''} Aligns with what we have been building.`);
    } else if (postTopicFragment) {
      lines.push(`the ${postTopicFragment} thread you are developing is exactly the right frame for what we are doing.`);
    } else {
      lines.push('strong perspective. The systems angle you take is the clearest framing I have seen on this.');
    }
    lines.push('Worth amplifying from the ExampleCo side too. ExampleCo');
    return cleanText(lines.join(' '));
  }

  const overlap = cf ? firstMeaningfulLine(cf.trending || cf.professional, 140) : '';
  const postTopicFragment = sentenceFragment(postTopic);
  const overlapFragment = givenClauseFragment(overlap).slice(0, 140).trim();
  const lines = [];
  if (first) lines.push(`${first},`);
  if (headline) {
    lines.push(`saw your recent post${postTopicFragment ? ' on ' + postTopicFragment : ''}. It connects to work I am doing on autonomous AI systems for ops and decision-making.`);
  } else if (postTopicFragment) {
    lines.push(`been thinking about your work on ${postTopicFragment}. It overlaps with what I am building on the autonomous AI side.`);
  } else {
  ...
  if (overlapFragment) {
    lines.push(`Given ${overlapFragment}, I would value your read on where human judgment should sit before software acts.`);
  } else {
...
function buildWhyThisNote(ev, cf) {
  ...
  const theirPost = headline
    ? (postsAbout ? `${headline} Consistent with their ongoing posting about ${sentenceFragment(postsAbout)}.` : headline)
...
function renderPickBlock(index, ev, cf) {
  const lines = [];
  const name = (ev && ev.contactName) || (cf && cf.name) || 'PRIVATE_NAME';
