...
//
// Locked 2026-05-13 after ExampleCo posted v5 to AILifeHacksByExampleCoyExampleCo and said
...
// 2026-05-25 ExampleCo: YouTube blocks datacenter IPs on yt-dlp with
// "Sign in to confirm you're not a bot." A one-time cookies export
// from ExampleCo's signed-in Chrome lives at this path on EC2 (mode 600,
...
function pickMusic(proposal) {
  ...
  const moodMap = [
    ...
    [/mystery|secret|hidden|ExampleCo/, 'mystery_ambient.mp3'],
  ];
...
function downloadRange(sourceUrl, startSec, endSec, outNoExt) {
  ...
  // 2026-05-25 ExampleCo: YouTube blocks EC2 datacenter IPs from fetching
  // video data regardless of cookies/PoToken/headless. The download
  // must happen on a residential IP (ExampleCo's PC) and the mp4 + vtt
