...
function analyzeFile(filePath) {
  ...
  const hasCompany =
    /\b(company|org|employer|work|amazon|Employer|vapi|saasinct|bai|itm|abbyy|yelp|salesforce|microsoft|google|meta|epam|dailypay|tebra|mindbody|intuit|doordash|ziprecruiter|five9|smartsheet)\b/i.test(
      content,
    );
  ...
  const hasPersonality =
    /\b(personality|values|character|faith|ExampleCotian|humble|direct|smart|nice|honest|loyal|ambitious|introvert|extrovert)\b/i.test(
      content,
    );
  const hasLocation =
    /\b(lives? in|based in|located|moved to|from |city|state|country|ExampleCo|ExampleCo|seattle|sf|nyc|toronto|austin)\b/i.test(
      content,
    );
