...
// C:Userslukedsecondbraindataagentvitest-heal-output.json (path.join then
...
const LOOP_INTERVAL_MS = parseInt(args.find((a) => a.startsWith('--interval='))?.slice(11) || '1800000', 10); // default 30 min
...
const VITEST_TIMEOUT_MS = parseInt(process.env.HEAL_TESTS_TIMEOUT_MS || String(DEFAULT_VITEST_TIMEOUT_MS), 10);
...
`;
  try {
    ...
    console.warn(`[heal-tests] stale vitest cleanup failed after ${reason}: ${String(e.message || e).slice(0, 160)}`);
    return 0;
  }
}
  ...
  if (fs.existsSync(LOCK_PATH)) {
    try {
      ...
      if (isPidAlive(lock.pid)) {
        console.warn(`[heal-tests] another heal-tests run is active (pid ${lock.pid}, started ${lock.startedAt}); skipping`);
        appendRunLog({
          attempt: 0,
          loopMode: LOOP,
          total: 0,
...
function runOnce(attempt) {
  // 2026-05-02 Luke #gap: same vitest-truncation bug fixed in manual-briefing-v3.js.
  ...
  if (fs.existsSync(outputFile)) {
    try { j = JSON.parse(fs.readFileSync(outputFile, 'utf-8')); }
    catch { /* fall through to stdout regex */ }
  }
  if (!j) {
    const match = raw.match(/\{[\s\S]*\}\s*$/);
    if (!match) {
      const reason = timedOut ? `vitest timed out (>${Math.round(VITEST_TIMEOUT_MS / 1000)}s)` : 'no JSON parsed';
    ...
    try { j = JSON.parse(match[0]); } catch (e) {
  ...
  for (const r of results) {
    ...
    for (const t of tests) {
      ...
      items.push({
        ...
        suggestSkip: /to contain|to match|to be|toEqual|toBe/i.test(fullErr) && !/TypeError|ReferenceError|undefined/i.test(fullErr),
      });
    }
  }
  ...
  // Without this log entry, the next briefing claim is fabrication. Luke 2026-04-28 #gap.
  appendRunLog({
    attempt: LOOP ? attempt : 0,
    loopMode: LOOP,
    total,
...
if (require.main === module) (async () => {
  ...
  function parseDeadlineToTodayCt(spec) {
    ...
    // Luke's Windows box, so a naive Date is correct here. Document the
  ...
  for (let attempt = 1; attempt <= LOOP_MAX_ATTEMPTS; attempt++) {
    ...
    if (r.ok) {
      ...
      if (lastRepairResult && lastRepairResult.repaired > 0) {
        console.log(`[heal-tests loop] repair pass closed ${lastRepairResult.repaired} item(s) before this green attempt: ${lastRepairResult.actions.filter(a => a.ok).map(a => a.action).join(' | ')}`);
      }
      process.exit(0);
    }
    if (attempt === LOOP_MAX_ATTEMPTS) {
      console.warn(`[heal-tests loop] reached max attempts (${LOOP_MAX_ATTEMPTS}) with ${r.failed} still failing -- exiting non-green`);
      process.exit(0);
    }
    if (!shouldRetryLoopRun(r, attempt, LOOP_MAX_ATTEMPTS)) {
      console.warn(`[heal-tests loop] runner failed (${r.parseError || 'unknown'}); not retrying tonight to avoid orphaned vitest workers`);
      process.exit(0);
    }
    // Repair pass: between attempts, try to actually fix what failed.
    // Closes the 2026-05-23 Luke gap: the loop was re-running vitest
    ...
    if (blocked) {
      try {
        ...
        console.log(`[heal-tests loop] repair pass: repaired=${rep.repaired}, queued=${rep.queued}, escalated=${rep.escalated}, skipRecommendations=${rep.skipRecommendations || 0}`);
        ...
        if (rep.repaired > 0) {
          ...
          console.log(`[heal-tests loop] ${rep.repaired} item(s) auto-repaired -- retrying immediately without the ${LOOP_INTERVAL_MS / 60000}m sleep`);
          continue;
        }
      } catch (e) {
        console.warn(`[heal-tests loop] repair pass threw, continuing: ${String(e.message || e).slice(0, 160)}`);
      }
    }
    ...
    if (prevSignature !== null && signature === prevSignature) {
      ...
      const skipNote = skipCount > 0
        ...
        : '';
      console.warn(`[heal-tests loop] no-progress early exit at attempt ${attempt} -- failing set unchanged from prior attempt (${r.failed} failing) and the repair pass had no automatable handler. Items have been queued for human triage in data/agent/auto-heal-needs-human.jsonl; the briefing will surface them.${skipNote}`);
      appendRunLog({
        attempt,
        loopMode: true,
        total: r.total,
    ...
    if (deadline) {
      ...
      if (nextAttemptAt >= deadline.getTime()) {
        console.warn(`[heal-tests loop] deadline early exit at attempt ${attempt} -- next attempt at ${new Date(nextAttemptAt).toISOString()} would land past the heal-window deadline ${deadline.toISOString()}.`);
        appendRunLog({
          attempt,
          loopMode: true,
          total: r.total,
    ...
    console.log(`[heal-tests loop] still ${r.failed} failing -- sleeping ${LOOP_INTERVAL_MS / 60000}m before next attempt`);
