 ...
 *
 * Locked rules (Luke 2026-05-03 / 2026-05-04):
 *   - 10 proposals daily, FRESH every day. No carryover. Yesterday's
 *     unselected proposals are dead.
 *   - If Luke does not approve any proposal that day, NO video gets made
 ...
 *   - During the day: Luke clicks approve on the dashboard, which calls
...
const PROPOSALS_DIR = path.join(REPO, 'data', 'agent', 'shorts-proposals');
const VIRAL_SKILL_PATH = path.join(REPO, 'skills', 'content', 'viral-shorts-judge.md');

// Engagement floors for source quality. 2026-05-25 Luke flagged on Otter:
...
function engagementFloors() {
  return {
    views: Math.max(0, Number(process.env.SHORTS_MIN_VIEWS) || 25000),
    likes: Math.max(0, Number(process.env.SHORTS_MIN_LIKES) || 500),
...
function passesRubric(proposal) {
  ...
  const axes = ['hook', 'frame', 'specificity', 'payoff', 'stake', 'visual', 'novelty', 'source_proof'];
...
// Returns the current date in Central Time (Luke's local TZ), since briefings
...
function fetchUrl(url) {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { 'User-Agent': 'AIDailyLifeHacks-proposals/1.0' } }, (res) => {
      let data = '';
...
async function pullXTrendingAI(opts = {}) {
  ...
  for (const q of queries) {
    try {
      const url = 'https://r.jina.ai/http://r.jina.ai/http://https://duckduckgo.com/html/?q=' + encodeURIComponent(q);
      const r = await fetchUrl(url);
      if (r.status !== 200) continue;
      const matches = [...r.body.matchAll(/\[([^\]]+?)\]\(https:\/\/duckduckgo\.com\/l\/\?uddg=([^)&]+)[^)]*\)\s+([^\n]+)/g)];
      for (const m of matches) {
        ...
        const snippet = snippetStart >= 0
          ...
          : m[3];
        const views = (snippet.match(/([\d,.]+)\s*\*\*?views?\*\*?/i) || snippet.match(/([\d,.]+)\s+views/i) || [])[1] || '';
  ...
  for (const item of deduped) {
    ...
    if (!sq.ok) { dropped += 1; continue; }
    qualified.push({ ...item, source_signal: sq.signal });
  }
  if (dropped > 0) {
    console.warn(`[shorts-proposals] source-quality gate dropped ${dropped}/${deduped.length} candidates (raised floor by env? SHORTS_MIN_VIEWS=${process.env.SHORTS_MIN_VIEWS || '5000'} SHORTS_MIN_LIKES=${process.env.SHORTS_MIN_LIKES || '100'} SHORTS_MIN_REPLIES=${process.env.SHORTS_MIN_REPLIES || '20'})`);
  }
    ...
    try {
      ...
      if (r.status === 200) {
        ...
        for (const it of items) {
          ...
          if (title && link) out.push({ source: new URL(f).hostname, title: title.trim(), url: link.trim() });
        }
      }
    } catch {}
  }
...
const CLAUDE_CLI_JS_CANDIDATES = [
  path.join(process.env.USERPROFILE || require('os').homedir(), 'AppData/Roaming/npm/node_modules/@anthropic-ai/claude-code/cli.js'),
...
function runClaudeCliAsync(args, env, timeoutMs, maxBuffer) {
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      ...
      finish({ status: null, stdout, stderr, error: new Error(`claude cli timed out after ${timeoutMs}ms`) });
    }, timeoutMs);
...
// Generate 10 fresh proposals via Claude Max CLI (no paid API).
async function generateProposalsWithClaude(signals) {
  ...
  const prompt = [
    ...
    '================ END SKILL ================',
    '',
    // Rejection feedback: topics Luke already rejected, plus his feedback
  ...
  if (!cliJs) {
    console.warn('[shorts-proposals] could not resolve claude cli path');
    return [];
  }
  ...
  try {
    const r = await runClaudeCliAsync(
    ...
    if (r.status !== 0 || !r.stdout) {
      ...
      console.warn('[shorts-proposals] claude cli exit', r.status, 'err:', err);
      return [];
    }
    ...
    try {
      ...
      fs.writeFileSync(path.join(PROPOSALS_DIR, `_last_raw_attempt${signals.attempt || 1}.txt`), out);
    } catch {}
    // Parse JSON array, tolerant of optional fences or surrounding prose
    let cleaned = out.replace(/^```json\s*/m, '').replace(/```\s*$/m, '').trim();
    ...
    console.warn('[shorts-proposals] Claude generation failed:', e.message);
    return [];
  }
}
...
function normalizeProposal(p, i, source) {
  const title = String(p.title || source?.title || `AI workflow from ${source?.source || 'today'}`).trim();
  ...
  return {
    ...
    trend_hook: String(p.trend_hook || `X trend source: ${source?.title || title}. ${source?.views ? `${source.views} views` : 'Public X search surfaced this'}${source?.likes ? `, ${source.likes} likes` : ''}${source?.replies ? `, ${source.replies} replies` : ''}. The video turns the thread into a practical workflow, not a model-release recap.`).trim(),
    why: String(p.why || 'The viewer gets one concrete AI workflow they can try today instead of just hearing another AI headline.').trim(),
    script: String(p.script || `Stop scrolling. ${title}. Here's the useful version: open the source, pull one repeatable task from it, and turn that into a reusable prompt or checklist. The win is not knowing the news. The win is having one workflow you can run today. Follow for one practical AI shortcut every day.`).trim(),
    source_url: sourceUrl,
    source_signal: typeof p.source_signal === 'string' ? p.source_signal : (source?.source_signal || null),
    // 2026-05-25 Luke flagged on Otter: each proposal must carry hard
    ...
    source_reposts: source?.reposts ?? source?.shares ?? p.source_reposts ?? p.source_shares ?? null,
    rubric: (p.rubric && typeof p.rubric === 'object') ? p.rubric : null,
    stock_queries: Array.isArray(p.stock_queries) && p.stock_queries.length >= 3
      ? p.stock_queries.slice(0, 3)
      : ['person using laptop AI workflow', 'dashboard automation closeup', 'creator editing short video'],
...
// the exact shape Luke rejected on 2026-05-11 ("Never Spend 3 Hours on Slides
...
// "trending" or "viral" alone are insufficient; Luke wants the receipt.
...
function validateProposals(proposals, opts = {}) {
  const seen = new Set();
  const genericFallbackPattern = /No proposals generated|Generation failed|regenerate|may have failed|open the source, pull one repeatable task|turns the thread into a practical workflow|not a model-release recap|AI workflow from today|Everyone is reacting to this AI workflow thread|The viewer gets one concrete AI workflow/i;
  return (proposals || []).filter((p) => {
    const text = JSON.stringify(p || {});
    const url = String(p?.source_url || '').trim();
    const title = String(p?.title || '').trim().toLowerCase();
    const scriptLead = String(p?.script || '').trim().toLowerCase().slice(0, 120);
    ...
    if (opts.requireX && !/\b(X|views?|likes?|replies?|reposts?|thread|viral|trending)\b/i.test(p.trend_hook)) return false;
...
function renderBriefingSection(date, proposals) {
  if (!Array.isArray(proposals) || proposals.length !== 10) {
    throw new Error(`TODAY'S 10 SHORTS PROPOSALS requires exactly 10 X-grounded proposals; got ${Array.isArray(proposals) ? proposals.length : 0}`);
  }
  const lines = [];
  ...
  proposals.forEach((p, i) => {
    ...
    // 2026-05-25 Luke flagged on Otter: every shorts proposal must cite
    // hard virality numbers (views/likes/shares), not vibes. Build a
    // metrics line from whatever engagement counts the source carried.
    const metrics = [];
    ...
    if (metrics.length > 0) {
      ...
      lines.push(`       Metrics: ${p.source_signal} (only one signal captured; raise SHORTS_MIN_* env vars for stricter sourcing)`);
    } else {
      lines.push('       Metrics: NONE captured. Source-quality gate may have drifted; rerun morning-shorts-proposals.js with debug.');
    }
    if (p.virality_proof) {
    ...
    // 2026-05-08 Luke dispatch: stop truncating the script at 140 chars in
  ...
  lines.push("    No carryover: tomorrow gets 10 brand new ideas regardless of what you approved here.");
  return lines.join('\n');
}
...
async function main() {
  ...
  if (!force && fs.existsSync(out)) {
    let existing;
    try { existing = JSON.parse(fs.readFileSync(out, 'utf8')); } catch { existing = null; }
    const validExisting = existing && Array.isArray(existing.proposals)
  ...
  if (signals.x.length === 0) {
    ...
    console.warn('[shorts-proposals] base X queries gave 0 qualified candidates; trying expanded virality-seeking queries');
    x = await pullXTrendingAI({ expand: true });
    signals = { x: x.slice(0, 30), date };
    if (signals.x.length === 0) {
      throw new Error('X trend research returned 0 engagement-qualified posts even after expanded query set');
    }
  }
  // Honor video rejection feedback (Luke 2026-05-18): pull every video Luke
  ...
  if (rejectionFeedback.notes.length > 0) {
    console.warn(`[shorts-proposals] honoring ${rejectionFeedback.notes.length} prior video rejection(s); excluding rejected topics and injecting feedback`);
  }
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS && byProposal.size < 10;) {
    ...
    for (let batchAttempt = batchStart; batchAttempt <= batchEnd; batchAttempt += 1) {
      ...
      batch.push(
        generateProposalsWithClaude(attemptSignals)
          ...
          .catch((e) => {
            console.warn(`[shorts-proposals] attempt ${batchAttempt}/${MAX_ATTEMPTS} failed: ${e.message}`);
            return [];
          }),
      );
    }
    ...
    for (const attemptProposals of attemptBatches) {
      for (const proposal of attemptProposals) {
        ...
        // matches a video Luke already rejected. The "same video" cannot
        // reappear unchanged regardless of how the LLM reworded the title.
        if (isAlreadyRejected(rejectionFeedback, proposal)) {
          console.warn(`[shorts-proposals] dropped re-proposal of rejected topic: ${proposal.title}`);
          continue;
        }
    ...
    if (byProposal.size >= 10) break;
    console.warn(`[shorts-proposals] attempts ${batchStart}-${batchEnd}/${MAX_ATTEMPTS}: pool ${byProposal.size}/10 rubric-passing proposals`);
  ...
  if (proposals.length !== 10) {
    throw new Error(`only ${proposals.length}/10 shorts proposals cleared X + rubric gates after ${MAX_ATTEMPTS} attempts; refusing to pad with weak entries`);
  }
  ...
  const state = {
    ...
    research_contract: 'X/trending-grounded: every proposal source_url must be an X post or thread, and the trend_hook must explain why it is hot now.',
    proposals: proposals.slice(0, 10).map((p, i) => ({ ...normalizeProposal(p, i, { title: p.title, url: p.source_url, source: 'generated' }), status: 'unapproved', approved_at: null })),
  };
  fs.writeFileSync(out, JSON.stringify(state, null, 2));

  const md = renderBriefingSection(date, state.proposals);
  console.log(md);
}
