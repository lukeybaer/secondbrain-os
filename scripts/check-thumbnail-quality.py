...
Pixel-statistic gate (no model, no API call, runs in <100ms):
  ...
  FAIL if pct_bright < 4% AND mean_luminance < 60  (no real highlights)
       unless the frame has strong asymmetric texture (dark real footage)
...
DARK_REAL_STDDEV_MIN = 35.0
DARK_REAL_LR_MAE_MIN = 25.0
DARK_REAL_NEAR_BLACK_MAX = 35.0
DARK_REAL_MIDTONE_MIN = 40.0
MIDTONE_LOW_THRESHOLD = 45
MIDTONE_HIGH_THRESHOLD = 140
...
# thumbnails ExampleCo rejected on 2026-04-29 (all 8.4-11.9) vs. the 3 known-
...
SYMMETRIC_REAL_SCENE_EDGE_MIN = 8.0
SYMMETRIC_REAL_SCENE_STDDEV_MIN = 45.0
...
def analyze(path: Path) -> dict:
    ...
    midtone = sum(hist[MIDTONE_LOW_THRESHOLD:MIDTONE_HIGH_THRESHOLD])
    pct_near_black = 100.0 * near_black / total
    pct_bright = 100.0 * bright / total
    pct_midtone = 100.0 * midtone / total
    ...
    dx = ImageChops.difference(small.crop((1, 0, w, h)), small.crop((0, 0, w - 1, h)))
    dy = ImageChops.difference(small.crop((0, 1, w, h)), small.crop((0, 0, w, h - 1)))
    spatial_detail = ImageStat.Stat(dx).mean[0] + ImageStat.Stat(dy).mean[0]
    ...
    metrics = {
        ...
        "pct_midtone": round(pct_midtone, 1),
        "lr_mirror_mae": round(lr_mae, 2),
        "spatial_detail": round(spatial_detail, 2),
    }
    ...
    dark_real_texture = (
        std_lum >= DARK_REAL_STDDEV_MIN
        and pct_near_black < PCT_NEAR_BLACK_MAX
        and (
            lr_mae >= DARK_REAL_LR_MAE_MIN
            or (
                pct_near_black <= DARK_REAL_NEAR_BLACK_MAX
                and pct_midtone >= DARK_REAL_MIDTONE_MIN
            )
        )
    )
    if (
        pct_bright < PCT_BRIGHT_MIN_FALLBACK
        and mean_lum < MEAN_LUMINANCE_FALLBACK
        and not dark_real_texture
    ):
        return {
            "ok": False,
    ...
    symmetric_real_scene = (
        spatial_detail >= SYMMETRIC_REAL_SCENE_EDGE_MIN
        and std_lum >= SYMMETRIC_REAL_SCENE_STDDEV_MIN
    )
    if lr_mae < LR_MIRROR_MAE_MIN and not symmetric_real_scene:
        return {
            "ok": False,
