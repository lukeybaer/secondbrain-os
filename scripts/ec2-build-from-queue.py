...
# records Luke's reject-with-feedback note (video_rejection_note /
# thumbnail_rejection_note). The build reads it so a regenerated video
# actually carries Luke's feedback into the render. Resolve both the EC2
...
def load_rejection_feedback(vid_id):
    """Return the latest reject-with-feedback note Luke left for this video.
    ...
    the feedback Luke types in the dashboard 'Reject and regen' box; before
    2026-05-18 it was stored but never read on regeneration, so Luke's
...
# 2026-05-05 Luke #learn: thumbnail card is a single-frame flash, not 0.25s.
...
# Locked 2026-05-03 from Luke's 6th rejection on nine_free_ai_tools_2026:
...
# Locked 2026-05-03 (v4) after Luke's rejection of the placeholder
...
def generate_voice(script, out_path, config, voice_id_override=None):
    # 2026-05-18 Luke: the narration voice MUST stay the original Amy voice
    ...
    # error the build silently shipped a different-sounding voice and Luke
    ...
    # into Luke's approval queue. Polly is only used when ALLOW_POLLY_VOICE=1
    ...
    # 2026-05-25 Luke #learn: when the rejection history says the voice
...
def add_voice_lead_pad(inp, out, pad_s=VOICE_LEAD_PAD_S):
    ...
    Luke rejected short009 on 2026-05-07 because the opening "AI" was cut off.
...
def write_ass(words, ass_path, thumb_offset=THUMBNAIL_CARD_DURATION_S, emphasis_indices=None):
    ...
    Locked 2026-05-05 by Luke ("the word popping up is crap, do way better").
    ...
    # 2026-05-06 Luke #learn: revert emphasis color from yellow to green.
    # The prior write_ass used &H0088FF00 (BGR neon green) and Luke
...
# 2026-05-04 Luke #learn: we already have a proven thumbnail skill at
...
def bright_gradient_thumbnail(title, out_path):
    ...
    has exhausted retries. Luke 2026-04-29: 'the briefing is Amy, the
...
def polish_voice(voice_in, voice_out):
    ...
    Luke's rejection 2026-05-03 said the narration was crackly. The existing
...
def pick_music_track(spec):
    ...
    # Match both AILifeHacks (legacy personal) and AILifeHacksByLukeyBaer (brand
...
def extract_list_items(spec, words):
    ...
    2026-05-06 Luke: extended so that an explicit spec["tool_list"] / spec[
    ...
    does not pattern-match but Luke explicitly wants 5 numbered tools shown.
    """
    ...
    # is what Luke asked for; the timestamp is best-effort when transcription
    # mangles it.
    explicit = spec.get("tool_list") or spec.get("items") or spec.get("list_items") or []
...
def build_numbered_overlay_ass(items, total_dur, out_path):
    ...
    Locked 2026-05-03 (v2) from Luke's "Those 1/2/3 labels look like shit
...
def render_playwright_overlay(spec, words, emphasis_indices, list_items, total_dur, out_path, work_dir):
    ...
    cmd = [
        ...
        "--channel", spec.get("channel", "AILifeHacksByLukeyBaer"),
        "--out", str(out_path),
    ]
...
def apply_regen_overrides(spec):
    ...
    Supported overrides:
      ...
      - opening_tag_override : string. Stamped on the spec so the
                               kinetic-caption renderer can pick it up
                               for the first ALLCAPS tag.
    ...
    if overrides.get("opening_tag_override"):
        spec["opening_tag_override"] = overrides["opening_tag_override"]
        print(f"  [regen-overrides] opening_tag -> {overrides['opening_tag_override']}")
...
def build_video(spec, config):
    ...
    # 2026-05-25 Luke #learn: the LLM feedback interpreter writes
    ...
    try:
        ...
        anim_result = _emit_anim(
            ...
            spec.get("channel", "AILifeHacksByLukeyBaer"),
        )
    ...
    # 3. Thumbnail
    # Luke 2026-04-29 dispatch: regen kept producing the same blank thumbs
    ...
    # 4. Stock footage. Locked 2026-05-03 from Luke's "now it's just one
    ...
    # 5a. Polish the voice (de-ess + low-pass + loudness norm), fixes Luke's
    ...
    # 5b. Mix voice with music bed, fixes Luke's "no viral/pleasant music
    ...
    # 5c. Build numbered overlay ASS file for list-style videos, fixes Luke's
    ...
    # 5d. Rich HTML/CSS motion overlay. This produces a transparent alpha MOV
...
def load_today_approved_proposal_ids():
    ...
    Locked 2026-05-04 by Luke: "If I skip, no video gets made." The
...
if __name__ == "__main__":
    ...
    # Luke 2026-04-29: auto-regen-rejected-videos.js calls this script with
    ...
    # Daily approval gate (locked 2026-05-04 by Luke #learn).
    # When run WITHOUT --id (i.e. the daily auto-generation path), only
    # proposals approved by Luke today can build. When run WITH --id (the
    ...
    for spec in queue.get("videos", []):
        ...
        # A re-queued rejected video carries a rejection note. It must ALWAYS
