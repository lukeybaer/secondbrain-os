 ...
 *
 * Locked 2026-04-30. Luke's rule: "rubric should always be updated to
 ...
 * What this script gives us is the GAP DETECTION -- the moment Luke
...
// When Luke writes "audio is too quiet", that's the volume_levels criterion.
  ...
  if (matches.length === 0) {
    ...
    for (const m of matches) {
      ...
      if (observedScore == null) {
        ...
        findings.push({
          kind: 'tool_scored_high_but_luke_rejected',
          criterion: m.criterion,
          observed_score: observedScore,
          why: m.why,
          recommendation: `the ${m.criterion} tool scored ${observedScore} but Luke still rejected -- the tool's detection logic needs to be tightened (false positive on detection, or score is gaming an easy threshold)`,
        });
      } else {
        findings.push({
