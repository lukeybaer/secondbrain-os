...
// Gmail escalation to ExampleCo.d.ExampleCo@gmail.com with the subject
...
// "heads up" line isn't getting ExampleCo to fix it either, the architecture has
...
function computeStaleRedProbes(diagDir, opts = {}) {
  ...
  for (const probe of probeNames) {
    const allRed = history.every((h) => h.checks[probe] && h.checks[probe].status === 'red');
    if (!allRed) continue;
...
// Part B: build the SAME-DAY acute alert for an unpushed-backlog stall, instead
// of waiting for the 3-day stale-red streak. Pure (no I/O) so it is testable.
// branchDivergence is the probeBranchDivergence result; failingSpecs is a
// best-effort list of the test files blocking the push (may be empty).
function buildAcuteBacklogAlert(branchDivergence = {}, failingSpecs = []) {
  const ahead = Number(branchDivergence.masterAheadOrigin) || 0;
  const ageH = Number(branchDivergence.oldestUnpushedAgeHrs) || 0;
  const specs = Array.isArray(failingSpecs) ? failingSpecs.filter(Boolean) : [];
  const blocker = specs.length
    ? `Pre-push gate blocked by failing tests: ${specs.slice(0, 6).join(', ')}` +
      (specs.length > 6 ? ` (+${specs.length - 6} more)` : '')
    : `Pre-push gate is blocking (failing tests not yet identified this run)`;
  const subject = `[Amy] Push stalled ${ageH.toFixed(0)}h - ${ahead} commits unpushed, trunk frozen`;
  const body = [
    `Local master is ${ahead} commits AHEAD of origin/master and NOT pushed.`,
    `Oldest unpushed commit is ${ageH.toFixed(1)}h old.`,
    ``,
    blocker,
    ``,
    `Why it matters: master is the shared trunk. While the push is blocked, every`,
    `concurrent Claude/Codex session is frozen out of pushing and all that work is`,
    `unbacked-up on this machine.`,
    ``,
    `Amy will NOT auto-push red and will NOT blind-fix a test. Reply HEAL to have`,
    `Amy re-run the gate and push if it is green, or HOLD to leave it.`,
  ].join('\n');
  const telegram =
    `Push stalled ${ageH.toFixed(0)}h: ${ahead} commits unpushed on master, trunk frozen. ` +
    (specs.length ? `Blocked by ${specs[0]}. ` : `Blocked by the test gate. `) +
    `Reply HEAL (re-run + push if green) or HOLD.`;
...
function draftGmailEscalation(entry, opts) {
  ...
  if (dryRun) {
    console.log(`[dry-run] would draft Gmail to ExampleCo.d.ExampleCo@gmail.com`);
  ...
  try {
    ...
    execSync(
      `python "${sendScript}" --to ExampleCo.d.ExampleCo@gmail.com --subject "${subject.replace(/"/g, '\\"')}" --body-file -`,
...
function main() {
  ...
  for (const s of result.stale) {
    ...
    console.log(
      `[escalate] ${s.probe} — ${s.streakDays}d red — drafted=${JSON.stringify(drafted)}`,
    );
  }
  process.exit(0);
}
