 ...
 * and triggers a fresh EC2 build for each. Closes the loop ExampleCo flagged
 ...
 *        - manifest.status stays pending_approval (ExampleCo reviews)
...
// E6 2026-06-12 (short002_wrongskill): rejection feedback reached this
// orchestrator but never crossed the process boundary into the build, and
// the promotion gate had no rule for the note. This lib is the single
// translation layer: notes -> production directives (for the prompt + the
// build hand-off) and notes -> implicated rubric criteria (for the
// improvement gate). See memory/feedback_regen_must_translate_rejection_into_build_param_changes.md.
const regenFeedback = require('./lib/regen-feedback-directives.js');
// 2026-05-26 ExampleCo #learn: new craft-driven pipeline. Niche YAML +
// reflection memory feed a Claude-driven planner; the executor reads
// the structured plan and runs ffmpeg mechanically. Gated by
// USE_PLANNER_PIPELINE=1 (default OFF for the migration week).
...
function describeFixFromRejection(rejection, video, result, thumbChanged, videoChanged) {
  const note = rejectionText(
    rejection,
    video.video_rejection_note || video.thumbnail_rejection_note || '',
  ).toLowerCase();
...
function shouldRegenerateVoice(video) {
  ...
  return /music|audio|voice|narration|tts|speech|pronunciation|crackly|silent|volume|sound|broken video stream|too short|near-empty|script missing/.test(
    note,
  );
}
...
function ec2BuildFailureReason(log, missingArtifactReason) {
  ...
  if (
    /quota_exceeded|exceeds your API key .*quota|credits remaining|required for this request/i.test(
      full,
    )
  ) {
    const quotaLine = (full.match(
      /This request exceeds your API key \([^)]+\) quota of \d+\. You have \d+ credits remaining, while \d+ credits are required/i,
    ) ||
      full.match(/This request exceeds[^"]+/i) ||
      full.match(/quota_exceeded[^.]+/i) || [tail])[0];
...
function classifyErrorCode(step, reason) {
  ...
  if (
    /quota_exceeded|exceeds your api key .*quota|credits remaining|required for this request|credits remain/i.test(
      r,
    )
  )
    return 'elevenlabs_quota';
  ...
  return `${step}_ExampleCo`;
}

// 2026-05-06 ExampleCo: "why can't you tell me if the reject triggered a regen?"
...
function pythonCandidates() {
  const fromEnv = [process.env.PY_LAUNCHER, process.env.PYTHON_EXE].filter(
    (p) => p && fs.existsSync(p),
  );
  const explicit = [];
  if (process.platform === 'win32') {
    // 2026-05-06 ExampleCo: Electron-spawned auto-regen kept hitting the
...
function runQualityGate(artifactPath, gatePath, missingMsg, timeoutMs = 15000) {
  ...
  for (const bin of candidates) {
    const r = spawnSync(bin, [gatePath, artifactPath], {
      encoding: 'utf-8',
      timeout: timeoutMs,
      ...HIDDEN_PROCESS,
    });
...
function regenThumbnailLocally(video, outPath) {
  ...
  const args = [
    ...
    '--channel',
    video.channel || 'AILifeHacksByExampleCoyExampleCo',
    '--rejection-note',
    video.thumbnail_rejection_note || '',
    '--out',
    outPath,
...
// 2026-05-06 ExampleCo: route video regen through the EC2 build script when SSH
...
// E6 2026-06-12: the hand-off file that ExampleCos ExampleCo's rejection feedback
// across the process boundary into ec2-build-from-queue.py. The queue spec
// (scripts/ec2-build-queue.json) never receives the note or the
// interpreter's overrides, which is exactly how short002_wrongskill rebuilt
// feedback-blind. The build reads build/<id>/regen_feedback.json (see
// load_regen_feedback_file in ec2-build-from-queue.py).
function buildRegenFeedbackPayload(v) {
  return {
    id: v.id,
    written_at: new Date().toISOString(),
    rejection_note: v.video_rejection_note || '',
    thumbnail_rejection_note: v.thumbnail_rejection_note || '',
    regen_overrides: v.regen_overrides || null,
    directives: v.regen_directives || [],
  };
}

// Once a regen is verified and promoted, remove the hand-off file so a later
// routine rebuild does not re-apply stale feedback. Best effort on both
// runtimes; the manifest note (cleared on promotion) is the primary
// self-cleaning signal.
function clearRegenFeedbackHandoff(id) {
  try {
    fs.rmSync(path.join(EC2_DATA_ROOT, 'data', 'youtube', 'build', id, 'regen_feedback.json'), {
      force: true,
    });
  } catch {
    /* best effort */
  }
  if (!isEc2LocalRuntime() && fs.existsSync(SSH_KEY)) {
    try {
      ssh(`rm -f ${EC2_DATA_ROOT}/data/youtube/build/${id}/regen_feedback.json`, {
        timeout: 30000,
      });
    } catch {
      /* best effort */
...
// E6 2026-06-12: the EC2 build path returns rubric_pass=true with no real
// rubric scores (the 22-criterion rubric never ran), so the rejection-token
// gate had nothing to verify and promoted unchanged videos. After an EC2
// build we score the regenerated artifact with the fast parallel rubric so
// the gate has real numbers for the criteria implicated by the rejection.
const RUBRIC_FAST = path.join(
  REPO,
  'src',
  'main',
  'empire',
...
function flattenRubricScores(detailed) {
  const out = {};
  for (const [k, v] of Object.entries(detailed || {})) {
    if (typeof v === 'number') out[k] = v;
    else if (v && typeof v === 'object' && typeof v.score === 'number') out[k] = v.score;
  }
  return out;
}

function scoreArtifactWithFastRubric(videoPath, video, thumbPath) {
  if (!fs.existsSync(RUBRIC_FAST)) {
    return { ok: false, reason: `fast rubric missing: ${RUBRIC_FAST}` };
  }
  if (!fs.existsSync(videoPath)) {
    return { ok: false, reason: `video missing for rubric scoring: ${videoPath}` };
  }
  const args = [RUBRIC_FAST, videoPath, '--platform', 'shorts'];
  if (video.title) args.push('--title', video.title);
  if (thumbPath && fs.existsSync(thumbPath)) args.push('--thumbnail', thumbPath);
  ...
  if (fs.existsSync(transcriptPath)) args.push('--transcript', transcriptPath);
  let lastErr = '';
  for (const bin of pythonCandidates()) {
    ...
    if (r.status === null) {
      return { ok: false, reason: `${bin} fast rubric did not exit (timeout)` };
    }
    const stdout = (r.stdout || '').trim();
    // Tools can print stray lines before the final JSON; parse from the
    // first '{' if the full trim fails.
    for (const candidate of [stdout, stdout.slice(stdout.indexOf('{'))]) {
      try {
        const parsed = JSON.parse(candidate);
        if (parsed && parsed.detailed_scores) {
          return {
            ok: true,
            overall_score: parsed.overall_score,
            virality_score: parsed.virality_score,
            scores: flattenRubricScores(parsed.detailed_scores),
          };
        }
      } catch {
        /* try next candidate */
    ...
    return {
      ok: false,
      reason: `fast rubric output unparseable (exit ${r.status}): ${(stdout || r.stderr || '').slice(0, 200)}`,
    };
  }
  return { ok: false, reason: `fast rubric could not run any python: ${lastErr}` };
}

function canRegenViaEc2() {
  if (isEc2LocalRuntime()) {
    regenEvent('ec2_probe', {
      ok: true,
      reason: 'local_ec2_runtime',
      keyPath: null,
      keyExists: false,
    });
    return true;
  ...
  const keyPath =
    process.env.SECONDBRAIN_SSH_KEY ||
...
function regenVideoOnEc2(v, localMp4, localThumb) {
  ...
  // E6 2026-06-12: hand the rejection context to the build subprocess. The
  // queue spec never ExampleCos it; without this file the EC2 rebuild is
  // feedback-blind, so a push failure is a build failure (fail loud).
  const feedbackTmp = path.join(os.tmpdir(), `regen-feedback-${v.id}.json`);
  try {
    fs.writeFileSync(feedbackTmp, JSON.stringify(buildRegenFeedbackPayload(v), null, 2));
    execSync(`ssh -i "${keyPath}" -o StrictHostKeyChecking=no ${host} "mkdir -p ${buildDir}"`, {
      stdio: 'pipe',
      timeout: 60000,
      ...HIDDEN_PROCESS,
    });
    execSync(
      `scp -i "${keyPath}" -o StrictHostKeyChecking=no "${feedbackTmp.replace(/\\/g, '/')}" ` +
        `${host}:${buildDir}/regen_feedback.json`,
      { stdio: 'pipe', timeout: 60000, ...HIDDEN_PROCESS },
    );
  } catch (e) {
    return {
      ok: false,
      reason: `could not push regen_feedback.json to EC2 (build would be feedback-blind): ${e.message.slice(0, 200)}`,
    };
  }
  // FORCE_REBUILD=1 so the build re-downloads stock footage instead of
  // reusing the literal clips ExampleCo rejected (E6: this path previously
  // cleared only assembly outputs, never the cached Pexels clips).
  const clearCmd =
    `rm -f ${filesToClear.join(' ')} ; ` +
    `cd ${EC2_DATA_ROOT} && FORCE_REBUILD=1 python3 -u scripts/ec2-build-from-queue.py --id ${v.id}`;
  const t0 = Date.now();
  const r = spawnSync(
    'ssh',
    ['-i', keyPath, '-o', 'StrictHostKeyChecking=no', '-o', 'ConnectTimeout=20', host, clearCmd],
  ...
  if (!pullOk) {
    return {
      ok: false,
      reason: ec2BuildFailureReason(buildLog, `ec2 mp4 pull failed after 3 attempts: ${pullErr}`),
    };
  }
  if (!fs.existsSync(localMp4)) {
    return {
      ok: false,
      reason: ec2BuildFailureReason(buildLog, 'ec2 build claimed success but no local mp4'),
    };
  }
  ...
  const eventCounts =
    playwrightReceipt && playwrightReceipt.event_counts ? playwrightReceipt.event_counts : {};
...
function regenVideoOnLocalEc2Build(v, localMp4, localThumb) {
  let plannerPlanPath = null;
  const buildDir = path.join('/opt/secondbrain/data/youtube/build', v.id);
  try {
    fs.mkdirSync(buildDir, { recursive: true });
  } catch {
    /* build creates it too */
  }
  if (fs.existsSync(localThumb)) {
    try {
      fs.copyFileSync(localThumb, path.join(buildDir, 'thumbnail.jpg'));
    } catch (e) {
  ...
  // (scripts/lib/plan-video-build-cli.js + scripts/build-from-plan.py).
  // Otherwise, run the legacy ec2-build-from-queue.py path but stamp
  // the spec with the channel niche YAML so daily-generation does not
  // drift from the craft contract (Codex review issue #4 prevention).
  const usePlannerPipeline = process.env.USE_PLANNER_PIPELINE === '1';

  // Best-effort niche YAML load. Missing YAML is loud-on-stderr but
  // does not crash the legacy path: NICHE_NOT_FOUND just means the
  // channel does not yet have a profile authored, and the legacy
  // hardcoded heuristics keep running. The planner path WILL fail
  // closed if it tries to run without a niche.
  let nicheObj = null;
  try {
    nicheObj = loadNiche(v.channel);
    // Stamp the in-memory spec used by the legacy fall-through. The
    // file on disk is owned by ec2-build-from-queue.py; we don't
    // mutate the queue file here.
    v = stampNicheOntoSpec(v, nicheObj);
  } catch (e) {
    console.warn(
      `  niche YAML missing or invalid for ${v.channel}: ${String(e.message || e).slice(0, 160)}`,
    );
  }
  ...
  if (usePlannerPipeline) {
    ...
    const planR = spawnSync(
      'node',
      [PLAN_VIDEO_BUILD_CLI, '--spec-json', specJson, '--niche', v.channel, '--out', planPath],
      {
        cwd: REPO,
        encoding: 'utf-8',
        timeout: 5 * 60 * 1000,
    ...
    // also expose plan_path: relying on the v mutation crossing the
    // function boundary has proven unreliable in this codebase (something
    // upstream rebuilds v from manifest between calls). Pass plan_path
    // back through the result object so the outer gates can read it from
    // auroraVideoResult.plan_path.
    plannerPlanPath = planPath;
    console.log(`  plan written -> ${planPath} (v.plan_path=${v.plan_path} v.id=${v.id})`);

    // 2. Execute via build-from-plan.py.
    const execR = spawnSync('python3', ['-u', BUILD_FROM_PLAN_PY, '--plan', planPath], {
      cwd: '/opt/secondbrain',
      encoding: 'utf-8',
      timeout: 30 * 60 * 1000,
  ...
  var r =
    typeof r !== 'undefined' && r
      ...
      : spawnSync('python3', ['-u', 'scripts/ec2-build-from-queue.py', '--id', v.id], {
          ...
          timeout: 30 * 60 * 1000,
          maxBuffer: 10 * 1024 * 1024,
          // 2026-05-25 ExampleCo #learn: the playwright overlay's sentenceToCard
          // produces junk titles ("2026", "-,") because the clause splitter
          // treats every hyphen as a clause boundary and the stopword list
          // strips short leading words. Until card derivation is rewritten,
          // the regen path ships videos without the tag overlay: kinetic
          // captions and animations.ass still render. Caller can still
          // re-enable explicitly via DISABLE_PLAYWRIGHT_OVERLAY=0.
          env: {
            ...process.env,
            FORCE_REBUILD: '1',
            DISABLE_PLAYWRIGHT_OVERLAY: process.env.DISABLE_PLAYWRIGHT_OVERLAY ?? '1',
          },
          ...HIDDEN_PROCESS,
        });
  const elapsed = ((Date.now() - t0) / 1000).toFixed(1);
  ...
  const builtCandidates = [
    // New planner-driven executor (scripts/build-from-plan.py) writes here.
    path.join('/opt/secondbrain/data/youtube/build-from-plan', v.id, 'final.mp4'),
    // Legacy ec2-build-from-queue.py paths.
  ...
  if (fs.existsSync(builtThumb)) {
    try {
      fs.copyFileSync(builtThumb, localThumb);
    } catch {
      /* not fatal */
    }
  }
  ...
  const numberedOverlayScore = stepCardCount + assNumberCount > 0 ? 100 : null;
  const rubricScores =
    numberedOverlayScore == null ? {} : { numbered_overlays: numberedOverlayScore };
  return {
    ok: true,
    size_bytes: stat.size,
    timing_s: { total_s: elapsed },
...
function regenVideoLocally(video, outPath, sourcePath, thumbPath) {
  ...
  // into ExampleCo's queue -- the failure mode that produced the "awkward
  ...
  // Locked 2026-05-01 from ExampleCo's "no video diversity did this pass QC?"
  ...
  if (!hasTranscript) {
    const transcribePy = path.join(REPO, 'src', 'main', 'empire', 'transcribe_word_level.py');
    if (fs.existsSync(transcribePy)) {
      ...
      for (const bin of tcCandidates) {
        ...
        if (
          ...
          fs.statSync(transcriptPath).size > 100
        ) {
          hasTranscript = true;
      ...
      if (!hasTranscript) {
        console.warn(
          `  transcribe failed (${lastTrErr}); will fall back to legacy v1 build (rubric will not gate this)`,
        );
      }
    }
  }
...
const SSH_KEY =
  process.env.SECONDBRAIN_SSH_KEY || path.join(os.homedir(), '.ssh', 'secondbrain-backend-key.pem');
const EC2_HOST = process.env.EC2_HOST || 'ec2-user@ExampleCo';
  ...
  try {
    if (fs.existsSync(REGEN_LOCK_PATH)) {
      ...
      if (age < REGEN_LOCK_STALE_MS) {
        ...
        return {
          ok: false,
          reason: `another auto-regen instance is running (pid=${holder}, lock age ${Math.round(age / 1000)}s)`,
        };
      }
      // Stale, reclaim.
      try {
        fs.unlinkSync(REGEN_LOCK_PATH);
      } catch {}
    }
    ...
    return {
      ok: false,
      reason: 'lock acquisition failed: ' + (e && e.message ? e.message : String(e)),
    };
  }
}
...
function main() {
  ...
  const isAbandonedNoArtifactStub = (v) => {
    if (!v || v.video_file) return false;
    const regenStatus = String(v.regen_status || '').toLowerCase();
    if (regenStatus === 'dead-letter') return true;
    if (regenStatus !== 'failed') return false;
    const evidence = [
      v.regen_error,
      v.regen_hard_block_reason,
      v.video_rejection_note,
      v.thumbnail_rejection_note,
    ]
      .filter(Boolean)
      .join(' ')
      .toLowerCase();
  ...
  if (!force) {
    candidates = candidates.filter(
      (v) => !['amy_review_queued', 'amy_review_in_progress'].includes(v.status),
    );
  }
  ...
  if (candidates.length === 0) {
    console.log(
      'no work to do (no videos with video_needs_regen=true or thumbnail_needs_regen=true' +
        (filterId ? ` matching id ${filterId}` : '') +
        ')',
    );
    console.log(
      'amy_review_queued items wait for Claude/Amy review. To override: node scripts/auto-regen-rejected-videos.js --force --id X',
    );
    return;
  }
  ...
  for (const v of candidates) {
    ...
    for (const step of stepGates) {
      ...
      if (gate.action === 'dead-letter') {
        console.error(
          `  dead-letter: ${step} attempts exhausted (${gate.attemptsSoFar}/${gate.lastDecision?.maxRetries}); skipping`,
        );
        regenEvent('dead_letter', {
          id: v.id,
          step,
          attempts: gate.attemptsSoFar,
          last_error_code: gate.lastDecision?.errorCode,
        });
        deadLetterAll = true;
    ...
    // 2026-05-24 ExampleCo gap: load the full rejection history from
    ...
    try {
      ...
      if (history.totalCount > 0) {
        ...
        console.log(
          `  loaded ${history.totalCount} prior rejection note(s) into the regen context`,
        );
      }
    } catch (e) {
      historyLoadError = e;
    ...
    // E6 2026-06-12: fail loud if a flagged video has no rejection context.
    const rejCtx = regenFeedback.resolveRejectionContext({
      manifestEntry: v,
      historyCount,
      historyError: historyLoadError,
    });
    if (rejCtx.failLoud) {
      console.error(`  FAIL LOUD: ${rejCtx.reason}`);
      v.regen_status = 'failed';
      v.regen_error = rejCtx.reason;
      v.regen_failed_at = new Date().toISOString();
      regenEvent('regen_fail_loud', { id: v.id, reason: rejCtx.reason });
      continue;
    }

    // E6 2026-06-12: translate rejection notes into concrete directives so
    // the build subprocess gets actionable instructions, not just raw text.
    try {
      const rejRows = regenFeedback.loadRejectionRows(v.id);
      const directives = regenFeedback.buildRegenDirectives(rejRows);
      if (directives.length > 0) {
        v.regen_directives = directives;
        regenEvent('regen_directives_built', { id: v.id, count: directives.length });
        console.log(`  built ${directives.length} production directive(s) from rejection history`);
      }
    } catch (e) {
      console.warn(`  could not build regen directives: ${String(e.message || e).slice(0, 160)}`);
    }

    // 2026-05-25 ExampleCo #learn: pass the rejection history through the
    ...
    try {
      ...
      if (fs.existsSync(interpreterCli)) {
        const cliR = spawnSync(
          'node',
          [interpreterCli, '--id', v.id, '--title', v.title || '', '--channel', v.channel || ''],
          {
            encoding: 'utf-8',
            timeout: 90_000,
        ...
        if (cliR.status === 0 && cliR.stdout && cliR.stdout.trim()) {
          try {
            ...
            if (overrides && !overrides.error) {
              ...
              console.log(
                `  interpreted rejections into overrides (confidence ${overrides.fix_confidence}; ${(overrides.banned_phrases || []).length} banned phrase(s); hook=${!!overrides.hook_phrase}, voice=${!!overrides.voice_id_override}, density=${overrides.animation_density || 'n/a'})`,
              );
            } else if (overrides && overrides.error) {
              console.warn(
                `  interpreter returned error: ${overrides.error}; build will use raw note only`,
              );
          ...
          console.warn(
            `  interpreter CLI exit ${cliR.status}: ${(cliR.stderr || '').slice(0, 160)}`,
          );
        }
      }
    } catch (e) {
      console.warn(
        `  interpreter wire-up error (non-fatal, build proceeds): ${String(e.message || e).slice(0, 160)}`,
      );
    }
    ...
    try {
      ...
      if (v.thumbnail_needs_regen) {
        ...
        if (!auroraResult.ok) {
          ...
          const recorded = regenRetry.recordAttempt({
            videoId: v.id,
            step: 'thumbnail',
            lastError: auroraResult.reason || 'ExampleCo',
            errorCode: classifyErrorCode('thumbnail', auroraResult.reason),
            rejectionNote: (v.thumbnail_rejection_note || '').slice(0, 240),
          });
          regenEvent('thumb_fail', {
            id: v.id,
            reason: auroraResult.reason,
            attempt: recorded.decision.attempt,
        ...
        regenEvent('thumb_done', {
          id: v.id,
          size_kb: Math.round((auroraResult.size_bytes || 0) / 1024),
          attempts: auroraResult.attempts?.length,
        });
        console.log(
          `  thumbnail done (${(auroraResult.size_bytes / 1024).toFixed(0)}KB, attempts=${auroraResult.attempts.length}, lr_mae=${auroraResult.metrics?.lr_mirror_mae})`,
        );
      ...
      if (v.video_needs_regen) {
        // 2026-05-06 ExampleCo: route through EC2 build script when SSH key exists
        ...
        if (useEc2) {
          console.log(
            '  regenerating video via EC2 (full skill stack: kinetic captions + animations + Bedrock thumb)...',
          );
        ...
        if (!auroraVideoResult.ok) {
          ...
          const recorded = regenRetry.recordAttempt({
            videoId: v.id,
            step: 'video',
            lastError: auroraVideoResult.reason || 'ExampleCo',
            errorCode,
            rejectionNote: (v.video_rejection_note || '').slice(0, 240),
          });
          regenEvent('video_fail', {
            id: v.id,
            reason: auroraVideoResult.reason,
            attempt: recorded.decision.attempt,
            exhausted: recorded.exhausted,
          });
          regenEvent('regen_end', {
        ...
        regenEvent('video_done', {
          id: v.id,
          size_mb: parseFloat(sizeMb),
          elapsed_s: totalS,
          path: useEc2 ? 'ec2' : 'local_aurora',
        });
        ...
        // 2026-05-26 ExampleCo #learn: thread plan_path explicitly via the
        // result object. The in-function v.plan_path mutation does not
        // survive the function-call boundary in this codebase (something
        // upstream re-reads v from manifest); ride the result instead.
        if (auroraVideoResult.plan_path) {
          v.plan_path = auroraVideoResult.plan_path;
        }
      }
      ...
      // If the file is byte-identical to what ExampleCo just rejected, treat
      ...
      if (failures.length > 0) {
        ...
        // NOT cycle this back into pending_approval. ExampleCo already saw
      ...
      // intact and never cycle a blank thumbnail back to ExampleCo.
      if (thumbChanged) {
      ...
      if (videoChanged) {
        ...
        const isPlannerBuild = !!v.plan_path;
        console.log(
          `  [video-gate-debug] v.id=${v.id} v.plan_path=${JSON.stringify(v.plan_path)} isPlannerBuild=${isPlannerBuild} videoQuality.ok=${videoQuality.ok}`,
        );
        if (!videoQuality.ok && !isPlannerBuild) {
          v.status = 'video_rejected';
          ...
          if (videoQuality.metrics) {
            ...
            v.regen_video_metrics = {
              ...(v.regen_video_metrics || {}),
              content_gate: videoQuality.metrics,
            };
          }
          ...
          const recorded = regenRetry.recordAttempt({
            videoId: v.id,
            step: 'video',
            lastError: videoQuality.reason || 'video content gate failed',
            errorCode: classifyErrorCode('video', videoQuality.reason),
            rejectionNote: (v.video_rejection_note || '').slice(0, 240),
          });
          ...
          regenEvent('video_gate_fail', {
            id: v.id,
            reason: videoQuality.reason,
            attempt: recorded.decision.attempt,
            exhausted: recorded.exhausted,
          });
          regenEvent('regen_end', {
        ...
        if (!videoQuality.ok && isPlannerBuild) {
          // 2026-05-26 ExampleCo #learn: gate was tuned for the OLD truly-blank
          // ec2-build-from-queue.py failure mode. Planner builds with dim
          // legitimate Pexels stock produce false positives here. Record
          // the advisory metric but DO NOT block promotion. Pure-blank
          // mp4s still get caught by mp4-size checks and ffprobe duration
          // mismatches earlier in the pipeline.
          console.warn(
            '  video content gate advisory (planner build, not blocking):',
            videoQuality.reason,
          );
          v.content_gate_planner_advisory_reason = videoQuality.reason;
        }
        if (videoQuality.metrics) {
          v.regen_video_content_metrics = videoQuality.metrics;
          v.regen_video_metrics = {
      ...
      // 2026-05-24 ExampleCo gap: stamp the per-artifact resolved_at on a fresh
      ...
      if (ranRubric) {
        if (auroraVideoResult.rubric_pass) {
          ...
          // actually address the latest rejection note. If ExampleCo rejected
          ...
          if (!tokenResult.ok) {
            ...
            v.regen_unaddressed_rejection = tokenResult.failures.map((f) => f.reason).join('; ');
          ...
          // Promoting would surface a broken artifact to ExampleCo. See
          // src/main/__tests__/auto-regen-manifest-cleanliness.test.ts.
          const gateAfter =
            (auroraVideoResult &&
              auroraVideoResult.metrics &&
              auroraVideoResult.metrics.content_gate) ||
            (v.regen_video_metrics && v.regen_video_metrics.content_gate) ||
            null;
          // 2026-05-26 ExampleCo #learn: the content gate was designed against
          // the OLD failure mode where ec2-build-from-queue.py produced
          // truly-blank video streams (the 2026-04-29 short009 incident).
          // The planner pipeline has its own semantic validator + niche
          // brightness_bias + Reflexion-style channel memory; dim-but-real
          // stock footage from Pexels produces frequent false positives
          // here. When this build came from the planner (v.plan_path set),
          // skip the strict blank-frame guard. Pure-blank video still gets
          // caught by other checks (mp4 size, ffprobe duration mismatch).
          const isPlannerBuild = !!v.plan_path;
          console.log(
            `  [gate-debug] v.id=${v.id} v.plan_path=${JSON.stringify(v.plan_path)} isPlannerBuild=${isPlannerBuild} samples_blank=${gateAfter && gateAfter.samples_blank}`,
          );
          if (gateAfter && Number(gateAfter.samples_blank || 0) > 0 && !isPlannerBuild) {
            ...
            const recorded = regenRetry.recordAttempt({
              videoId: v.id,
              step: 'video',
              lastError: v.video_rejection_note,
              errorCode: classifyErrorCode('video', v.video_rejection_note),
              rejectionNote: v.video_rejection_note.slice(0, 240),
            });
            if (recorded.exhausted) {
              v.regen_status = 'dead-letter';
            ...
            regenEvent('video_gate_fail', {
              id: v.id,
              reason: v.video_rejection_note,
              attempt: recorded.decision.attempt,
              exhausted: recorded.exhausted,
            });
            console.log(
              `  content_gate blank-frame guard: refusing to promote ${v.id}, demoted to video_rejected and re-queued for regen.`,
            );
            continue;
          }
          if (gateAfter && Number(gateAfter.samples_blank || 0) > 0 && isPlannerBuild) {
            console.log(
              `  content_gate flagged ${gateAfter.samples_blank}/${gateAfter.samples_taken} frames but isPlannerBuild; recording metric, NOT blocking promotion.`,
            );
            v.content_gate_planner_advisory = `${gateAfter.samples_blank} dim sampled frame(s); not blocking because planner pipeline has its own validators.`;
          }
          ...
          delete v.regen_hard_blocked;
          delete v.regen_hard_block_reason;
          // 2026-05-06 ExampleCo: 'I still don't see the comments from last time
          ...
          if (latestRejection) {
            // 2026-05-25 ExampleCo flagged: the video approval card showed
            ...
            if (rejectionTargetId && rejectionTargetId !== v.id) {
              console.warn(
                `[auto-regen] WARNING: latestRejection id=${rejectionTargetId} does not match video id=${v.id}; skipping feedback_history append to avoid cross-video recycling`,
              );
            }
            const historyEntry = {
              video_id: v.id,
              ...
              rejected_at:
                latestRejection.rejectedAt || v.regen_started_at || new Date().toISOString(),
            ...
            const existingIdx = (v.feedback_history || []).findIndex(
              (h) => rejectionText(h.note || h).trim() === historyEntry.note.trim(),
            );
            if (existingIdx >= 0) {
              v.feedback_history[existingIdx] = {
                ...v.feedback_history[existingIdx],
                ...historyEntry,
              };
            } else {
          ...
          console.log(
            `  manifest entry updated -> pending_approval (rubric cleared, rejection tokens verified)`,
          );
        } else {
          v.regen_status = 'rubric_failed';
          // Status stays at 'video_rejected' -- do NOT cycle a video that
          // failed the rubric back into ExampleCo's queue.
...
// 2026-05-24 ExampleCo gap: "re-queue those videos with the rejection history,
...
function assembleFullRejectionHistory(id, opts = {}) {
  ...
  let raw;
  try {
    raw = fs.readFileSync(rejectionsPath, 'utf8');
  } catch {
    return empty;
  }
  const rows = raw
  ...
  rows.forEach((r, ix) => {
    ...
    if (!note) return;
    const line = `[${ix + 1}] ${date} (${target || 'ExampleCo-target'}): ${note}`;
    if (target === 'video') lines.video.push(line);
    else if (target === 'thumbnail') lines.thumbnail.push(line);
    else {
      lines.video.push(line);
      lines.thumbnail.push(line);
    }
  });
