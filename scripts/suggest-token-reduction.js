...
function buildSuggestion(summary) {
  if (!summary) {
    return {
      ...
      suggestion: 'Insufficient data — yesterday\'s token-usage file is missing. Run scripts/collect-daily-token-usage.js manually or check the health-self-heal log to see why the 5:15 AM collector did not drop a file.',
      suggested_changes: [
        {
          where: 'collector pipeline',
          what: 'Run "cd secondbrain && node scripts/collect-daily-token-usage.js" to regenerate yesterday\'s usage file, then re-run the suggester. If it still fails, check that ~/.claude/projects/*/*.jsonl contain timestamps for yesterday in America/Chicago timezone.',
          estimated_savings: 'N/A — diagnostic step',
          why: 'No usage data means no suggestion. Fix the collector first.',
        },
      ],
      diagnostics: null,
    };
...
function main() {
  ...
  const files = loadUsageFiles(target);
  const summary = summarizeForPrompt(files, target);
  const built = buildSuggestion(summary);
