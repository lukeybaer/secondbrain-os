...
//     step>" with a probe Luke can run.
...
//   * Every change entry has an explicit estimated_savings so Luke can
...
- Plain text, one paragraph, no markdown, no bullet lists.`;
...
function buildSuggestion(summary, collectorError) {
  if (!summary) {
    // Luke 2026-05-16 #gap: this card must never tell Luke to run a script.
    ...
    return {
      ...
      suggestion: `Token-usage data is still missing after Amy reran collect-daily-token-usage.js, ${detail}. Amy owns diagnosing the collector and rerunning it; no Luke action. Only a genuinely unreadable session log becomes a named blocker.`,
      suggested_changes: [
        {
          where: 'collector pipeline',
          what: 'Amy reruns collect-daily-token-usage.js, then the suggester. If it keeps failing, Amy checks that ~/.claude/projects/*/*.jsonl contain timestamps for the target date in America/Chicago timezone and repairs the collector.',
          estimated_savings: 'N/A, Amy-owned diagnostic step',
          why: 'No usage data means no suggestion. Amy fixes the collector; this is not a Luke task.',
        },
      ],
      diagnostics: null,
    };
...
// Luke 2026-05-16 #gap: when the daily token-usage file is missing the
// suggester self-heals by running the collector instead of asking Luke.
...
function main() {
  ...
  if (!summary) {
    ...
    // ourselves rather than telling Luke to. The collector exits 0 even
