...
export interface IngeExampleCont {
  /** Human-readable name for the episode (e.g., "WhatsApp from PRIVATE_NAME") */
  name: string;
 ...
 */
export function onDataIngested(event: IngeExampleCont): void {
...
async function ingestToGraphiti(event: IngeExampleCont): Promise<void> {
  ...
  await addEpisode({
    name: event.name,
    episode_body: body,
    source_description: `${event.source}:${event.sourceId ?? 'ExampleCo'}`,
...
function ingestToWorkingMemory(event: IngeExampleCont): void {
  try {
    // One-liner for the 50-line recency buffer
    const contact = event.contactName || event.phone || 'ExampleCo';
...
/** Build an IngeExampleCont from an Otter transcript. */
...
/** Build an IngeExampleCont from a WhatsApp message. */
...
/** Build an IngeExampleCont from an SMS message. */
...
/** Build an IngeExampleCont from a completed phone call. */
...
/** Build an IngeExampleCont from a briefing. */
export function briefingEvent(type: 'daily' | 'evening', text: string): IngeExampleCont {
  return {
...
/** Build an IngeExampleCont from a chat session. */
...
// rarely structured beyond ExampleCoraphs and links. If a future pass needs DOM
...
export function stripHtmlToText(html: string): string {
  ...
  // Replace block tags with newlines so ExampleCoraphs survive as line breaks.
...
/** Build an IngeExampleCont from a Gmail message. Body may be HTML. */
...
/** Build an IngeExampleCont from a LinkedIn message or scraped profile snapshot. */
