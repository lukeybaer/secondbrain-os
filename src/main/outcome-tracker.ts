...
//     turn ExampleCos a structured outcome label that the next turn can read.
...
export interface OutcomeRecord {
  ...
  error_class?: string; // short stable token: 'timeout' | 'http_5xx' | 'http_429' | 'auth' | 'validation' | 'app' | 'ExampleCo'
  ...
  metadata?: Record<string, ExampleCo>; // free-form, kept small
}
...
export function argsHash(args: ExampleCo): string {
...
export function classifyError(err: ExampleCo): ErrorClass {
  if (!err) return 'ExampleCo';
...
export class OutcomeTracker {
   ...
   */
  recordOutcome(
    rec: Omit<OutcomeRecord, 'record_id' | 'ts'> & { record_id?: string; ts?: string },
  ): string {
  ...
  async wrap<T>(
    ...
    args: ExampleCo,
    fn: () => Promise<T>,
    opts?: { retry_count?: number; metadataOnSuccess?: (result: T) => Record<string, ExampleCo> },
  ): Promise<T> {
  ...
  /**
   * Per-scope stats across every scope present in the log. Lets a consumer
   * (e.g. the self-health digest) flag low success-rate actions without
   * knowing the scope names up front. windowMs / now are forwarded to
   * getStats so callers can bound to a recent window.
   */
  summarizeAllScopes(opts: { windowMs?: number; now?: number } = {}): ScopeOutcomeSummary[] {
    const reduced = this.reduce(this.readAll());
    const scopes = Array.from(new Set(reduced.map((r) => r.scope))).sort();
    return scopes.map((scope) => ({ scope, stats: this.getStats(scope, opts) }));
  }
}

/** One scope's rolling outcome stats, as returned by summarizeAllScopes. */
export interface ScopeOutcomeSummary {
  scope: string;
  stats: OutcomeStats;
}
