...
export function scanForPii(text: string): PiiDetection[] {
  ...
  // Collect every pattern's first match, then resolve overlaps. Several
  // patterns overlap by construction: Passport ([A-Z]{1,2}\d{6,9}) and Driver
  // License ([A-Z]{1,2}\d{5,8}) both match one alphanumeric token, and the
  // labeled patterns (Bank Account "account #: 1234567890", Routing) share
  // their digit core with the generic SSN/Phone patterns. Without resolution a
  // single value fires several near-identical alerts.
  const raw: RawMatch[] = [];
  PII_PATTERNS.forEach(({ name, regex }, order) => {
    const match = regex.exec(text);
    if (!match || isSafeMatch(text, match[0])) return;
    raw.push({
      name,
      start: match.index,
  ...
  // Longest match wins each overlap (the most specific detection, e.g. the
  // labeled "Routing Number" over the bare 9-digit SSN shape); ties break by
  // pattern priority. Non-overlapping matches are all kept.
  raw.sort((a, b) => b.end - b.start - (a.end - a.start) || a.order - b.order);
  const detections: PiiDetection[] = [];
  const claimed: Array<[number, number]> = [];
  for (const m of raw) {
    if (claimed.some(([s, e]) => m.start < e && m.end > s)) continue;
    claimed.push([m.start, m.end]);
    detections.push({ piiType: m.name, snippet: redact(text, m.value) });
  }
  return detections;
}
