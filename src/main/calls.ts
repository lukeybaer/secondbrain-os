...
import { heuristicCallScore, callScoreToRunScore } from './call-score';
...
const CALL_TYPE_KEYWORDS: Record<string, string[]> = {
  ...
  contractor: ['contractor', 'plumber', 'elecExampleCon', 'handyman', 'repair'],
...
export async function refreshCallStatus(
  ...
  if (justEnded && transcript) {
    ...
    // Post-hoc call-quality score into the decoupled run-score store, keyed by
    // the call id. Heuristic (no LLM cost) so every finished call gets a
    // goal-attainment score that trends in the observability digest alongside
    // briefing/heal quality. Best-effort: scoring must never break the refresh.
    try {
      const dir = getConfig().dataDir;
      if (dir) {
        const callScore = heuristicCallScore(transcript, existing.instructions);
        appendScore(
          path.join(dir, 'agent', 'run-scores.jsonl'),
          callScoreToRunScore(updated.id, callScore, 'heuristic', new Date().toISOString()),
        );
      }
    } catch (scoreErr: any) {
      console.warn(`[calls] call scoring failed: ${scoreErr.message}`);
    }
  }
...
async function buildCallbackAssistantConfig(callerPhone: string): Promise<object> {
  ...
  // firstMessage is the literal opener Vapi speaks to every inbound caller and
  // persists on the static assistant config. It MUST be neutral, never contain
  // the owner's name. Owner-warmth ("Hey ExampleCo, what's going on?") is enforced
  // through the system prompt and Amy's caller-id behavior, not by mutating
  // the persistent firstMessage. Locked by vapi-assistant-is-claude.test.ts
  // (2026-05-10) after Codex repeatedly re-introduced owner-flavored static
  // greetings that embarrassed ExampleCo when strangers got greeted as him.
  const firstMessage =
    incomplete.length > 0
      ? `Hi, this is Amy. Thanks for calling back!`
      : history.length > 0
        ? `Hi, this is Amy. Good to hear from you.`
        : `Hi, this is Amy speaking, how can I help you?`;
...
export async function fetchAndSyncInboundCalls(): Promise<void> {
  ...
  const phoneNumberIds = Array.from(
    new Set([config.vapiPhoneNumberId, config.vapiInboundPhoneNumberId].filter(Boolean)),
  );
  if (!config.vapiApiKey || phoneNumberIds.length === 0) return;

  const callsById = new Map<string, any>();
  for (const phoneNumberId of phoneNumberIds) {
    let res: Response;
    try {
      res = await fetch(`https://api.vapi.ai/call?phoneNumberId=${phoneNumberId}&limit=20`, {
        headers: { Authorization: `Bearer ${config.vapiApiKey}` },
      });
    } catch {
      continue;
  ...
  const vapiCalls = Array.from(callsById.values());
  for (const vc of vapiCalls) {
    ...
    const record: CallRecord = {
      ...
      phoneNumber: callerPhone || 'ExampleCo',
...
export async function handleVapiFunctionCall(
  ...
  if (functionName === 'run_claude_code') {
    ...
    return { result: 'Task queued. the owner will be called back when complete.' };
  }
  ...
  if (functionName === 'bridge_in_owner') {
    ...
    const topic: string = parameters.topic ?? 'ExampleCo topic';
    ...
    try {
      const result = await initiateBridgeIn(
        parameters.live_call_id ?? 'ExampleCo',
        callerPhone,
        callerName,
        topic,
      );
      ...
      return { result: "I had trouble connecting to the owner. I'll pass along your message." };
    }
  }
  ...
  if (functionName === 'manage_task') {
    ...
    const action = parameters.action ?? 'ExampleCo';
  ...
  if (functionName === 'send_message') {
    ...
    return { result: `Channel "${parameters.channel ?? 'ExampleCo'}" not available yet.` };
  }
...
export interface BridgeSession {
  ...
  caller_phone: string;
  bridge_call_id?: string; // Vapi call ID to the owner's private SIM
...
/**
 * Initiate bridge-in: put caller on hold, call the owner's private SIM,
 ...
 *   2. Initiate outbound call to the owner's private SIM
 *   3. the owner answers → EA: "You have [caller] about [topic]. Bridge in?"
 *   4. the owner says "Yes" → Vapi conference transfer merges both legs
 *   5. EA drops off, caller+the owner continue directly
 */
