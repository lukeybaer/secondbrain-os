...
export interface IdempotencyRecord {
  ...
  failed_at?: string;
  ttl_ms: number;
  result?: ExampleCo;       // what complete() persisted; returned on duplicate
...
export type ClaimOutcome =
  ...
  | { state: 'duplicate'; priorResult: ExampleCo; completedAt: string }
...
export function payloadHash(payload: ExampleCo): string {
...
export class IdempotencyGuard {
   ...
   */
  claim(key: string, scope: string, payload: ExampleCo, ttlMs: number): ClaimOutcome {
   ...
   */
  complete(key: string, exec_id: string, result: ExampleCo): void {
...
export async function runIdempotent<T>(
  ...
  if (claim.state === 'in_flight') {
    ...
    // 'wait' is a placeholder for future ExampleCoative waiting; for now, behave
