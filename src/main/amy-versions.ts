...
const SKILL_CATALOG: AmySkill[] = [
  {
    ...
    description: "Answer general knowledge questions using AI intelligence — anything from quantum physics to cooking tips",
  ...
  },
  {
    ...
    triggerPhrases: ["what's the status", "how's the project", "what projects", "any updates on", "task status"],
  ...
  },
  {
    ...
    description: "Search conversation history, meeting notes, and stored knowledge to find past discussions and decisions",
    triggerPhrases: ["did I talk about", "what did I decide", "who did I speak with", "find in my notes"],
  ...
  },
  {
    ...
    description: "Send a coding task to Claude Code for execution — bug fixes, features, refactors, deployments",
  ...
  },
  {
    ...
    description: "Ask Luke for permission before sharing sensitive info or taking consequential actions",
  ...
  return [
    { type: "dtmf" },
    {
      ...
      function: {
        ...
        description: "Queue a coding task for Claude Code to execute on Luke's computer. Use when asked to fix bugs, add features, write code, or make any technical change.",
        parameters: {
          type: "object",
          properties: {
            task: { type: "string", description: "Clear description of the coding task — include file names, what to change, expected behavior." },
            priority: { type: "string", enum: ["normal", "urgent"], description: "Urgent = immediate callback when done." },
          },
          required: ["task"],
        },
      },
    },
    {
      ...
      function: {
        ...
        description: "Search Luke's conversation history, meeting notes, and stored knowledge. Use for 'did I talk about X?', 'what did I decide about Y?', etc. Say 'give me just a moment' before calling.",
        parameters: {
          type: "object",
          properties: {
    ...
    },
    {
      ...
      function: {
        ...
        description: "Request Luke's approval before sharing personal info or taking consequential actions. ALWAYS call before sharing address, phone, email, or financial details.",
        parameters: {
          type: "object",
          properties: {
            request_type: { type: "string", enum: ["share_pii", "transfer_call", "commit_to_action", "reputation_risk"] },
            description: { type: "string", description: "What you're about to do, in plain English." },
            data_category: { type: "string", description: "Type of data: home_address, phone_number, email, employer, financial, etc." },
          },
    ...
    },
    {
      ...
      function: {
        ...
        description: "Flag embarrassing, defamatory, legally risky, or misrepresentational statements. Flag immediately, continue the call.",
        parameters: {
          type: "object",
          properties: {
    ...
    },
    {
      ...
      function: {
        name: "bridge_in_luke",
        description: "Connect caller directly to Luke. Ask name first if unknown. Say 'Let me get them for you' then call immediately.",
        parameters: {
          type: "object",
          properties: {
  ...
  return [
    ...base,
    {
      ...
      function: {
        ...
        description: "Query active projects and their task statuses. Use when Luke asks about project progress, task counts, or what needs attention.",
        parameters: {
          type: "object",
          properties: {
            project_name: { type: "string", description: "Optional — filter to a specific project by name (partial match)." },
          },
        },
      },
    },
    {
      ...
      function: {
        ...
        parameters: {
          type: "object",
          properties: {
            assignee: { type: "string", enum: ["Luke", "Amy", "Claude Code"], description: "Filter by who's responsible." },
    ...
    },
    {
      ...
      function: {
        name: "manage_task",
        description: "Create or update a project task or todo item. Use when Luke says 'add a task', 'mark that done', or 'create a todo'.",
        parameters: {
          type: "object",
          properties: {
            action: { type: "string", enum: ["create_todo", "complete_todo", "create_project_task", "update_task_status"], description: "What to do." },
            ...
            assignee: { type: "string", enum: ["Luke", "Amy", "Claude Code"], description: "Who's responsible." },
    ...
    },
    {
      ...
      function: {
        name: "send_message",
        description: "Send a message via Telegram on Luke's behalf. Use when Luke says 'message them', 'let them know', 'send a text'.",
        parameters: {
          type: "object",
          properties: {
...
const BUILT_IN_VERSIONS: AmyVersion[] = [
  {
    ...
    description: "Original Amy: gpt-4o, basic tools (code tasks, knowledge queries, approvals, bridge-in). No skill awareness.",
    llm: { provider: "openai", model: "gpt-4o" },
    voice: { provider: "11labs", voiceId: "paula" },
    identity: "You are a helpful AI assistant representing the user.",
    skills: SKILL_CATALOG.filter(s => ["Search Knowledge", "Queue Coding Task", "Request Approval", "Bridge Call to Luke", "Flag Reputation Risk"].includes(s.name)),
    rules: V1_RULES,
  ...
  },
  {
    ...
    description: "Full skill catalog, project/todo queries, task management, direct tool execution. Still gpt-4o.",
    llm: { provider: "openai", model: "gpt-4o" },
    voice: { provider: "11labs", voiceId: "paula" },
  ...
  },
  {
    ...
    createdAt: new Date().toISOString(),
    description: "Claude as the LLM via custom endpoint. Same brain as Claude Code. Full skill catalog + unified agent core.",
...
export async function buildDataSnapshot(): Promise<{
  ...
  try { projects = await listProjects(); } catch { /* ignore */ }
  try { todos = listTodos(); } catch { /* ignore */ }
  try {
