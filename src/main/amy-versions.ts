  ...
  return [
    ...
    },
    {
      ...
      function: {
        ...
        description:
          "Connect caller directly to the owner. Ask name first if unknown. Say 'Let me get them for you' then call immediately.",
        parameters: {
          type: 'object',
          properties: {
...
export async function buildVersionedSystemPrompt(
  ...
  if (context.callDirection === 'inbound' && context.callerPhone) {
    try {
      ...
      parts.push(
        `\n> CALLER IDENTIFICATION SYSTEM ACTIVE. Keyword for unknown callers claiming to be the owner: "${store.keyword}"`,
      );
    } catch {
      // non-critical
    }
