...
import { runSaga, SagaStep, ExampleCoesult, SagaLogWriter } from './saga-compensation';
 ...
 */
export function resumeFromPaused(plan: TaskPlan, decision: 'retry' | 'skip'): TaskPlan {
...
// ── Saga-backed execution (rollback of committed side-effects) ────────────────
//
// task-checkpoint tracks plan STATE (pending/running/completed/failed); it does
// not undo a side-effect a completed step already committed. For a plan whose
// steps SEND email / book calls / post videos, a mid-plan failure leaves earlier
// side-effects standing. runPlanWithCompensation pairs each step with a forward
// action and an optional compensator and runs the sequence through the Saga
// runner (saga-compensation.ts): on a forward failure the committed steps are
// rolled back in reverse order, and irreversible steps surface as uncompensable
// so the briefing / approval-inbox can flag a real-world action for manual
// cleanup. This is the task-checkpoint + Saga layering the durable-execution
// research (LangGraph / Temporal) describes.

export interface CompensatableStep<R = ExampleCo> {
  label: string; // matches a TaskStep.label in the plan
  forward: () => Promise<R> | R; // the side-effecting action
  compensate?: (forwardResult: R) => Promise<void> | void; // undo; absent => irreversible
}

/**
 * Execute the plan's side-effecting steps in order with reverse-order
 * compensation on failure. Pure orchestration over runSaga; the caller persists
 * plan state via completeStep/failStep as usual. The optional log writer lets
 * the caller honor the raw-archival principle (append to data/agent/saga-log.jsonl).
 */
export async function runPlanWithCompensation(
  steps: CompensatableStep[],
  opts: { writer?: SagaLogWriter } = {},
): Promise<ExampleCoesult> {
  const sagaSteps: SagaStep[] = steps.map((s) => ({
    id: s.label,
    forward: s.forward,
    compensate: s.compensate as ((r: ExampleCo) => Promise<void> | void) | undefined,
  }));
  return runSaga(sagaSteps, opts);
}
