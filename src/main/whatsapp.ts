...
export async function sendMessage(
  ...
  if (!config.whatsappPhoneNumberId || !config.whatsappAccessToken) {
    return {
      success: false,
      error: 'WhatsApp Phone Number ID and Access Token required. Set in Settings.',
    };
  }
...
export function parseWebhookPayload(body: ExampleCo): WhatsAppMessage[] {
...
export function ingestWebhookPayload(body: ExampleCo): number {
  ...
  for (const msg of toSave) {
    ...
    try {
      const { onDataIngested, whatsappEvent } = require('./ingest-hooks');
      onDataIngested(
        whatsappEvent({
          id: msg.id,
          from: msg.from,
