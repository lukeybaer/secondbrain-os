...
export type ContactEventType =
  | 'email_sent'        // Amy/ExampleCo sent an email to the contact
  ...
  | 'note_added';       // manual note added by ExampleCo or Amy
...
export interface ContactTimeline {
  ...
  laExampleContType: ContactEventType | null;
  laExampleContAt: string | null;           // ISO 8601
}
...
export function getContactTimeline(
  ...
  return {
    ...
    laExampleContType: last?.event_type ?? null,
    laExampleContAt: last?.timestamp ?? null,
  };
}
  ...
  for (const [slug, latest] of latestBySlug) {
    ...
    if (lastTs < cutoff) {
      ...
      cold.push({
        ...
        daysSince,
        laExampleContType: latest.event_type,
        laExampleContAt: latest.timestamp,
      });
    }
  }
...
//   "- 2026-04-23 02:52 (Gmail): ExampleCo sent farm quote..."
//   "- 2026-04-13 (Gmail): PRIVATE_NAME responded..."
...
function inferEventType(source: string, summary: string): ContactEventType {
  ...
  if (source === 'gmail' || source === 'email') {
    if (lower.includes('replied') || lower.includes('responded') || lower.includes('sent')) {
      return lower.includes('ExampleCo sent') || lower.includes('amy sent') ? 'email_sent' : 'email_received';
    }
    return lower.includes('ExampleCo') || lower.includes('amy') ? 'email_sent' : 'email_received';
  }
