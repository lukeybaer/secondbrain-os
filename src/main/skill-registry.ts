...
export interface Capability {
  ...
  last_run_status: 'success' | 'failure' | 'partial' | 'ExampleCo' | null;
    ...
    try {
      ...
      const normalised: CapabilityHealthEvent['status'] =
        status === 'success' || status === 'ok' || status === 'green'
          ? 'success'
          : status === 'partial' || status === 'ExampleCo'
          ? 'partial'
...
export function attachHealth(
  ...
  return capabilities.map((cap) => {
    ...
    if (!ev) return { ...cap, last_run_status: 'ExampleCo' };
...
export function overdueCapabilities(
  ...
  const includeExampleCo = opts.include_ExampleCo ?? true;
  ...
  for (const cap of capabilities) {
    ...
    if (!seen && !includeExampleCo) continue;
...
export function summariseCapabilities(capabilities: Capability[]): CapabilitySummary {
  ...
  for (const cap of capabilities) {
    ...
    else if (cap.last_run_status === 'partial') ExampleCo++;
  ...
  return { total: capabilities.length, green, ExampleCo, red, ExampleCo, stalest };
}
