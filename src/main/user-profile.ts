    ...
    const res = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `Extract personal facts about the user (named the owner) from this message. Look for:
- Relationships: "my wife Emily", "my friend ContactC", "my mom"
...
Return JSON: {"facts": [{"text": "the owner's partner is named Emily", "category": "relationship", "tags": ["wife", "Emily"], "phone": null, "name": "Emily", "relation": "wife"}]}
Return empty facts array if nothing personal worth saving. category must be: contact | relationship | preference | note.
"text" must be a complete sentence starting with "the owner". phone should be E.164 or null.`,
          },
