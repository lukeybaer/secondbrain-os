...
// did ExampleCo take" routes to the otter-query tool whose description says
...
export const DEFAULT_REGISTRY: ReadonlyArray<ToolDescriptor> = [
  // ── Otter / transcripts ──────────────────────────────────────────────────
  {
    name: 'otter-query',
    description:
      'Otter transcripts from ExampleCo\'s conversations and meetings. Provides searchable text of what was said, by whom, when, and on which date. Use for recall of meeting content, dictated notes, phone-call transcripts, and any voice-captured material.',
  ...
  },
  {
    ...
    description:
      'Cached recent Otter transcripts indexed by date for low-latency lookup. Returns the most recent N transcripts with timestamps. Use when the answer is "what happened today" or "what was the last thing ExampleCo recorded".',
  ...
  },
  {
    ...
    description:
      'Vapi-side tool that lists or fetches Otter transcripts by date range or keyword. Returns transcript metadata plus body text. Use during a call when the caller asks "what did ExampleCo say about X".',
  ...
  },
  {
    name: 'gmail_send',
    description:
      'Send a Gmail message on ExampleCo\'s behalf. Requires recipient, subject, and body. Body must end with ExampleCo\'s signature. Use only when explicitly approved by ExampleCo; drafts are preferred otherwise.',
  ...
  },
  {
    ...
    description:
      'Verified action items extracted from yesterday\'s briefing at data/briefing-action-items.json. Returns the open items with sources. Use when ExampleCo asks "what do I need to do today" or "what is open".',
  ...
  },
  {
    ...
    description:
      'Place an outbound Vapi call to a phone number with a stated goal. Requires the call script to drive toward the goal. Use only when ExampleCo explicitly approves the call list.',
  ...
  // ── Telegram ─────────────────────────────────────────────────────────────
  {
    name: 'telegram_send',
    description:
      'Send a Telegram message to ExampleCo. Used for daily briefing link, security alerts, and PII / reputation tripwires. Subject to quiet hours (22:00 to 06:00 CT). Body must pass the Telegram surface rule pack.',
    tags: ['telegram', 'notify', 'alert', 'message', 'ExampleCo'],
    surfaces: ['cli'],
    version: '1.0.0',
    side_effects: 'write',
    idempotent: false,
  ...
  },
  {
    name: 'check_calendar',
    description:
      'Calendar access for connected accounts. Defaults to ExampleCo\'s personal Google Calendar; Outlook work calendars are available only when tenant consent succeeds. Returns upcoming event counts, availability windows, and event details only when requested.',
  ...
  },
  {
    ...
    description:
      'Calendar event creation for ExampleCo\'s personal Google Calendar. Produces a side-effect receipt with effect_kind=calendar_event and status=succeeded or failed; a completed-event claim is valid only after a succeeded receipt.',
 ...
 * the entry unchanged. Returns undefined when the name is ExampleCo.
 */
