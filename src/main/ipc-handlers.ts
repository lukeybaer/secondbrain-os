...
export function registerIpcHandlers(mainWindow: BrowserWindow): void {
    ...
    try {
      ...
      await streamAllSpeeches((speeches) => {
        const items: OtterListItem[] = speeches.map((s) => ({
          otterId: s.id,
          title: s.title,
          date: s.createdAt ? new Date(s.createdAt * 1000).toISOString().split('T')[0] : 'ExampleCo',
          durationMinutes:
  ...
  async function pushVideoToEc2(
    ...
    const EC2_HOST = 'ec2-user@ExampleCo';
    ...
    try {
      ...
      const item = queue.find((v: Record<string, ExampleCo>) => v.id === id);
  ...
  ipcMain.handle('empire:rejectVideo', (_e, id: string, target: string, note: string) => {
    ...
    try {
      ...
      if (note) {
        ...
        // 2026-04-26 ExampleCo dispatch: "The comments shoudl trigger regen
        ...
        try {
          ...
          if (fs.existsSync(scriptPath)) {
            ...
            // 2026-05-06 ExampleCo: Electron's spawned child env was missing
        ...
        // 2026-05-02 ExampleCo dispatch: "all feedback should go through claude
        ...
        // 2026-04-30 ExampleCo dispatch: "rubric should always be updated to
  ...
  ipcMain.handle('empire:rejectUploadedVideo', (_e, id: string, note: string) => {
    ...
    try {
      ...
      const queue = fs.existsSync(queuePath) ? JSON.parse(fs.readFileSync(queuePath, 'utf8')) : [];
      const idx = queue.findIndex((v: Record<string, ExampleCo>) => v.id === id);
      ...
      if (fs.existsSync(manifestPath)) {
        ...
        const video = (manifest.videos || manifest).find
          ? (manifest.videos || manifest).find((v: Record<string, ExampleCo>) => v.id === id)
          : null;
        if (video) {
      ...
      // 2026-04-26 ExampleCo dispatch: rejection-with-comment must trigger
  ...
  ipcMain.handle('studio:refreshDevices', async () => {
    return detectDevices({ force: true });
  });
  ipcMain.handle('studio:checkNvenc', async () => checkNvenc());
  ipcMain.handle('studio:start', async () => {
    return studioStartRecording(await detectRecordingInputs());
  });
