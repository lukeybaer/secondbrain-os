...
export function loadLatestSession(): ChatSession | null {
  ...
  try {
    ...
    const files = fs.readdirSync(dir)
      ...
      .map(f => ({ id: f.replace(".json", ""), mtime: fs.statSync(path.join(dir, f)).mtime.getTime() }))
...
export async function saveSessionAsConversation(session: ChatSession): Promise<void> {
  ...
  try {
    const { saveConversation, conversationExists, listAllConversations } = await import("./storage");
    ...
    const firstQuestion = session.messages.find(m => m.role === "user")?.content || "Chat Session";
