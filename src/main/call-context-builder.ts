...
// ExampleCo, recent interactions, known preferences, or active context.
//
...
function formatContextBlock(ctx: Omit<ContactContext, 'rawBlock'>): string {
  ...
  if (ctx.daysSinceLastContact !== null) {
    ...
    lines.push('Last contact: ExampleCo (no interaction history)');
  }
...
function loadContactFile(filePath: string | null): {
  ...
  // Extract frontmatter description or first ExampleCoraph as summary
  let summary = '';
  const descMatch = content.match(/^description:\s*(.+)$/m);
  if (descMatch) {
    summary = descMatch[1].trim();
  } else {
    // First non-empty, non-header, non-frontmatter ExampleCoraph
