...
export const MEMORY_TOOLS: AnthropicToolDefinition[] = [
  {
    ...
    input_schema: {
      ...
      properties: {
        query: {
          type: 'string',
          description: 'Keyword query. Named entities, project names, and specific phrases work best.',
        },
        top_k: {
          type: 'number',
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        topic: {
          type: 'string',
          description: 'Short slug for the memory topic (e.g. "dentist-project", "spouse-visa-timeline").',
        },
        content: {
          type: 'string',
          description: 'The memory content to persist. Write in first person as Amy, concise and factual.',
        },
      },
    ...
    switch (name) {
      case 'memory_search': {
        ...
        const lines = results.map(
          (r, i) => `${i + 1}. [${r.relPath}:${r.lineNumber}] score=${r.score.toFixed(3)}\n   ${r.excerpt}`,
        );
        return {
          success: true,
      ...
      case 'memory_list_blocks': {
        ...
        if (names.length === 0) {
          return { success: true, output: 'No core blocks found. Use memory_write_block to create one.' };
        }
      ...
      default:
        return { success: false, output: `Unknown memory tool: ${name}` };
    }
  } catch (err) {
...
): { role: 'user'; content: [{ type: 'tool_result'; tool_use_id: string; content: string; is_error?: boolean }] } {
  return {
    role: 'user',
    content: [
      {
