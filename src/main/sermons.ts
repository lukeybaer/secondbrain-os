 ...
 * sermons.ts
 *
 * the speaker's sermon collection system. Detects sermons from Peter (the owner's parent)
 * in ingested Otter.ai conversations and stores them in a dedicated collection
 * for the "sermon collection" project.
 *
...
// Speaker identification tokens for sermon detection. "peter" is a common
// first name and a Bible-apostle name so is not PII on its own. Full last
// name was previously hardcoded here and stripped on 2026-04-11. If you
// have a different sermon speaker, override this via SERMON_SPEAKER_NAMES
// env var (comma-separated list).
const PETER_NAMES = process.env.SERMON_SPEAKER_NAMES?.split(',').map((s) =>
  s.trim().toLowerCase(),
) ?? ['peter'];
...
export function detectPeterSermon(
  ...
  // Also check transcript for "dad" references (the owner calling Peter "dad")
  ...
  // Check for the owner's explicit markers like "this conversation is with Peter"
  const explicitPeterMarker =
    /conversation.*(?:is |with )peter|peter.*(?:my|owner's).*(?:dad|father)/i.test(transcript);
...
export function processConversationForSermon(meta: ConversationMeta, transcript: string): boolean {
  ...
  const sermonMeta: SermonMeta = {
    ...
    title: meta.title || `the speaker's Teaching — ${meta.date}`,
    date: meta.date,
    ...
    keywords: meta.keywords.filter(
      (k) => RELIGIOUS_KEYWORDS.has(k.toLowerCase()) || k.toLowerCase().includes('speaker'),
    ),
