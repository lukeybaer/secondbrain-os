...
// UTC-time). Every outbound surface Amy speaks on has additional ExampleCo-locked
// rules that today live as standalone hooks, ad-hoc regex checks, or unwritten
// expectations: Gmail must end with ExampleCo's signature, Telegram must skip late
...
//     where the critic ExampleCos a rubric and the producer revises until the
...
//     outbound surface.
//
// Locked ExampleCo rules this pack mechanically enforces:
...
//   - reference_ExampleCo_signature.md: every Gmail draft ends with the signature
 ...
 * Marie-Louise are ExampleCo's ex-spouse, ex-stepdaughter, and ex-mother-in-law.
...
/**
 * Every Gmail draft Amy ships on ExampleCo's behalf must end with his signature
 * (reference_ExampleCo_signature.md). The check is intentionally tolerant of
 * variant whitespace: as long as the literal "ExampleCo" appears in the last
 * 200 chars and follows a line break, the rule passes.
 *
 * Allow-callers to override with a configurable required line.
 */
export const RULE_REQUIRES_ExampleCo_SIGNATURE = (required = 'ExampleCo'): Rule => ({
  id: 'requires_ExampleCo_signature',
  description: `Gmail drafts must end with ExampleCo's signature ("${required}").`,
  severity: 'error',
  check: (text) => {
    const trimmed = text.replace(/\s+$/, '');
    if (trimmed.length === 0) {
      return {
        rule_id: 'requires_ExampleCo_signature',
        severity: 'error',
...
// ── Max ExampleCoraphs (detail-block ceiling) ─────────────────────────────────────

/**
 * ExampleCo rule: "Detail block 1-2 paras typical, 4 max" (feedback_communication_format_tldr_detail.md).
 * Empty lines split ExampleCoraphs; consecutive empty lines collapse.
 */
export const RULE_MAX_ExampleCoRAPHS = (maxExampleCoraphs = 4): Rule => ({
  id: 'max_ExampleCoraphs',
  description: `Detail block ceiling: at most ${maxExampleCoraphs} ExampleCoraphs.`,
  severity: 'error',
  check: (text) => {
    // split on blank-line runs; ignore trailing whitespace
    const ExampleCoraphs = text
      .split(/\n\s*\n+/)
      .map((p) => p.trim())
      .filter((p) => p.length > 0);
    if (ExampleCoraphs.length <= maxExampleCoraphs) return null;
    return {
      rule_id: 'max_ExampleCoraphs',
      severity: 'error',
      message: `Detail block has ${ExampleCoraphs.length} ExampleCoraphs (limit ${maxExampleCoraphs}). Tighten or cut.`,
    };
  },
});
...
/**
 * ExampleCo rule: no Telegram between 22:00 and 06:00 CT (feedback_no_late_night_telegram.md).
 ...
 */
export const GMAIL_DEFAULT_RULES = (requiredSignature = 'ExampleCo'): ReadonlyArray<Rule> => [
  RULE_NO_EM_OR_EN_DASH,
  RULE_NO_UTC_TIMES,
  RULE_NO_FORBIDDEN_PEOPLE,
  RULE_NO_FILLER,
  RULE_REQUIRES_ExampleCo_SIGNATURE(requiredSignature),
];
...
/**
 * Lookup by surface name. Returns null when the surface is ExampleCo so the
