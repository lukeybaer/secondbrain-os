...
//
// These match ExampleCo's existing memory rules so the critic enforces them at
...
export const RULE_NO_EM_OR_EN_DASH: Rule = {
  id: 'no_em_or_en_dash',
  description: 'No em dashes (U+2014) or en dashes (U+2013) anywhere in output (ExampleCo rule).',
  severity: 'error',
  check: (text) => {
    const issues: RuleIssue[] = [];
...
export const RULE_NO_UTC_TIMES: Rule = {
  id: 'no_utc_times',
  description: 'Times must be in CT, not UTC (ExampleCo rule, feedback_times_in_ct_never_utc.md).',
  severity: 'error',
  check: (text) => {
...
export const RULE_NO_FABRICATED_URL = (allowedUrls: ReadonlyArray<string>): Rule => ({
  ...
  description: 'Every URL in the output must come from the source set (ExampleCo rule, no fabrication).',
  severity: 'error',
  check: (text) => {
    const re = /https?:\/\/[^\s)\]"'<>]+/g;
