...
export async function regenRejectedVideos(contentRoot: string): Promise<RegenResult[]> {
  ...
  for (const video of flagged) {
    ...
    // SAME bad artifact back into ExampleCo's queue. Never again.
    ...
    if (result.success) {
      ...
      if (failures.length > 0) {
        ...
        // so this video does NOT cycle back into ExampleCo's review queue.
      ...
      // state intact -- never cycle a blank thumbnail back to ExampleCo.
      if (thumbChanged) {
      ...
      // 2026-05-05 ExampleCo: the review screen has a "Previous Feedback" panel
      ...
      // the video comes back to ExampleCo he sees both his original feedback
