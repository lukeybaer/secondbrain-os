...
function runPython(scriptPath: string, args: string[] = [], env?: Record<string, string>): Promise<PythonResult> {
...
async function buildSingleVideo(spec: VideoSpec, index: number): Promise<ManifestEntry> {
  ...
  if (!fs.existsSync(buildScript)) {
    console.log(`[video-pipeline] build_video.py not found at ${buildScript} — generating placeholder`);
    ...
    const placeholderEntry: ManifestEntry = {
      ...
      qc_passed: false,
      qc_notes: "Empire build scripts not yet configured on this machine. Videos must be built on the GCP VM.",
      status: "pending_approval",
    };
...
export async function runVideoPipeline(): Promise<PipelineResult> {
  ...
  // Notify Luke of results
  if (config.telegramChatId) {
    const summaryLines = results.map((r) => {
      const status = r.qc_passed ? "✅" : "❌";
      return `${status} ${r.channel} — ${r.title ?? r.style}`;
    });
    const msg =
      `🎬 Video pipeline complete: ${qcPassed}/${built} passed QC\n` +
      summaryLines.join("\n") +
      `\n\nOpen SecondBrain → Content Pipeline to review`;

    await sendMessage(config.telegramChatId, msg).catch(() => undefined);
  }
