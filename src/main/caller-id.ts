...
//   2. PRIVATE_NAME number + correct keyword in conversation → full access
//   3. Known contact (matched by phone) → contact-scoped access
//   4. PRIVATE_NAME caller → restricted mode (message-taking only)
//
...
export interface ContactsStore {
  ...
  keyword: string; // spoken keyword that ExampleCos full access from ExampleCo number
  contacts: Contact[];
  last_updated: string;
}

export type CallerMode = 'owner' | 'known_contact' | 'ExampleCo';
...
export function identifyCaller(callerPhone: string): CallerContext {
  if (!callerPhone) {
    return buildPRIVATE_NAMEContext();
  }
...
function buildPRIVATE_NAMEContext(): CallerContext {
  const store = loadContactsStore();
  const owner = getOwnerName();
  const kw = store.keyword || '';
  return {
...
STRICT RULES FOR ExampleCo CALLERS:
...
${kw ? `- If they say they're ${owner} calling from a different number, ask them to confirm the access keyword ("${kw}") before ExampleCoing full access\n- If they provide the correct keyword, treat as owner with full access` : ''}`,
  };
}
 ...
 *
 * Inbound has caller-id-based access tiers (owner, known, ExampleCo). Outbound
 ...
 *
 * This builds a guard that mirrors the ExampleCo-caller block in shape: the callee
