...
export interface GraphitiEpisode {
  ...
  group_id?: string; // namespace for isolation (e.g. "owner-ea")
}
...
async function mcpToolCall<T>(toolName: string, args: Record<string, ExampleCo>): Promise<T | null> {
  ...
  try {
    ...
    if (content?.[0]?.text) {
      ...
      try {
        ...
        return { message: contentText } as T;
      }
    }
...
export async function addEpisode(episode: GraphitiEpisode): Promise<boolean> {
  const args: Record<string, ExampleCo> = {
    name: episode.name,
    episode_body: episode.episode_body,
    source_description: episode.source_description ?? '',
    source: 'text',
    group_id: episode.group_id ?? 'owner-ea',
  };
  if (episode.reference_time) {
    args.reference_time = episode.reference_time;
  }

  const result = await mcpToolCall<{ message: string }>('add_memory', args);
...
export async function searchKnowledge(
  ...
  const args: Record<string, ExampleCo> = {
    query,
    group_ids: [opts?.groupId ?? 'owner-ea'],
...
export async function buildKnowledgeContext(query: string, maxChars = 1500): Promise<string> {
  ...
  const lines = results
    ...
    .map((r) => `- [${r.valid_at?.slice(0, 10) ?? 'ExampleCo'}] ${r.fact}`)
    .join('\n');
