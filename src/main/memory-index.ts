...
export function writeWorkingMemory(content: string): void {
  ...
  const trimmed = lines.length > WORKING_MEMORY_MAX_LINES
...
export function upsertMemory(
  ...
  if (existingEntry) {
    ...
    existingEntry.decay_rate = existingEntry.mentions >= 3 ? 0.02 : 0.10;
  ...
  const entry: MemoryEntry = {
    ...
    mentions: 1,
    last_accessed: now(),
    decay_rate: opts?.decayRate ?? 0.10,
    valid_at: now(),
    tier: 2,
  };
 ...
 */
export function loadRelevantMemories(minWeight = 0.3, maxEntries = 8): Array<{ topic: string; content: string; weight: number }> {
  const index = loadIndex();
...
export function runNightlyDecay(): { decayed: number; archived: number; pruned: number } {
  ...
  for (const entry of index.entries) {
    ...
    if (daysSinceAccess > 0) {
      ...
      entry.weight = Math.max(0, entry.weight - entry.decay_rate * daysSinceAccess);
    ...
    if (entry.weight < 0.05 && !entry.invalid_at) {
      ...
      if (fs.existsSync(filePath)) {
        const content = fs.readFileSync(filePath, "utf-8");
        appendToArchive(`## ${entry.topic} (archived — weight: ${entry.weight.toFixed(3)})\n${content}`);
