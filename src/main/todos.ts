...
export type Assignee = 'the owner' | 'Amy' | 'Claude Code';
...
function getDefaultTodos(): Todo[] {
  const seed: Todo = {
    id: 'todo_seed_twilio',
    text: 'Discuss Twilio number port options with the owner',
    completed: false,
    dueDate: '2026-04-04',
    priority: 'high',
    assignee: 'Amy',
    tags: ['infrastructure'],
    notes:
      "Decide: port owner's main number to Twilio (Amy owns main number, the owner gets private SIM) vs get separate Twilio number for Amy. Also discuss SMS history import options.",
