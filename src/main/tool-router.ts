...
//     not the command to run it" which mirrors ExampleCo's locked rule
...
import { solveToolRules, type BlockedTool, type ToolRule } from './tool-rules-solver';
...
// runs without gating; 'review' queues into the approval inbox for ExampleCo's
...
export interface ToolDescriptor {
  ...
  // invoke it (ExampleCo rule: feedback_knowledge_not_rules_for_amy.md).
  description: string;
...
export interface RouteResult {
  ...
  // Candidate tools removed by declarative sequencing or approval rules.
  blocked_by_rules?: BlockedTool[];
  // Tools withheld specifically because approval is still required.
  requires_approval?: string[];
  // True when a terminal rule says the current tool unit is already done.
  terminated_by_rule?: boolean;
}
...
export interface RouteOpts {
  ...
  // Optional declarative sequencing / approval rules. When supplied, route()
  // removes blocked tools before scoring so unsafe tools are never offered
  // as candidates to the caller.
  tool_rules?: ReadonlyArray<ToolRule>;
  tool_history?: ReadonlyArray<{ tool: string }>;
  approved_tools?: ReadonlyArray<string>;
}
...
export function route(
  ...
  const ruleResult = opts.tool_rules
    ? solveToolRules({
        rules: opts.tool_rules,
        history: opts.tool_history ?? [],
        available: filtered.map((t) => t.name),
        approvals: opts.approved_tools ?? [],
      })
    : null;
  const allowedNames = ruleResult ? new Set(ruleResult.allowed) : null;
  const ruleFiltered = allowedNames ? filtered.filter((t) => allowedNames.has(t.name)) : filtered;

  const indices = ruleFiltered.map(indexTool);
  ...
  return {
    ...
    blocked_by_rules: ruleResult && ruleResult.blocked.length ? ruleResult.blocked : undefined,
    requires_approval:
      ruleResult && ruleResult.requires_approval.length ? ruleResult.requires_approval : undefined,
    terminated_by_rule: ruleResult ? ruleResult.terminated : undefined,
  };
}
