...
Return JSON:
- people: names to find (e.g. ["PRIVATE_NAME", "PRIVATE_NAME Smith"]). Include first + last names separately.
...
// Parses "today", "yesterday", "this week", "last week", "last Friday" from
// the question and recent history. Returns a dateFilter string (YYYY-MM-DD)
// and strictFilter=true for exact-date matches, false for "since" ranges.

function parseDateFilter(
  question: string,
  history: ChatMessage[],
): { dateFilter: string | null; strictFilter: boolean } {
  // Look at current question + last few turns to catch follow-ups like "from today"
  const combined = [question, ...history.slice(-4).map(m => m.content)].join(" ").toLowerCase();

  const today = new Date();
  const iso = (d: Date) => d.toISOString().split("T")[0];

  if (/\btoday\b/.test(combined)) {
    return { dateFilter: iso(today), strictFilter: true };
  }

  if (/\byesterday\b/.test(combined)) {
    const d = new Date(today);
    d.setDate(d.getDate() - 1);
    return { dateFilter: iso(d), strictFilter: true };
  }
  ...
  if (/\bthis week\b/.test(combined)) {
    ...
    return { dateFilter: iso(d), strictFilter: false };
  }

  if (/\blast week\b/.test(combined)) {
    const d = new Date(today);
    const day = d.getDay();
    d.setDate(d.getDate() - (day === 0 ? 6 : day - 1) - 7); // Monday last week
    return { dateFilter: iso(d), strictFilter: false };
  }

  const dayNames = ["sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday"];
  const m = combined.match(/\blast\s+(sunday|monday|tuesday|wednesday|thursday|friday|saturday)\b/);
  if (m) {
    const target = dayNames.indexOf(m[1]);
    const d = new Date(today);
    const cur = d.getDay();
  ...
  return { dateFilter: null, strictFilter: false };
}
...
function applyDateFilter(
  ...
  if (strict) return metas.filter(m => m.date === dateFilter);
  return metas.filter(m => m.date >= dateFilter);
}
...
function searchByPlan(plan: SearchPlan, candidates?: ConversationMeta[]): ConversationMeta[] {
  const all = candidates ?? getAllConversationMeta();
  ...
  // When no keyword terms, return candidates sorted newest-first
  if (allTerms.length === 0) return [...all].sort((a, b) => b.date.localeCompare(a.date));
  ...
  const metaMatches = all.map(meta => {
    ...
    .filter(({ score }) => score > 0)
    // Primary sort: score desc. Tie-break: date desc (newest first).
...
export async function chat(
  ...
  // Phase 1b: Detect relative date references and pre-filter candidates
  const { dateFilter, strictFilter } = parseDateFilter(question, history);
  const allMetas = getAllConversationMeta();
  let candidates = allMetas;
  if (dateFilter) {
    const dateFiltered = applyDateFilter(allMetas, dateFilter, strictFilter);
    chatDebugLog(`Date filter "${dateFilter}" (strict=${strictFilter}): ${dateFiltered.length} matches from ${allMetas.length} total`);
