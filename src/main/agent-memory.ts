...
export function getAgentMemory(agentId: string): AgentMemory {
  if (!registry.has(agentId)) {
    ...
    if (!seed) throw new Error(`Unknown agent: "${agentId}". Register a seed in AGENT_SEEDS or create AgentMemory directly.`);
...
export class AgentMemory {
  ...
  async buildSystemPrompt(basePrompt: string, options?: { maxMemoryChars?: number; graphitiQuery?: string }): Promise<string> {
    const maxChars = options?.maxMemoryChars ?? 3000;

    // Build three-tier memory context (Tier 1 working + Tier 2 indexed)
    let memoryContext: string;
    try {
      memoryContext = buildMemoryContext({ maxChars: Math.floor(maxChars * 0.4), minWeight: 0.3 });
    } catch {
      // Fall back to flat memory file if index not initialized
      const memory = await this.read();
      memoryContext = memory.length > maxChars
        ? memory.slice(0, maxChars) + "\n\n*(memory truncated — full version stored locally)*"
        : memory;
    }

    // Also include the agent's own memory file (EA_MEMORY.md)
    const agentMemory = await this.read();
    const agentMemoryExcerpt = agentMemory.length > Math.floor(maxChars * 0.35)
      ? agentMemory.slice(0, Math.floor(maxChars * 0.35)) + "\n*(truncated)*"
      : agentMemory;

    // Graphiti knowledge context (temporal graph — async, non-blocking)
    let graphitiContext = "";
    if (options?.graphitiQuery) {
      try {
        graphitiContext = await buildKnowledgeContext(options.graphitiQuery, Math.floor(maxChars * 0.25));
      } catch { /* Graphiti unavailable — continue without it */ }
    }
...
Be specific and actionable. These notes will be written to your memory and used to improve future calls.`;
    ...
    try {
      appendToArchive(`## Call Reflection — ${input.contactName || input.phoneNumber}\n**Outcome:** ${input.outcome}\n${reflection}`);
    ...
    if (input.transcript) {
      ...
      }).catch(() => { /* Graphiti may be unavailable */ });
    }

    // Persist reflection log for audit trail
    try {
      const logPath = path.join(path.dirname(this.filePath), `${this.agentId}-reflection-log.jsonl`);
