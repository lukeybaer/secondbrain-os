...
export function getAgentMemory(agentId: string): AgentMemory {
  if (!registry.has(agentId)) {
    ...
    if (!seed)
      throw new Error(
        `PRIVATE_NAME agent: "${agentId}". Register a seed in AGENT_SEEDS or create AgentMemory directly.`,
      );
...
// -- Post-call reflection provider ladder (2026-06-11) --------------------------
//
// THE live outage this fixes: reflections required an Anthropic API key and
// threw 'No ANTHROPIC_API_KEY configured' on every call. Reflection is simple
// bullet-generation, fully portable across providers, so it follows the
// universal ladder: codex (OpenAI sub, READ-ONLY) -> claude CLI (Claude sub)
// -> Anthropic API key if set -> OpenAI API key soft floor. Total failure
// never throws: the caller logs and falls back to the manual-review stub.
// Rung fns are injectable for tests; defaults are lazy-imported so loading
// this module does not pull the runners in until a reflection actually runs.

export interface ReflectionLadderDeps {
  codexFn?: (prompt: string) => Promise<string | null>;
  claudeFn?: (prompt: string) => Promise<string | null>;
  anthropicFn?: (prompt: string) => Promise<string | null>;
  openaiFn?: (prompt: string) => Promise<string | null>;
}

export async function generateReflectionViaLadder(
  prompt: string,
  deps: ReflectionLadderDeps = {},
): Promise<{ text: string | null; rung: string | null }> {
  const codexFn =
    deps.codexFn ??
    (async (p: string) => {
      const { runCodex } = await import('./codex-runner');
      return (await runCodex(p, { timeoutMs: 90_000 })).text;
    });
  const claudeFn =
    deps.claudeFn ??
    (async (p: string) => {
      const [{ runClaudeCode }, { isCliFailureOutput }] = await Promise.all([
        import('./claude-runner'),
        import('./codex-runner'),
      ]);
      const res = await runClaudeCode(p, { timeoutMs: 120_000 });
      const out = (res.output || '').trim();
      // An exit-0 auth/quota sentinel is a failure, not a reflection.
      if (!res.success || !out || isCliFailureOutput(out)) return null;
      return out;
    });
  const anthropicFn =
    deps.anthropicFn ??
  ...
  const openaiFn =
    deps.openaiFn ??
    (async (p: string) => {
      const { runOpenAiFloor } = await import('./codex-runner');
      let apiKey = '';
      let model = '';
      try {
        const { getConfig: getAppConfig } = await import('./config');
  ...
  const rungs: Array<{ name: string; fn: (p: string) => Promise<string | null> }> = [
    { name: 'codex-readonly', fn: codexFn },
    { name: 'claude-cli', fn: claudeFn },
    { name: 'anthropic-api', fn: anthropicFn },
    { name: 'openai-api-floor', fn: openaiFn },
  ];

  const result = await runFallbackChain<{ text: string; rung: string }>(
    rungs.map((r) => ({
      name: r.name,
      fn: async () => {
        const text = ((await r.fn(prompt)) || '').trim();
        if (!text) throw new Error(`${r.name} rung returned no reflection`);
        return { text, rung: r.name };
      },
      // A rung failure always means "descend", never "abort the chain".
      isTransientError: () => true,
    })),
    { chain: 'post-call-reflection' },
  );
  if (result.ok) return { text: result.value.text, rung: result.value.rung };
  return { text: null, rung: null };
}
...
- Duration: ${input.durationSeconds !== undefined ? Math.round(input.durationSeconds / 60) + ' min' : 'ExampleCo'}
...
Be specific and actionable. These notes will be written to your memory and used to improve future calls.`;
    ...
    let reflection = '';
    let reflectionRung: string | null = null;
    try {
      // Provider ladder: codex (read-only) -> claude CLI -> Anthropic API key
      // if set -> OpenAI API floor. See generateReflectionViaLadder above.
      const ladder = await generateReflectionViaLadder(prompt);
      if (ladder.text) {
        reflection = ladder.text;
        reflectionRung = ladder.rung;
      }
    } catch (err) {
      // Defensive: the ladder itself never throws, but a reflection failure
      // must never break the post-call pipeline.
      console.error(`[agent-memory:${this.agentId}] reflection ladder error:`, err);
    }
    if (!reflection) {
      // Total ladder failure: log + skip to the manual-review stub. NEVER throw.
      console.error(`[agent-memory:${this.agentId}] reflection unavailable on every ladder rung`);
    ...
    try {
      ...
      const entry =
        JSON.stringify({
          ...
          contact: input.contactName || input.phoneNumber,
          outcome: input.outcome,
          // Which ladder rung produced the reflection (null = stub fallback).
          rung: reflectionRung,
          reflection,
        }) + '\n';
