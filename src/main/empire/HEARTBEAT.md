...
2. Build **3 fresh Channel1 videos** in 3 different styles:
...
3. Build **2 fresh Channel2 videos** for kids channel:
...
6. Send morning brief to owner via Telegram:
...
- Upload: youtube_token.json = Channel1; youtube_token_kids.json = Channel2

## Approval Check (ongoing)
If `empire/state/pending_approval_{YYYYMMDD}.json` exists and no approval received:
- Check if owner replied with approval
...
- Upload next Channel1 video → youtube_token.json
- Upload next Channel2 video → youtube_token_kids.json
- Mark as posted
...
2. Format and send daily report to owner
3. Save flag

## Pending Telegram Report
If `empire/analytics/pending_report.txt` exists:
1. Read and send to owner
2. Delete the file

Otherwise: HEARTBEAT_OK

## Daily Exec Update (include in every morning brief)
- Remind owner: "Still need Kling API key from you , needed for Format 4 (realistic human presenter style)"
