...
describe('mcp-server-health: registration', () => {
  it('registers a server and reports unknown status until first sample', () => {
...
describe('mcp-server-health: pickHealthy', () => {
  ...
  it('falls through to unknown candidate when allowed', () => {
    const mon = new McpServerHealth({
      open_after_consecutive_failures: 1,
    });
    mon.registerServer('down');
    mon.registerServer('unknown');
    mon.recordCall('down', false, 100, 'e');
    const pick = mon.pickHealthy(['down', 'unknown']);
    expect(pick).toBe('unknown');
  });
