 ...
 *
 * Validates that the speaker's sermons are correctly detected from conversation
...
describe('detectPeterSermon', () => {
  ...
  it('does NOT detect a work meeting as a sermon', () => {
    const meta = makeMeta({
      speakers: ['Peter', 'Alex', 'Zach', 'the owner'],
  ...
  it("detects 'dad' in transcript with religious context", () => {
    const meta = makeMeta({
      speakers: ['Unnamed Speaker', 'Jeff', 'the owner'],
...
describe('processConversationForSermon', () => {
  it('saves a detected sermon to the sermons directory', () => {
    ...
    expect(sermons[0].speaker).toBe('the speaker');
