 ...
 *
 * Regression guard for ITEM 8 - video regeneration with Luke's feedback.
 *
...
describe('(a) ec2-build-from-queue.py ingests the rejection note', () => {
  ...
  it('forces a rebuild when a re-queued video carries a rejection note', () => {
    src = fs.readFileSync(QUEUE_BUILDER, 'utf8');
  ...
  it('get_rejection_note reads the rejection_note field from a spec', () => {
  ...
  it('get_rejection_note also honors the legacy video_rejection_note field', () => {
    const out = runPySnippet(
      QUEUE_BUILDER,
  ...
  it('get_rejection_note returns empty string when there is no feedback', () => {
    const out = runPySnippet(
      QUEUE_BUILDER,
...
describe('(b) card text is derived strictly from the script', () => {
  ...
  it('keeps a title whose words all appear in the script', () => {
    const out = runPySnippet(
      QUEUE_BUILDER,
  ...
  it('rejects a title with wording the script never says', () => {
    // "human taste" is the exact invented-card bug Luke flagged.
    const out = runPySnippet(
      QUEUE_BUILDER,
...
describe('(c) seedance-broll.py derives shots from the actual script', () => {
  ...
  it('build_shot_prompts uses real script sentences as shot subjects', () => {
    const out = runPySnippet(
      SEEDANCE,
  ...
  it('falls back to the title only when no script is present (never invents)', () => {
    const out = runPySnippet(
      SEEDANCE,
