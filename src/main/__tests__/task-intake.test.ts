...
// actionType mirrors the allowlist gate: confident extractions classify as an
// information-only type unless the raw text contains SIDEFX.
const fakeExtract: ExtractFn = async (raw) => ({
  prompt: `cleaned: ${raw}`,
  confident: raw.includes('CLEAR'),
  reason: raw.includes('CLEAR') ? 'clear request' : 'garbled',
  actionType: raw.includes('SIDEFX') ? 'send' : raw.includes('CLEAR') ? 'summarize' : 'ExampleCo',
});
...
describe('drainIntake', () => {
    ...
    writeQueue([
      {
        ...
        source: 'otter',
        itemRef: 'ref-a',
        comment: 'approve the second one',
        match_hash: 'h1',
      },
      {
        ts: 't2',
  ...
  it('holds a confident extraction whose actionType is side-effecting (allowlist gate)', async () => {
    writeQueue([
      {
        ts: new Date().toISOString(),
        source: 'gmail',
        itemRef: 'r-sidefx',
        comment: 'CLEAR SIDEFX: email Ed the contract',
        match_hash: 'sx1',
      },
    ]);
    const result = await drainIntake(queuePath, consumedPath, fakeExtract);
  ...
  it('auto-approves a confident explicit extraction and holds an unclear one', async () => {
    writeQueue([
      {
        ts: 't1',
        source: 'otter',
        itemRef: 'r1',
        comment: '#Amy CLEAR: summarize the PRIVATE_NAME deck status',
        match_hash: 'c1',
      },
      {
        ts: 't2',
        source: 'otter',
        itemRef: 'r2',
        comment: 'uh the thing I I you know',
        match_hash: 'c2',
      },
    ]);
    const result = await drainIntake(queuePath, consumedPath, fakeExtract);
  ...
  it('records but does not auto-approve confident passive Gmail research without #Amy', async () => {
    writeQueue([
      {
        ts: new Date().toISOString(),
        source: 'gmail',
        ...
        comment: 'CLEAR: research this email thread',
        match_hash: 'passive-gmail',
      },
    ]);
    ...
    expect(task?.meta?.explicitRequest).toBe(false);
  });

  it('requires a real #Amy token for gmail_amy_email auto-approval', async () => {
    writeQueue([
      {
        ts: new Date().toISOString(),
        source: 'gmail_amy_email',
        itemRef: 'msg-no-token',
        subject: 'Amy research note',
        comment: 'CLEAR: summarize this thread',
        match_hash: 'gmail-no-token',
      },
      {
        ts: new Date().toISOString(),
        source: 'gmail_amy_email',
    ...
    expect(result.approved).toBe(1);
    const tasks = listTasks();
    expect(tasks.find((t) => t.source?.ref === 'msg-no-token')?.approved).toBeUndefined();
    expect(tasks.find((t) => t.source?.ref === 'msg-token')?.approved).toBe(true);
  });

  it('does not auto-approve Vapi rows unless the explicit trigger flag is present', async () => {
    writeQueue([
      {
        ts: new Date().toISOString(),
        source: 'vapi_call',
        itemRef: 'call-no-trigger',
        comment: 'CLEAR: include this in the briefing',
        match_hash: 'vapi-no-trigger',
      },
      {
        ts: new Date().toISOString(),
        source: 'vapi_call',
        itemRef: 'call-trigger',
        comment: 'CLEAR: include this in the briefing',
        explicitAmyTrigger: true,
        match_hash: 'vapi-trigger',
      },
    ]);
    ...
    expect(result.approved).toBe(1);
    const tasks = listTasks();
    expect(tasks.find((t) => t.source?.ref === 'call-no-trigger')?.approved).toBeUndefined();
    expect(tasks.find((t) => t.source?.ref === 'call-trigger')?.approved).toBe(true);
  });
