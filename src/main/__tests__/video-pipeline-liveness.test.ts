...
describe('content-review manifest — freshness and structure', () => {
  ...
  it('every video with status pending_approval has a video_file', () => {
    ...
    // Exclude test stubs injected by other spec files (e.g. rejection-token tests)
    const pending = manifest.videos.filter(
      (v: any) => v.status === 'pending_approval' && !v.id?.startsWith('spec_stub'),
    );
...
describe('video quality rubric v21 -- accuracy fields updated', () => {
  ...
  it('rubric version is 21', () => {
    expect(parseInt(String(rubric.version).replace(/^v/i, ''), 10)).toBeGreaterThanOrEqual(21);
  });

  it('cta_placement accuracy is at least 87', () => {
    // ExampleCo 2026-04-29: convert stale exact-match guards to floor guards so
    // the nightly enhancement loop can advance the rubric without breaking
    // the test surface. The point of these asserts was "never regress below
    // run-N's targets," not "never improve."
    expect(rubric.categories.content.subcriteria.cta_placement.detection_accuracy_self_assessment).toBeGreaterThanOrEqual(87);
  });
...
describe('video quality rubric v24 -- run-14 accuracy fields updated', () => {
  ...
  it('rubric version is 24', () => {
    expect(parseInt(String(rubric.version).replace(/^v/i, ''), 10)).toBeGreaterThanOrEqual(24);
  });
  ...
  it('pacing tool field is at least v4', () => {
    expect(rubric.categories.audio.subcriteria.pacing.tool).toMatch(/analyze-(audio-)?pacing-v([4-9]|\d{2})/);
  });

  it('hook_strength tool field is at least v5', () => {
    expect(rubric.categories.content.subcriteria.hook_strength.tool).toMatch(/analyze-hook-strength-v([5-9]|\d{2})/);
  });

  it('clarity tool field is at least v5', () => {
    expect(rubric.categories.visual.subcriteria.clarity.tool).toMatch(/analyze-clarity-v([5-9]|\d{2,})/);
  });
...
describe('video quality rubric run 25 -- accuracy fields updated to 87%', () => {
  ...
  // Floor guards (was exact-match) so future nightly upgrades don't regress the
  // test suite. ExampleCo 2026-04-29 dispatch covered this in the broader video-test
  // staleness pattern.
  it('music_mix accuracy is at least 87 after run-25', () => {
    expect(rubric.categories.audio.subcriteria.music_mix.detection_accuracy_self_assessment).toBeGreaterThanOrEqual(87);
  });

  it('thumbnail_appeal accuracy is at least 87 after run-25', () => {
    expect(rubric.categories.technical.subcriteria.thumbnail_appeal.detection_accuracy_self_assessment).toBeGreaterThanOrEqual(87);
  });

  it('trending_topic_alignment accuracy is at least 87 after run-25', () => {
    expect(rubric.categories.platform.subcriteria.trending_topic_alignment.detection_accuracy_self_assessment).toBeGreaterThanOrEqual(87);
  });

  it('music_mix tool field points to v5 or later', () => {
    expect(rubric.categories.audio.subcriteria.music_mix.tool).toMatch(/v([5-9]|[1-9]\d)/);
  });

  it('thumbnail_appeal tool field points to v5 or later', () => {
    expect(rubric.categories.technical.subcriteria.thumbnail_appeal.tool).toMatch(/v([5-9]|[1-9]\d)/);
  });

  it('trending_topic_alignment tool field points to v5 or later', () => {
    expect(rubric.categories.platform.subcriteria.trending_topic_alignment.tool).toMatch(/v([5-9]|[1-9]\d)/);
  });
...
describe('video quality rubric run 28 -- accuracy fields updated to 89%', () => {
  ...
  it('rubric version is at least 28', () => {
    expect(parseInt(String(rubric.version).replace(/^v/i, ''), 10)).toBeGreaterThanOrEqual(28);
  });

  it('hook_strength accuracy is at least 89 after run-28', () => {
    expect(rubric.categories.content.subcriteria.hook_strength.detection_accuracy_self_assessment).toBeGreaterThanOrEqual(89);
  });

  it('pacing accuracy is at least 89 after run-28', () => {
    expect(rubric.categories.audio.subcriteria.pacing.detection_accuracy_self_assessment).toBeGreaterThanOrEqual(89);
  });

  it('volume_levels accuracy is at least 89 after run-28', () => {
    expect(rubric.categories.audio.subcriteria.volume_levels.detection_accuracy_self_assessment).toBeGreaterThanOrEqual(89);
  });

  it('background_noise accuracy is at least 89 after run-28', () => {
    expect(rubric.categories.audio.subcriteria.background_noise.detection_accuracy_self_assessment).toBeGreaterThanOrEqual(89);
  });

  it('hook_strength tool field points to v6 or later', () => {
    expect(rubric.categories.content.subcriteria.hook_strength.tool).toMatch(/v([6-9]|[1-9]\d)/);
  });

  it('pacing tool field points to v5 or later', () => {
    expect(rubric.categories.audio.subcriteria.pacing.tool).toMatch(/v([5-9]|[1-9]\d)/);
  });

  it('volume_levels tool field points to v4 or later', () => {
    expect(rubric.categories.audio.subcriteria.volume_levels.tool).toMatch(/v([4-9]|[1-9]\d)/);
  });
...
describe('video quality tools run 24 -- rubric accuracy fields updated', () => {
  ...
  it('rubric version is 29 or later', () => {
    expect(parseInt(String(rubric.version).replace(/^v/i, ''), 10)).toBeGreaterThanOrEqual(29);
  });

  it('transitions accuracy is at least 89', () => {
    expect(rubric.categories.visual.subcriteria.transitions.detection_accuracy_self_assessment).toBeGreaterThanOrEqual(89);
  });

  it('cta_placement accuracy is at least 89', () => {
    expect(rubric.categories.content.subcriteria.cta_placement.detection_accuracy_self_assessment).toBeGreaterThanOrEqual(89);
  });

  it('caption_readability accuracy is at least 89', () => {
    expect(rubric.categories.technical.subcriteria.caption_readability.detection_accuracy_self_assessment).toBeGreaterThanOrEqual(89);
  });

  it('transitions tool points to v6 or later', () => {
    expect(rubric.categories.visual.subcriteria.transitions.tool).toMatch(/v([6-9]|[1-9]\d)/);
  });

  it('cta_placement tool points to v6 or later', () => {
    expect(rubric.categories.content.subcriteria.cta_placement.tool).toMatch(/v([6-9]|[1-9]\d)/);
  });

  it('caption_readability tool points to v5 or later', () => {
    expect(rubric.categories.technical.subcriteria.caption_readability.tool).toMatch(/v([5-9]|[1-9]\d)/);
  });
