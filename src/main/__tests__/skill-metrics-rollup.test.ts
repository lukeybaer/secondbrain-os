...
describe('skill-metrics-rollup', () => {
  ...
  it('daily-briefing detects todays briefing existence and counts blockers', () => {
    ...
    writeFile(briefing, [
      `Good morning ExampleCo -- ${today}`,
      'BLOCKERS / NEEDS FROM ExampleCo:',
      '  1. LinkedIn auth',
      '  2. Snack Dude AWS',
      '',
      'OK section here',
  ...
  // Regression: empty measurement windows must be flagged as insufficient data,
  // never read as a quality regression. Before this, a 3am run with no artifacts
  // yet produced a low "floor" quality that the checker mistook for a SKILL.md
  // regression and recommended reverting all 5 skills, every single night.
  it('flags insufficientData for every priority skill when no artifacts exist in the window', () => {
    const out = mod.rollupAll();
    for (const skill of ['video-aurora', 'captions-aurora', 'thumbnail-aurora', 'daily-briefing', 'daily-linkedin-scan']) {
      expect(out[skill].insufficientData).toBe(true);
    }
  });

  it('clears insufficientData for video-aurora once there is real signal in the window', () => {
    const f = path.join(tmpRoot, 'content-review', 'published', 'a.mp4');
    writeFile(f, 'x');
    setMtime(f, 2);
    const out = mod.rollupAll();
    expect(out['video-aurora'].insufficientData).toBe(false);
  });

  it('clears insufficientData for daily-briefing once todays briefing exists', () => {
    const today = new Date().toISOString().slice(0, 10);
    const briefing = path.join(tmpRoot, 'data', 'briefings', `briefing-${today}.md`);
    writeFile(briefing, `Good morning ExampleCo -- ${today}\nOK section\n`);
    const out = mod.rollupAll();
    expect(out['daily-briefing'].insufficientData).toBe(false);
  });

  it('treats a recent failed linkedin scan as real data, not insufficient data', () => {
    const snap = path.join(tmpRoot, 'data', 'agent', 'linkedin-scan-summary.json');
    writeFile(snap, JSON.stringify({
      lastRunAt: new Date().toISOString(),
      success: false,
    ...
    const out = mod.rollupAll();
    // A scan that ran recently and failed IS a signal -- do not suppress it.
    expect(out['daily-linkedin-scan'].insufficientData).toBe(false);
    expect(out['daily-linkedin-scan'].scanSuccess).toBe(0);
  });

  // Codex finding 2: a recent but corrupt summary file is a failure signal, not
  // absence of data. A scan that broke mid-write must not be suppressed.
  it('treats a recently-written corrupt linkedin summary as real data, not insufficient', () => {
    const snap = path.join(tmpRoot, 'data', 'agent', 'linkedin-scan-summary.json');
    writeFile(snap, '{ this is not valid json');
    const out = mod.rollupAll();
    expect(out['daily-linkedin-scan'].insufficientData).toBe(false);
    expect(out['daily-linkedin-scan'].scanSuccess).toBe(0);
  });

  // Codex finding 1: thumbnails ride video output. If videos published in the
  // window but zero thumbnails were produced (and zero thumb rejections), that
  // is the broken state we must catch -- NOT insufficient data.
  it('does not flag thumbnail insufficientData when videos published but no thumbnails exist', () => {
    const f = path.join(tmpRoot, 'content-review', 'published', 'clip.mp4');
    writeFile(f, 'x');
    setMtime(f, 2);
    const out = mod.rollupAll();
    expect(out['thumbnail-aurora'].thumbCount24h).toBe(0);
    expect(out['thumbnail-aurora'].insufficientData).toBe(false);
  });

  it('still flags thumbnail insufficientData when there is no video or thumb activity at all', () => {
    const out = mod.rollupAll();
    expect(out['thumbnail-aurora'].insufficientData).toBe(true);
  });
});
