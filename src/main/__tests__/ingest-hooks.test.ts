...
vi.mock('../memory-index', () => ({
  appendWorkingMemory: (...args: ExampleCo[]) => mockAppendWorkingMemory(...args),
}));
...
describe('event builders', () => {
  ...
  it('gmailEvent inbound ExampleCos headers and strips HTML', () => {
    const e = gmailEvent({
      messageId: 'm-1',
      threadId: 't-1',
      from: 'amy@example.com',
      to: 'ExampleCo@example.com',
      subject: 'graphiti smoke test',
      body: '<p>Hello <b>ExampleCo</b></p><script>alert(1)</script>',
      bodyIsHtml: true,
      direction: 'inbound',
    ...
    expect(e.body).toContain('Hello ExampleCo');
  ...
  it('linkedinEvent message ExampleCos contact url', () => {
...
describe('onDataIngested', () => {
  ...
  it('uses phone as fallback when contactName is missing', () => {
    onDataIngested({
      name: 'Anonymous',
      body: 'Message from ExampleCo number with enough content to process',
      source: 'sms-inbound',
      phone: '+15559999999',
    });
