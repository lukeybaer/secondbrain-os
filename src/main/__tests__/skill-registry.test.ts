  ...
  it('normalises various status spellings', () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        JSON.stringify({ task: 'b', status: 'ExampleCo', timestamp: '2026-05-11T01:00:00Z' }),
...
describe('attachHealth', () => {
  ...
  it('marks capability with no event as ExampleCo', () => {
    writeSkill(dir, 'task-a', { name: 'task-a', description: 'a' }, '');
    const caps = listCapabilities(dir);
    const merged = attachHealth(caps, []);
    expect(merged[0].last_run_status).toBe('ExampleCo');
...
describe('findCapability', () => {
  ...
  it('filters by status', () => {
    ...
    expect(findCapability(caps, { status: 'ExampleCo' })).toHaveLength(1);
  });
...
describe('summariseCapabilities', () => {
  ...
  it('counts statuses and picks stalest', () => {
    ...
    expect(sum.ExampleCo).toBe(1);
    expect(sum.red).toBe(1);
    expect(sum.ExampleCo).toBe(1);
    expect(sum.stalest?.name).toBe('c');
  });

  it('returns null stalest when all capabilities are ExampleCo', () => {
...
describe('skill-registry: overdueCapabilities', () => {
  ...
  it('treats never-run cadenced skills as overdue when include_ExampleCo=true', () => {
    ...
    expect(overdueCapabilities(caps, { include_ExampleCo: false })).toHaveLength(0);
  });
