/**
 * Regression guard for ExampleCo's 2026-04-29 followup dispatch:
...
describe('healVideoFeedbackLoop actually triggers regen (ExampleCo 2026-04-29)', () => {
  const src = fs.readFileSync(HEALTH_HEAL, 'utf8');
  const fn = src.slice(
    src.indexOf('function healVideoFeedbackLoop'),
    src.indexOf('function healVideoFeedbackLoop') + 4000,
  );
  ...
  it('healVideoFeedbackLoop surfaces failures with errorinfo (no silent swallow)', () => {
    ...
    expect(fn).toMatch(/sendTelegramAlert\(\s*`\[Amy Health\] auto-regen failed/);
  });
});
