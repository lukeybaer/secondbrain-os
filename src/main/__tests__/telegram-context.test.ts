...
const sources: ContextSources = {
  tasks: [
    {
      id: 'task-dentist',
      title: 'Find dentist in ExampleCo without X-rays',
      prompt: 'Research dentists near ExampleCo and call them about a cleaning.',
      status: 'running',
    },
    {
      id: 'task-old',
  ...
  projects: [
    { id: 'proj-ExampleCo', name: 'ExampleCo Stadium Launch' },
...
describe('matchMessageContext', () => {
  ...
  it('matches a project named in the message', () => {
    const result = matchMessageContext(
      { text: 'How is the ExampleCo stadium launch going?' },
      sources,
    );
    expect(result.kind).toBe('project');
    expect(result.ref).toBe('proj-ExampleCo');
  });

  it('matches an in-flight task named in the message', () => {
    const result = matchMessageContext(
      { text: 'Any update on the dentist in ExampleCo?' },
      sources,
    );
