...
describe('task-checkpoint', () => {
  ...
  it('increments retries on failure and pauses after max_retries', () => {
    let plan = createTaskPlan('t', 'g', [{ label: 's1', type: 'tool_call', max_retries: 2 }]);
  ...
  it('detectStuckPattern returns null for small sample', () => {
    const plans = [createTaskPlan('t', 'g', [{ label: 's', type: 'tool_call' }])];
...
describe('runPlanWithCompensation (saga rollback of committed side-effects)', () => {
  it('commits all steps on the clean path', async () => {
    const compensated: string[] = [];
    const res = await runPlanWithCompensation([
      { label: 'send', forward: () => 'sent', compensate: () => void compensated.push('send') },
      { label: 'book', forward: () => 'booked', compensate: () => void compensated.push('book') },
    ]);
    expect(res).toEqual({ ok: true, committed: ['send', 'book'] });
    expect(compensated).toEqual([]);
  });

  it('rolls back committed steps in reverse order on a mid-plan failure', async () => {
    const undone: string[] = [];
    const res = await runPlanWithCompensation([
      { label: 'a', forward: () => 1, compensate: () => void undone.push('a') },
      { label: 'b', forward: () => 2, compensate: () => void undone.push('b') },
      {
        label: 'c',
        forward: () => {
    ...
    expect(res.compensated).toEqual(['b', 'a']);
    expect(undone).toEqual(['b', 'a']);
  });

  it('surfaces an irreversible committed step as uncompensable for manual cleanup', async () => {
    const res = await runPlanWithCompensation([
      { label: 'send-email', forward: () => 'sent' }, // no compensator
      {
        label: 'write',
        forward: () => {
