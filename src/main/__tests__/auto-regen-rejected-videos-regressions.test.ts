...
describe('auto regen rejection regressions', () => {
  ...
  it('maps detected step overlays into the numbered_overlays rejection gate', () => {
    ...
    expect(src).toContain('const assNumberCount = (log.match(/\"type\":\\s*\"number\"/g) || []).length;');
  ...
  it('only clears cached EC2 voice files when the rejection is about audio', () => {
    ...
    expect(src).toContain("'voice.mp3', 'voice_human.mp3', 'voice_padded.mp3', 'voice_polished.mp3'");
  });
  ...
  it('writes semantic fix summaries instead of generic animation counts', () => {
    ...
    expect(src).toContain('v.feedback_history[existingIdx] = { ...v.feedback_history[existingIdx], ...historyEntry };');
  ...
  it('uses one latest rejection variable for rubric and legacy promotion paths', () => {
    ...
    expect(src).toContain('const latestRejection = verifyRejectionTokens.latestRejectionFor(v.id);');
    expect(src).toContain('verifyRejectionTokens.evaluate(v, latestRejection)');
    expect(src).toContain('describeFixFromRejection(latestRejection');
