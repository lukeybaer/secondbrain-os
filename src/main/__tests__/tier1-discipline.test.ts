...
describe('MEMORY.md Tier 1 discipline', () => {
  ...
  it('contains the audit command (in the state-locations reference)', () => {
    // The audit command moved out of MEMORY.md into the loaded reference file
    // when MEMORY.md was trimmed under the 200-line Tier-1 cap (commit c80e28a5).
    const stateLoc = fs.readFileSync(STATE_LOCATIONS, 'utf-8');
    expect(stateLoc).toMatch(/Audit command/i);
    expect(stateLoc).toMatch(/find C:\/Users\/\w+/);
  });
...
describe('MEMORY.md State locations table is a contract, not a description', () => {
  ...
  beforeAll(() => {
    // The State-locations table was relocated to the loaded reference file to
    // keep MEMORY.md under its 200-line Tier-1 cap (commit c80e28a5); parse it
    // where it now lives.
    src = fs.readFileSync(STATE_LOCATIONS, 'utf-8');
...
describe('reference_amy_state_locations.md discipline', () => {
  ...
  it('lists the audit command', () => {
    expect(src).toMatch(/find C:\/Users\/\w+/);
