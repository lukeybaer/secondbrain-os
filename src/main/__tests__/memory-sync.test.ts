...
`,
  ...
  await fsp.writeFile(
    path.join(memDir, 'project_projecta.md'),
    `---
name: ContactA Project Build
description: Stadium audience sync app with ContactA
type: project
---
...
`,
  ...
  await fsp.writeFile(
    ...
    `---
name: Contact A
description: ProjectA co-founder, inner circle
type: user
category: inner-circle
---

## Professional
- Co-founder of ProjectA
...
describe('readCanonicalMemory', () => {
  it('reads all memory files and returns structured content', () => {
    ...
    expect(result).toContain('the owner');
  ...
  it('filters by type', () => {
    ...
    expect(usersOnly).toContain('the owner');
    expect(usersOnly).toContain('Contact A'); // contacts are type: user
...
describe('readContactMemory', () => {
  it('reads a specific contact by slug', () => {
    ...
    expect(contact!.name).toBe('Contact A');
    expect(contact!.body).toContain('ProjectA');
  });
...
describe('searchMemoryFiles', () => {
  it('finds files by content keyword', () => {
    const results = searchMemoryFiles('ProjectA');
    expect(results.length).toBeGreaterThanOrEqual(1); // at least the project + contact files with ProjectAant contact
  });

  it('finds files by name', () => {
    const results = searchMemoryFiles('ContactA');
    expect(results.length).toBeGreaterThanOrEqual(1);
    expect(results[0].name).toContain('ContactA');
  });
