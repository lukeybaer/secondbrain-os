...
describe('tool-fallback-chain', () => {
  ...
  describe('nonRetryableTagClassifier', () => {
    it('treats nonRetryable=true as permanent', () => {
      const err: { message: string; nonRetryable: boolean } = { message: 'bad input', nonRetryable: true };
  ...
  describe('runFallbackChain', () => {
    ...
    it('falls through to secondary on transient primary error', async () => {
      ...
      const providers: FallbackProvider<string>[] = [
        { name: 'primary', fn: async () => { throw transient; } },
    ...
    it('aborts on permanent error and does NOT call sibling providers', async () => {
      ...
      const providers: FallbackProvider<string>[] = [
        { name: 'primary', fn: async () => { throw Object.assign(new Error('bad input'), { status: 400 }); } },
    ...
    it('returns chain-exhausted when every provider transient-fails', async () => {
      const providers: FallbackProvider<string>[] = [
        { name: 'a', fn: async () => { throw Object.assign(new Error('rl'), { status: 429 }); } },
        { name: 'b', fn: async () => { throw Object.assign(new Error('rl'), { status: 429 }); } },
        { name: 'c', fn: async () => { throw Object.assign(new Error('rl'), { status: 429 }); } },
      ];
      const result = await runFallbackChain(providers, { chain: 'test' });
      expect(result.ok).toBe(false);
      if (!result.ok) {
    ...
    it('honors custom isTransientError per provider', async () => {
      ...
      const provider1: FallbackProvider<string> = {
        name: 'primary',
        fn: async () => { throw Object.assign(new Error('5xx'), { status: 503 }); },
    ...
    it('writes one AttemptRecord per attempt to writer', async () => {
      ...
      const providers: FallbackProvider<string>[] = [
        { name: 'primary', fn: async () => { throw Object.assign(new Error('rl'), { status: 429 }); } },
        { name: 'secondary', fn: async () => 'ok' },
      ];
      await runFallbackChain(providers, { chain: 'gmail-send', writer: (r) => records.push(r) });
      expect(records).toHaveLength(2);
      expect(records[0]).toMatchObject({ chain: 'gmail-send', provider: 'primary', outcome: 'transient_error' });
      expect(records[1]).toMatchObject({ chain: 'gmail-send', provider: 'secondary', outcome: 'success' });
      expect(records.every((r) => typeof r.duration_ms === 'number' && r.duration_ms >= 0)).toBe(true);
    });
  });
  ...
  describe('runFallbackChainOrThrow', () => {
    ...
    it('throws FallbackChainExhausted on chain exhaustion', async () => {
      ...
      await expect(runFallbackChainOrThrow(providers, { chain: 'voice-out' })).rejects.toBeInstanceOf(FallbackChainExhausted);
    });
    it('FallbackChainExhausted carries chain id and result', async () => {
      const providers: FallbackProvider<number>[] = [
        { name: 'a', fn: async () => { throw Object.assign(new Error('rl'), { status: 429 }); } },
      ];
      try {
