...
// Mock personas
vi.mock('../personas', () => ({
  listPersonas: () => [{ id: 'p1', name: 'Test Persona', instructions: 'You are a test persona.' }],
}));
...
vi.mock('../todos', () => ({
  listTodos: () => [
    { id: 'td1', title: 'Buy groceries', status: 'pending', priority: 'high', assignee: 'the owner' },
...
vi.mock('../config', () => ({
  getConfig: () => ({
    ...
    ec2BaseUrl: 'https://ea.example.com',
    vapiWebhookSecret: 'configured-secret',
    vapiApiKey: 'test-key',
    vapiPhoneNumberId: 'test-phone',
    amyVersion: 2,
  }),
  getVapiWebhookUrl: () => 'https://ea.example.com/vapi/webhook',
  getVapiWebhookServer: () => ({
    url: 'https://ea.example.com/vapi/webhook',
...
describe('Amy Versions', () => {
  ...
  describe('Tool Generation', () => {
    it('v1 has basic tools (dtmf + 5 functions)', () => {
      ...
      expect(funcTools.length).toBe(5); // run_claude_code, query_knowledge, request_approval, flag_reputation_risk, bridge_in_owner
    });
  ...
  describe('Prompt Building', () => {
    ...
    // guards the section in buildVersionedSystemPrompt that tells Amy her
    // mic is always live and forbids narrating actions/waits/thoughts.
    // See memory/feedback_amy_hot_mic_no_narration.md for the full rule.
    it('includes LIVE MIC DISCIPLINE section forbidding narration of internal state', async () => {
      const v2 = getAmyVersion(2)!;
      const prompt = await buildVersionedSystemPrompt(v2, {
        callDirection: 'outbound',
      });
      expect(prompt).toContain('LIVE MIC DISCIPLINE');
      // Banned phrases explicitly called out
      expect(prompt).toContain('Pressing 3');
      expect(prompt).toContain('Waiting silently');
      // Core principle
      expect(prompt.toLowerCase()).toContain('microphone');
      expect(prompt.toLowerCase()).toMatch(/hot|always live|real time/);
      // Must apply to all Amy versions that build via buildVersionedSystemPrompt
      const v1 = getAmyVersion(1)!;
      const prompt1 = await buildVersionedSystemPrompt(v1, { callDirection: 'outbound' });
      expect(prompt1).toContain('LIVE MIC DISCIPLINE');
    });
  });
  ...
  describe('Vapi Assistant Config', () => {
    it('builds complete outbound config with serverUrl', async () => {
      ...
      expect(config.serverUrl).toContain('example.com');
    ...
    it('builds inbound config', async () => {
      ...
      expect(config.model.tools.length).toBeGreaterThan(5);
    });
  });
