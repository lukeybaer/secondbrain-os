...
interface MockSpawnBehavior {
  ...
  exitDelay?: number;
  /** Simulate a closed stdin stream throwing if stop writes after exit. */
  throwOnWriteAfterExit?: boolean;
}
...
function createMockProcess(behavior?: MockSpawnBehavior): any {
  ...
  proc.stdin.write = vi.fn((data: string) => {
    if (proc._exited && behavior?.throwOnWriteAfterExit) {
  ...
  if (behavior?.stdoutData) {
    process.nextTick(() => proc.stdout.emit('data', Buffer.from(behavior.stdoutData!)));
  }
...
vi.mock('child_process', () => ({
  spawn: (command: string, args: string[]) => {
    ...
    let behavior: MockSpawnBehavior | undefined;
    if (command === 'ffprobe' && spawnBehaviorQueue[0]?.forCommand !== 'ffprobe') {
      behavior = { exitImmediately: true, exitCode: 1 };
    } else {
      behavior = spawnBehaviorQueue.shift();
    }
...
function saveStudioConfigSeeded(config: Parameters<typeof saveStudioConfig>[0]) {
  const cameras = (config as any).cameras;
  if (Array.isArray(cameras)) {
    seedProbeCache(
      cameras.map((camera) => camera.name).filter(Boolean),
      true,
    );
  }
  return saveStudioConfig(config);
}
...
describe('Studio Config', () => {
  it('returns default config when none exists', () => {
    ...
    expect(config.lowerThirdName).toBe('the owner');
  ...
  it('detects retake phrases', async () => {
    ...
    const mockTranscript = {
      words: [
        ...
        { word: 'the owner', start: 1.0, end: 1.3, confidence: 0.99 },
        ...
        { word: 'ExampleCo', start: 6.3, end: 6.7, confidence: 0.99 },
      ],
...
describe('Studio Render Source Validation', () => {
  it('blocks rendering when the EDL references a missing camera source', async () => {
    const { renderFromEDL } = await import('../studio-render');
    const recDir = path.join(testRoot, 'data', 'studio', 'recordings', 'rec_render_missing');
    await fsp.mkdir(recDir, { recursive: true });

    const recording = {
      id: 'rec_render_missing',
      startedAt: '2026-04-04T10:00:00Z',
      stoppedAt: '2026-04-04T10:00:20Z',
      durationSeconds: 20,
      cameras: [
        { id: 'cam1', name: 'FrontCam', position: 'front' as const, enabled: true },
        { id: 'cam2', name: 'SideCam', position: 'side' as const, enabled: true },
      ],
      files: {
        side: path.join(recDir, 'side.mkv'),
      },
      screenFile: path.join(recDir, 'screen.mkv'),
      status: 'stopped' as const,
      markers: [],
    };

    await expect(
      renderFromEDL(recording as any, [{ camera: 'front', start: 0, end: 10, type: 'key_point' }]),
    ).rejects.toThrow('missing source "front"');
  });
});
...
function queueDetectedDevicesSpawn(devices: Array<{ name: string; type: 'video' | 'audio' }>): void {
  spawnBehaviorQueue.push({
    exitImmediately: true,
    exitCode: 1,
    stderrData: devices.map((d) => `[dshow @ 0x000001] "${d.name}" (${d.type})`).join('\n'),
  });
}
...
function queueAliveSpawn(): void {
  ...
  spawnBehaviorQueue.push({
    stderrData: 'frame=   12 fps=24 q=20.0 size=      64KiB time=00:00:00.50 bitrate=1048.6kbits/s',
  });
}

/** Helper: queue a spawn that dies after a delay (simulates mid-recording FFmpeg crash) */
function queueDyingSpawn(delayMs: number, stderrMsg?: string, withProgress = true): void {
  spawnBehaviorQueue.push({
    exitDelay: delayMs,
    exitCode: 1,
    stderrData:
      stderrMsg ||
      (withProgress
        ? 'frame=   12 fps=24 q=20.0 size=      64KiB time=00:00:00.50 bitrate=1048.6kbits/s'
        : ''),
  });
}
...
function queueFfprobeDuration(seconds: number): void {
  spawnBehaviorQueue.push({
    forCommand: 'ffprobe',
    exitImmediately: true,
    exitCode: 0,
    stdoutData: String(seconds),
  });
}

describe('Studio Device Detection Cache', () => {
  it('force detection replaces a larger stale cache with the current device list', async () => {
    queueDetectedDevicesSpawn([
      { name: 'OldCam A', type: 'video' },
      { name: 'OldCam B', type: 'video' },
      { name: 'OldMic', type: 'audio' },
    ]);
    const oldDevices = await detectDevices({ force: true });
    expect(oldDevices.map((d) => d.name)).toEqual(['OldCam A', 'OldCam B', 'OldMic']);

    queueDetectedDevicesSpawn([{ name: 'CurrentCam', type: 'video' }]);
    const currentDevices = await detectDevices({ force: true });
    expect(currentDevices.map((d) => d.name)).toEqual(['CurrentCam']);

    const cached = await detectDevices();
    expect(cached.map((d) => d.name)).toEqual(['CurrentCam']);
  });

  it('recording input detection uses a fresh snapshot instead of a stale larger cache', async () => {
    queueDetectedDevicesSpawn([
      { name: 'OldCam A', type: 'video' },
      { name: 'OldCam B', type: 'video' },
      { name: 'OldMic', type: 'audio' },
    ]);
    await detectDevices({ force: true });

    queueDetectedDevicesSpawn([
      { name: 'CurrentCam', type: 'video' },
      { name: 'OBS Virtual Camera', type: 'video' },
      { name: 'CurrentMic', type: 'audio' },
    ]);
    const inputs = await detectRecordingInputs();

    expect(inputs.cameras?.map((c) => c.name)).toEqual(['CurrentCam']);
    expect(inputs.audioDevice).toBe('CurrentMic');
  });

  it('deduplicates concurrent forced device detections', async () => {
    queueDetectedDevicesSpawn([{ name: 'CurrentCam', type: 'video' }]);

    const [first, second] = await Promise.all([
      detectDevices({ force: true }),
      detectDevices({ force: true }),
    ]);

    expect(first.map((d) => d.name)).toEqual(['CurrentCam']);
    expect(second.map((d) => d.name)).toEqual(['CurrentCam']);
    expect(spawnCallLog.filter((c) => c.args.includes('-list_devices'))).toHaveLength(1);
  });
});
...
describe('Camera Recording Pipeline', () => {
  ...
  it('post-recording validation marks low-data camera files as source failures', async () => {
    saveStudioConfigSeeded({
      cameras: [{ id: 'cam1', name: 'TestCam', position: 'front', enabled: true }],
      recordScreen: false,
    });
    ...
    fs.writeFileSync(filePath, Buffer.alloc(10_000));
    queueFfprobeDuration(10);

    const stopResult = await stopRecording();
    expect(stopResult.success).toBe(true);
    expect(stopResult.recording!.status).toBe('error');
    expect(stopResult.recording!.files.front).toBeUndefined();
    expect(stopResult.recording!.error).toContain('video data too low');
  });

  it('post-recording validation keeps valid static webcam footage that compresses small', async () => {
    saveStudioConfigSeeded({
      cameras: [{ id: 'cam1', name: 'StaticCam', position: 'front', enabled: true }],
      recordScreen: false,
    });
    ...
    queueFfprobeDuration(15);
    queueRemuxSpawn();

    const stopResult = await stopRecording();
    expect(stopResult.success).toBe(true);
    expect(stopResult.recording!.status).toBe('stopped');
    expect(stopResult.recording!.files.front).toBe(filePath);
    expect(stopResult.recording!.error).toBeUndefined();
  });
  ...
  it('fails the whole start when any selected camera fails to start', async () => {
    saveStudioConfigSeeded({
      cameras: [
    ...
    expect(result.error).toContain('BadCam');
    expect(result.error).toContain('requires every selected camera');
  });
  ...
  it('does not accept a camera that stays alive without warmup video', async () => {
    saveStudioConfigSeeded({
      cameras: [{ id: 'cam1', name: 'StalledCam', position: 'front', enabled: true }],
      recordScreen: false,
    });

    queueDyingSpawn(1500, '', false);
    queueDyingSpawn(1500, '', false);
    queueDyingSpawn(1500, '', false);
    ...
    expect(result.success).toBe(false);
    expect(result.error).toContain('StalledCam');
    expect(result.error).toContain('requires every selected camera');
  });
  ...
  it('fallback spawn args include -vcodec and -video_size from forceFormat', async () => {
    ...
    // 1. Default format fails
    queueFailSpawn('Error: Could not connect pins');
    // 2. First fallback (mjpeg 1280x720) succeeds
    queueAliveSpawn();
...
describe('NexiGo Camera Stability (regression)', () => {
  ...
  it('mid-recording camera death is logged', async () => {
    ...
    // Camera starts OK but dies after 1.5s
    queueDyingSpawn(1500);
