/**
 * Regression guard for the 2026-04-23 dashboard-renderer bugs Luke flagged
 * from the public briefing at http://98.80.164.16:3001/briefing:
 *
 ...
 *      Luke read as "news: approved - awaiting implementation". Fix: add an
...
describe('APPROVED -- awaiting implementation — not rendered as News', () => {
  it('sectionBucket places APPROVED in bucket 3 (Pipeline health and monitoring)', () => {
    const src = loadEc2Server();
    // 2026-04-26 regroup: APPROVED moved from bucket 2 -> bucket 3 to live
    // alongside FEATURE BACKLOG / TOKEN USAGE / AWS COSTS / AMY PROJECTS
    // ASSIGNED. The "Pipeline + money" mental model collapsed when the
    // dashboard re-clustered into 4 explicit groups (Pipeline / System
    // monitoring / Pipeline health / Owned application).
    const fnMatch = src.match(/function sectionBucket\(title\)[\s\S]*?\n\}/);
    expect(fnMatch).toBeTruthy();
    const body = fnMatch![0];
    expect(body).toMatch(/if \(t\.startsWith\('APPROVED'\)\) return 3/);
    expect(body).not.toMatch(/if \(t\.startsWith\('APPROVED'\)\) return 2/);
  });
