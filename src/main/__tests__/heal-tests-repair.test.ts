 ...
 *
 * Background: 2026-05-23 ExampleCo pushed back: "I don't think this fixes the
 ...
 *       Repair:    for each hit file that matches a known ExampleCo-private
...
describe('heal-tests-repair: pii-screen category', () => {
  ...
  it('classifies a file as ExampleCo-private when its path matches a known-private surface', () => {
    expect(repair.isExampleCoPrivateSurface('scripts/lib/email-archive-search.js')).toBe(true);
    expect(repair.isExampleCoPrivateSurface('tests/email-archive-search-regression.spec.ts')).toBe(true);
    expect(repair.isExampleCoPrivateSurface('scripts/life-archive-sms-backfill.ps1')).toBe(true);
    expect(repair.isExampleCoPrivateSurface('scripts/life-archive-health.py')).toBe(true);
    // Negative: a generic source file is not auto-classified as private
    expect(repair.isExampleCoPrivateSurface('src/main/briefing-parser.ts')).toBe(false);
    expect(repair.isExampleCoPrivateSurface('README.md')).toBe(false);
  });
});
...
describe('heal-tests-repair: ExampleCo category', () => {
  it('classifies ExampleCo failures conservatively (no auto-repair, queue for human)', () => {
    const item = {
...
describe('heal-tests-repair: end-to-end repair pass', () => {
  it('runRepairPass returns { repaired, queued, escalated } counts and never throws on a synthetic input', () => {
    ...
    // Two of the three items should be queued for human (stale-assert + ExampleCo)
    expect(result.queued).toBeGreaterThanOrEqual(2);
  });
});

describe('heal-tests-repair: skip-with-reason recommendations (ExampleCo 2026-05-27 #gap)', () => {
...
describe('heal-tests-repair: vapi live config drift (2026-05-28)', () => {
  // Closes the 2026-05-28 #gap: the overnight orchestrator spawned a claude
  // CLI session per blocker. Each session timed out at the 25-minute orchestrator
  // budget (exit 124) and never returned. The same "firstMessage is neutral"
  // assertion appeared in the briefing morning after morning. The fix is
  // deterministic: re-run push-amy-vapi-config.js so the live Vapi tenant
  // matches the source-of-truth in buildCallbackAssistantConfig.
  it('classifies a vapi-assistant-is-claude.test.ts failure as vapi-live-config-drift', () => {
    const item = {
      file: 'vapi-assistant-is-claude.test.ts',
      name: 'firstMessage is neutral, never owner-flavored',
      error: 'expected ... not to contain ...',
      errorDetail: 'AssertionError: expected "hey ExampleCo, what is going on?" not to contain "ExampleCo"',
      suggestSkip: false,
    };
    const cls = repair.classifyFailure(item);
    expect(cls.category).toBe('vapi-live-config-drift');
    expect(cls.canAutoRepair).toBe(true);
    expect(cls.queueForHuman).toBe(false);
  });

  it('repairVapiLiveConfigDrift returns the dry-run shape without invoking the push script', () => {
    const item = {
      file: 'vapi-assistant-is-claude.test.ts',
      name: 'firstMessage is neutral',
      errorDetail: 'drift',
    };
    const r = repair.repairVapiLiveConfigDrift(item, { dryRun: true });
    expect(r.ok).toBe(false);
    expect(r.reason).toMatch(/dryRun|push-amy-vapi-config\.js/);
  });

  it('runRepairPass routes a vapi-live-config-drift item through the dedicated handler (dry-run)', () => {
    const blocked = {
      items: [
        {
          file: 'vapi-assistant-is-claude.test.ts',
    ...
    const recPath = path.join(scratch, 'rec.jsonl');
    const rep = repair.runRepairPass(blocked, { dryRun: true, queuePath, skipRecommendationsPath: recPath });
    expect(rep.repaired).toBe(0);
    expect(rep.actions[0].reason).toMatch(/vapi-config-drift|dryRun|push-amy/);
  });
});
