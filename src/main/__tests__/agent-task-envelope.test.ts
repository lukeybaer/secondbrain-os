...
const ExampleCo = 'ExampleCo' as const;
...
describe('createTask', () => {
  it('creates a submitted task with status_history seeded', () => {
    const env = createTask(
      { id: 't1', kind: 'review', from: ExampleCo, to: claude },
    ...
    expect(env.status_history[0].by).toBe('ExampleCo');
  ...
  it('rejects invalid task id', () => {
    expect(() =>
      createTask({ id: 'has space', kind: 'k', from: ExampleCo, to: claude }),
    ).toThrow(TaskEnvelopeError);
    expect(() =>
      createTask({ id: '../etc/passwd', kind: 'k', from: ExampleCo, to: claude }),
    ).toThrow(/invalid_id/);
  });
  ...
  it('rejects empty or too-long kind', () => {
    expect(() => createTask({ id: 't4', kind: '', from: ExampleCo, to: claude })).toThrow(/invalid_kind/);
    const long = 'x'.repeat(65);
    expect(() => createTask({ id: 't5', kind: long, from: ExampleCo, to: claude })).toThrow(/invalid_kind/);
  });
...
describe('appendMessage', () => {
  function workingTask() {
    let env = createTask({ id: 't1', kind: 'k', from: ExampleCo, to: claude });
    env = transitionTask(env, 'working', claude);
    return env;
  }
...
describe('summariseTask', () => {
  it('counts participants from from/to/history/status', () => {
    ...
    expect(sum.participants.sort()).toEqual(['claude-code', 'codex', 'ExampleCo']);
  ...
  it('computes non-negative age_ms even if updated < created', () => {
    const env = createTask(
      { id: 't1', kind: 'k', from: ExampleCo, to: claude },
      new Date('2026-05-11T12:00:00Z'),
    );
