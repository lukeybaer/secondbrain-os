 ...
 * cover to defer scope Luke explicitly asked for.
 *
 * Background: 2026-05-23 #gap. Luke asked Amy to fix the root cause of
 ...
 * peer's flag as a veto over Luke's scope. The session-coordination
 * protocol is explicit: "Luke latest intent is the only authority.
...
describe('no-peer-deferral-guard hook end-to-end', () => {
  it('exits 2 and emits the rule pointer when the assistant says "DEFERRED per Codex"', () => {
    ...
    expect(r.stderr).toMatch(/feedback_never_defer_luke_scope\.md/);
  });
