...
describe('RULE_NO_HEDGING', () => {
  ...
  it('flags multiple hedges in one text', () => {
    const out = RULE_NO_HEDGING.check('I think it seems like Luke would prefer this.');
    expect(out).not.toBeNull();
    const issues = Array.isArray(out) ? out : [out!];
...
describe('RULE_REQUIRES_LUKE_SIGNATURE', () => {
  const rule = RULE_REQUIRES_LUKE_SIGNATURE('Luke');

  it('passes when signature is on its own line at the end', () => {
    const text = 'Dear Bryant,\n\nLet us catch up Friday.\n\nLuke';
  ...
  it('does not accept "Luke" inline only', () => {
    const text = 'Hi, this is Luke speaking. Thanks for the call.';
    const out = rule.check(text);
    expect(out).not.toBeNull();
  });

  it('accepts a configurable required string', () => {
    const r = RULE_REQUIRES_LUKE_SIGNATURE('Amy');
    expect(r.check('Hello there,\n\nThanks.\n\nAmy')).toBeNull();
    expect(r.check('Hello there,\n\nThanks.\n\nLuke')).not.toBeNull();
  });
});

describe('RULE_MAX_PARAGRAPHS', () => {
  it('passes when paragraph count is within limit', () => {
    const rule = RULE_MAX_PARAGRAPHS(4);
    const text = ['P1.', 'P2.', 'P3.'].join('\n\n');
    expect(rule.check(text)).toBeNull();
  });

  it('fails when paragraph count exceeds limit', () => {
    const rule = RULE_MAX_PARAGRAPHS(2);
  ...
  it('ignores leading and trailing blank lines', () => {
    const rule = RULE_MAX_PARAGRAPHS(1);
    expect(rule.check('\n\nP1.\n\n')).toBeNull();
  });
});
...
describe('surface packs', () => {
  ...
  it('GMAIL_DEFAULT_RULES enforces signature', () => {
    ...
    expect(ids).toContain('requires_luke_signature');
    expect(ids).toContain('no_forbidden_people');
  ...
  it('rulesForSurface dispatches to the right pack', () => {
    ...
    expect(rulesForSurface('gmail').some((r) => r.id === 'requires_luke_signature')).toBe(true);
  });
});
...
describe('end-to-end via checkRules', () => {
  ...
  it('gmail draft missing signature fails GMAIL_DEFAULT_RULES', () => {
    ...
    expect(issues.some((i) => i.rule_id === 'requires_luke_signature')).toBe(true);
  });
