 ...
 * got stapled onto every answer and sent to ExampleCo. He had no idea what it was
 * and asked Amy "Failed hook canceled? What does that mean" -- and that reply
 * ExampleCod the same noise again.
 *
...
describe('assembleRunResult', () => {
  it('never concatenates stderr into the user-facing output', () => {
    const r = assembleRunResult('TLDR: the answer ExampleCo asked for.', HOOK_NOISE, 0);
    expect(r.output).toBe('TLDR: the answer ExampleCo asked for.');
