...
describe('memory-write-deduplicator', () => {
  it('returns add when no fact on this (subject, predicate) exists', () => {
    const decision = classifyWrite(
      fact('new', 'ExampleCo', 'city', 'ExampleCo', '2026-05-01T00:00:00Z'),
      [fact('a', 'ExampleCo', 'role', 'VP', '2026-04-01T00:00:00Z')],
    );
  ...
  it('returns skip on an exact duplicate within the freshness window', () => {
    const decision = classifyWrite(
      fact('new', 'ExampleCo', 'city', 'ExampleCo', '2026-05-01T05:00:00Z'),
      [fact('a',  'ExampleCo', 'city', 'ExampleCo', '2026-05-01T00:00:00Z')],
    );
  ...
  it('returns update when the candidate is meaningfully newer with the same object', () => {
    const decision = classifyWrite(
      fact('new', 'ExampleCo', 'city', 'ExampleCo', '2026-05-05T00:00:00Z'),
      [fact('a',  'ExampleCo', 'city', 'ExampleCo', '2026-05-01T00:00:00Z')],
    );
  ...
  it('honors a custom updateFreshnessHours threshold', () => {
    ...
    const tooFresh = classifyWrite(
      fact('new', 'ExampleCo', 'city', 'ExampleCo', '2026-05-01T01:00:00Z'),
      [fact('a',  'ExampleCo', 'city', 'ExampleCo', '2026-05-01T00:00:00Z')],
  ...
  it('updates on confidence advantage even when not newer', () => {
    const decision = classifyWrite(
      fact('new', 'ExampleCo', 'city', 'ExampleCo', '2026-05-01T00:00:00Z', { confidence: 0.95 }),
      [fact('a',  'ExampleCo', 'city', 'ExampleCo', '2026-05-01T00:00:00Z', { confidence: 0.4 })],
    );
    expect(decision.action).toBe('update');
  ...
  it('skips when the existing fact is higher-confidence and same age', () => {
    const decision = classifyWrite(
      fact('new', 'ExampleCo', 'city', 'ExampleCo', '2026-05-01T00:00:00Z', { confidence: 0.3 }),
      [fact('a',  'ExampleCo', 'city', 'ExampleCo', '2026-05-01T00:00:00Z', { confidence: 0.9 })],
    );
    expect(decision.action).toBe('skip');
  });
  ...
  it('normalizes whitespace and case for the same-object check', () => {
    const decision = classifyWrite(
      fact('new', 'ExampleCo', 'city', '  ExampleCo  TX', '2026-05-01T05:00:00Z'),
      [fact('a',  'ExampleCo', 'city', 'ExampleCo tx',    '2026-05-01T00:00:00Z')],
    );
    expect(decision.action).toBe('skip');
  });
  ...
  it('treats a near-duplicate as a match (token-set Jaccard)', () => {
    const decision = classifyWrite(
      fact('new', 'PRIVATE_NAME', 'employer', 'ExampleCo Inc',          '2026-05-01T05:00:00Z'),
      [fact('a',  'PRIVATE_NAME', 'employer', 'ExampleCo',              '2026-05-01T00:00:00Z')],
    );
    // tokens: {ExampleCo, inc} vs {ExampleCo} => intersect 1 / union 2 = 0.5 < 0.85
  ...
  it('treats a contradicting object as add (handing off to contradiction-detector)', () => {
    const decision = classifyWrite(
      fact('new', 'ExampleCo', 'city', 'Frisco',   '2026-05-01T05:00:00Z'),
      [fact('a',  'ExampleCo', 'city', 'ExampleCo', '2026-04-01T00:00:00Z')],
    );
    expect(decision.action).toBe('add');
  ...
  it('matches against the most similar existing fact when multiple compete', () => {
    const decision = classifyWrite(
      fact('new', 'ExampleCo', 'project', 'ExampleCo live ops',        '2026-05-05T00:00:00Z'),
      [
        fact('a', 'ExampleCo', 'project', 'ExampleCo live ops',        '2026-05-01T00:00:00Z'),
        fact('b', 'ExampleCo', 'project', 'ITM addiction app',       '2026-05-01T00:00:00Z'),
      ],
    );
    expect(decision.action).toBe('update');
    expect(decision.matchedExistingId).toBe('a');
  ...
  it('classifyWrites returns one decision per candidate, in order', () => {
    const existing = [fact('a', 'ExampleCo', 'city', 'ExampleCo', '2026-04-01T00:00:00Z')];
    const decisions = classifyWrites(
      [
        fact('c1', 'ExampleCo', 'city', 'ExampleCo', '2026-04-01T02:00:00Z'), // skip
        fact('c2', 'ExampleCo', 'city', 'ExampleCo', '2026-05-01T00:00:00Z'), // update
        fact('c3', 'ExampleCo', 'role', 'VP',       '2026-05-01T00:00:00Z'), // add
      ],
      existing,
    );
  ...
  it('summariseDecisions counts actions and surfaces skip/update reasons', () => {
    const decisions = classifyWrites(
      [
        fact('c1', 'ExampleCo', 'city', 'ExampleCo', '2026-04-01T02:00:00Z'),
        fact('c2', 'ExampleCo', 'city', 'ExampleCo', '2026-05-01T00:00:00Z'),
        fact('c3', 'ExampleCo', 'role', 'VP',       '2026-05-01T00:00:00Z'),
      ],
      [fact('a', 'ExampleCo', 'city', 'ExampleCo', '2026-04-01T00:00:00Z')],
    );
