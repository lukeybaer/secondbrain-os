...
const validatePerson = (raw: ExampleCo): Person | null => {
  if (typeof raw !== 'object' || raw === null) return null;
  const r = raw as Record<string, ExampleCo>;
...
describe('maybeExtract', () => {
  it('returns a confident result when the model is confident and citation is grounded', async () => {
    const source = 'Call ExampleCo at +15551234567 if you need anything.';
    const out = await maybeExtract<Person>({
      ...
      runExtractor: () =>
        JSON.stringify({
          result: { name: 'ExampleCo', phone: '+15551234567' },
          confidence: 0.95,
    ...
    expect(out.result).toEqual({ name: 'ExampleCo', phone: '+15551234567' });
  ...
  it('abstains and returns null when citations are not in the source', async () => {
    ...
    const out = await maybeExtract<Person>({
      ...
      runExtractor: () =>
        JSON.stringify({
          result: { name: 'ExampleCo', phone: '555-FAKE' },
          confidence: 0.99,
          citations: ['555-FAKE'],
        }),
  ...
  it('abstains when validate() rejects the extracted shape', async () => {
    const out = await maybeExtract<Person>({
      validate: validatePerson,
      description: 'phone',
      sourceText: 'ExampleCo is here',
      runExtractor: () =>
        JSON.stringify({
          result: { name: 'ExampleCo' }, // missing phone
          confidence: 0.9,
          citations: ['ExampleCo'],
        }),
      abstentionsLogPath: tempLog(),
  ...
  it('clamps invalid confidence values to [0,1]', async () => {
    const out = await maybeExtract<Person>({
      validate: validatePerson,
      description: 'phone',
      sourceText: 'ExampleCo at +15551234567',
      runExtractor: () =>
        JSON.stringify({
          result: { name: 'ExampleCo', phone: '+15551234567' },
          confidence: 5,
