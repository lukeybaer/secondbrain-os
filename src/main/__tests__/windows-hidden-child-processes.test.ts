...
describe('Windows child process windows stay hidden', () => {
  it('hides the detached workers spawned by content rejection', () => {
    ...
    expect(src).toMatch(/spawn\('node', \[scriptPath, '--id', id\], \{[\s\S]*?detached:\s*true[\s\S]*?windowsHide:\s*true/);
    expect(src).toMatch(/spawn\('node', \[[\s\S]*?dispatchScript[\s\S]*?\], \{[\s\S]*?detached:\s*true[\s\S]*?windowsHide:\s*true/);
    expect(src).toMatch(/spawn\('node', \[rubricScript, '--id', id\], \{[\s\S]*?detached:\s*true[\s\S]*?windowsHide:\s*true/);
  });
  ...
  it('hides nested rejection feedback and auto-regen subprocesses', () => {
    ...
    expect(dispatch).toMatch(/spawn\('claude', \[[\s\S]*?\], \{[\s\S]*?detached:\s*true[\s\S]*?windowsHide:\s*true/);
    expect(regen).toContain("const HIDDEN_PROCESS = process.platform === 'win32' ? { windowsHide: true } : {};");
    expect(regen).toMatch(/spawnSync\(bin, \[gatePath, artifactPath\], \{[\s\S]*?\.\.\.HIDDEN_PROCESS/);
    expect(regen).toMatch(/spawnSync\('ssh', \[[\s\S]*?\], \{[\s\S]*?\.\.\.HIDDEN_PROCESS/);
