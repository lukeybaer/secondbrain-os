 ...
 * up with my feedback." Whenever Luke rejects a video, the
...
describe('rejection->rubric audit -- structure', () => {
  ...
  it('emits one of three findings per rejection: new_criterion_needed, tool_scored_high_but_luke_rejected, tool_scored_low_threshold_too_lax', () => {
    const src = fs.readFileSync(SCRIPT, 'utf8');
    expect(src).toMatch(/new_criterion_needed/);
    expect(src).toMatch(/tool_scored_high_but_luke_rejected/);
  ...
  it('reads scores from manifest.rubric_scores so it can compare what the tool said vs what Luke said', () => {
    const src = fs.readFileSync(SCRIPT, 'utf8');
    expect(src).toMatch(/rubric_scores/);
    // The kind=tool_scored_high_but_luke_rejected branch only fires when
    // the tool scored >= 70 but Luke still rejected -- a false-pass signal.
