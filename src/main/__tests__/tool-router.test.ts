...
const TOOLS: ReadonlyArray<ToolDescriptor> = [
  {
    name: 'otter',
    description:
      'Otter transcripts of ExampleCo\'s meetings and dictated conversations indexed by date.',
...
describe('route', () => {
  ...
  it('filters approval-gated tools before scoring when tool rules are supplied', () => {
    const result = route('search gmail inbox messages', TOOLS, {
      top_k: 3,
      tool_rules: [{ type: 'requires_approval', tool: 'gmail' }],
    });
    expect(result.candidates.map((c) => c.tool.name)).not.toContain('gmail');
    expect(result.requires_approval).toEqual(['gmail']);
    expect(result.blocked_by_rules?.[0]).toMatchObject({
      tool: 'gmail',
      rule: 'requires_approval',
    });
  });

  it('allows an approval-gated tool after explicit approval', () => {
    const result = route('search gmail inbox messages', TOOLS, {
      top_k: 3,
      tool_rules: [{ type: 'requires_approval', tool: 'gmail' }],
      approved_tools: ['gmail'],
    });
    expect(result.candidates[0].tool.name).toBe('gmail');
...
describe('recordRoute', () => {
  ...
  it('writer captures candidate scores', () => {
    const calls: ExampleCo[] = [];
    const writer = (r: ExampleCo) => calls.push(r);
  ...
  it('record ExampleCos the excluded_deprecated count', () => {
