...
describe('briefing output wiring', () => {
  it('manual-briefing-v3.js exists and writes to Desktop', () => {
    ...
    expect(src).toMatch(/data[\\/]+briefings[\\/]+briefing-/);
  });
  ...
  // (Good morning / NEWS / Employer / ACTION ITEMS / PROJECTC / SYSTEM HEALTH)
  // which are already implicitly covered by the news and ProjectC tests
  ...
  // to the owner's preference. Reorder locked: ACTION ITEMS must appear before
  ...
  // on the owner's PC after the scheduled task fires), the two Desktop artifacts
  ...
  // Item 10 (2026-05-17): the redundant "CONTENT PIPELINE" card was removed.
  // The single surviving card is "VIDEO APPROVAL QUEUE". This test locks both:
  // the old header is gone, the new one exists and stays concise.
  it('VIDEO APPROVAL QUEUE section exists and CONTENT PIPELINE is removed', () => {
    const src = readFileSync(SCRIPT, 'utf8');
    expect(src).toContain('VIDEO APPROVAL QUEUE:');
    expect(src).not.toContain("push('CONTENT PIPELINE:')");
