 ...
 * ExampleCo's review queue. Prior fix (2026-04-26) caught regen paths that lied
 ...
 * a black canvas. mtime moved, status went to pending_approval, ExampleCo saw
...
function writePpm(file: string, pixelValue: (x: number, y: number) => number) {
  const width = 180;
  const height = 320;
  const chunks = [`P3\n${width} ${height}\n255\n`];
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      const v = Math.max(0, Math.min(255, Math.round(pixelValue(x, y))));
      row.push(`${v} ${v} ${v}`);
    }
...
describe('thumbnail quality gate -- script behavior', () => {
  ...
  it('approves dark real-footage texture even when bright pixels are below 4%', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'thumb-gate-dark-real-'));
    const img = path.join(dir, 'dark-real.ppm');
    writePpm(img, (x, y) => (((x * 17 + y * 31) % 10) < 4 ? 5 : 85));

    const result = runGate(img);

    expect(result).not.toBeNull();
    expect(result!.ok).toBe(true);
    expect(result!.metrics?.pct_bright).toBeLessThan(4);
    expect(result!.metrics?.mean_luminance).toBeLessThan(60);
    expect(result!.metrics?.stddev_luminance).toBeGreaterThanOrEqual(35);
    expect(result!.metrics?.lr_mirror_mae).toBeGreaterThanOrEqual(25);
  });

  it('approves soft night footage with varied midtones even when highlights are sparse', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'thumb-gate-soft-night-'));
    const img = path.join(dir, 'soft-night.ppm');
    writePpm(img, (x, y) => {
      const band = (y * 7 + x * 3) % 20;
      const base = band < 6 ? 10 : band < 17 ? 50 : 130;
      return base + (x > 90 ? 8 : 0);
    });
    ...
    expect(result!.metrics?.mean_luminance).toBeLessThan(60);
    expect(result!.metrics?.stddev_luminance).toBeGreaterThanOrEqual(35);
    expect(result!.metrics?.pct_near_black).toBeLessThanOrEqual(35);
    expect(result!.metrics?.pct_midtone).toBeGreaterThanOrEqual(40);
  });

  it('approves centered real-scene texture even when the mirror score is low', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'thumb-gate-centered-real-'));
    const img = path.join(dir, 'centered-real.ppm');
    writePpm(img, (x, y) => {
      const mirroredX = x < 90 ? x : 179 - x;
      return ((mirroredX * 13 + y * 17) % 11) < 5 ? 90 : 225;
    });
    ...
    expect(result).not.toBeNull();
    expect(result!.ok).toBe(true);
    expect(result!.metrics?.lr_mirror_mae).toBeLessThan(14);
    expect(result!.metrics?.spatial_detail).toBeGreaterThanOrEqual(8);
  });

  it('still rejects flat dark frames with no real texture', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'thumb-gate-flat-dark-'));
    const img = path.join(dir, 'flat-dark.ppm');
    writePpm(img, () => 55);
    ...
    expect(result!.reason).toMatch(/no real highlights|procedural thumbnail|dominantly dark/i);
  });
});
