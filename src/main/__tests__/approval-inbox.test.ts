...
describe('approval-inbox', () => {
  ...
  it('decideApproval marks a request approved and records decidedBy + decidedAt', () => {
    ...
    expect(decided.decidedBy).toBe('luke');
  ...
  it('decideApproval throws when the request is already decided', () => {
    ...
    decideApproval(dir, req.id, { decision: 'approved', decidedBy: 'luke' });
    expect(() =>
      decideApproval(dir, req.id, { decision: 'rejected', decidedBy: 'luke' }),
    ).toThrow(/already approved/);
  });

  it('decideApproval throws for unknown id', () => {
    expect(() =>
      decideApproval(dir, 'nope', { decision: 'approved', decidedBy: 'luke' }),
    ).toThrow(/not found/);
  });
  ...
  it('getInboxStats aggregates statuses including ttl-effective expired', () => {
    ...
    decideApproval(dir, a.id, { decision: 'approved', decidedBy: 'luke' });
    decideApproval(dir, c.id, { decision: 'rejected', decidedBy: 'luke' });
