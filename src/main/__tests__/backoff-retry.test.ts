...
function fakeSleep(): { fn: (ms: number) => Promise<void>; calls: number[] } {
  ...
  return {
    calls,
    fn: async (ms: number) => { calls.push(ms); },
  };
}
...
describe('computeDelay', () => {
  ...
  it('decorrelated jitter uses prevDelay', () => {
    ...
    expect(computeDelay(2, 100, 30_000, 'exponential', 'decorrelated', 500, fixedRng(1))).toBe(1500);
  });
...
describe('retry', () => {
  ...
  it('does not retry non-retryable errors', async () => {
    ...
    let calls = 0;
    await expect(
      retry(async () => { calls++; throw new Error('400 Bad Request'); }, { sleep: sleep.fn }),
    ).rejects.toThrow('400');
    expect(calls).toBe(1);
    expect(sleep.calls).toHaveLength(0);
  });
  ...
  it('respects custom retryable predicate', async () => {
    const sleep = fakeSleep();
    let calls = 0;
    const customRetry = (e: unknown) => (e as Error).message.includes('please');
    await retry(
      async () => {
        calls++;
  ...
  it('honors maxAttempts = 1 (no retries)', async () => {
    ...
    let calls = 0;
    await expect(
      retry(async () => { calls++; throw new Error('500'); }, { maxAttempts: 1, sleep: sleep.fn }),
    ).rejects.toThrow();
    expect(calls).toBe(1);
    expect(sleep.calls).toHaveLength(0);
  });
  ...
  it('clamps maxAttempts to >=1 even if 0 supplied', async () => {
    ...
    let calls = 0;
    await expect(
      retry(async () => { calls++; throw new Error('500'); }, { maxAttempts: 0, sleep: sleep.fn }),
    ).rejects.toThrow();
    expect(calls).toBe(1);
  });
});
...
describe('presets', () => {
  it('RETRY_VAPI uses vapi predicate', async () => {
    ...
    let calls = 0;
    await expect(
      retry(async () => { calls++; throw new Error('voicemail-detected'); }, { ...RETRY_VAPI, sleep: sleep.fn }),
  ...
  it('RETRY_TELEGRAM does not retry message-too-long', async () => {
    ...
    let calls = 0;
    await expect(
      retry(async () => { calls++; throw new Error('message is too long'); }, { ...RETRY_TELEGRAM, sleep: sleep.fn }),
    ).rejects.toThrow();
    expect(calls).toBe(1);
  });
});
