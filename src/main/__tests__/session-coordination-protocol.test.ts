...
describe('shared agent coordination hook', () => {
  it('states that Luke intent is the only authority and Claude Code and Codex are peers', () => {
    ...
    expect(out).toContain('Luke latest intent is the only authority');
  ...
  it('blocks claimed files with a fast-path takeover instruction instead of agent hierarchy', () => {
    ...
    expect(stderr).toContain('Options, in Luke-intent order');
