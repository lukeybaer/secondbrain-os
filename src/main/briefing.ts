...
//   3. Employer GROUP NEWS (up to 2 articles) + MORTGAGE INDUSTRY NEWS (up to 3 articles)
//   4. Contact Intelligence + Reputation + Videos + Amy calls + LinkedIn (Saturdays)
//
// Dedup: articles are keyed by URL + title prefix. Employer and Mortgage are deduped
...
async function fetchEmployerArticles(): Promise<NewsArticle[]> {
  ...
  if (cfg.newsApiKey) {
    try {
      const query = encodeURIComponent('"Employer" OR "Employer"');
      ...
      console.warn(
        '[briefing] NewsAPI Employer failed, trying Google News RSS:',
        (err as Error).message,
      );
    }
  }
...
export async function sendDailyBriefing(): Promise<void> {
  ...
  // Build a dedup set from AI + World articles, then filter Employer and Mortgage
