// pii-vault.ts
// AES-256-GCM encrypted vault for the owner's PII.
...
async function logAndAlert(
  ...
  if (config.telegramChatId) {
    ...
    sendMessage(config.telegramChatId, msg, 'pii').catch(() => undefined);
  }
}
...
export function listPiiCategories(): Array<{
  category: PiiCategory;
  label: string;
  can_share: boolean;
}> {
