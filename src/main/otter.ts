...
export async function login(): Promise<void> {
  ...
  if (config.otterSessionCookie && config.otterUserId) {
    ...
    debugLog(`login(): using stored session cookie (userId=${userId})`);
    try {
  ...
  if (!config.otterEmail || !config.otterPassword) {
    throw new Error("Otter.ai credentials required. Click 'Login with Otter.ai' in Settings to authenticate with Google, or enter your email + password.");
  }
  ...
  debugLog(`Login user info: userid=${data.userid}, email=${data.email}, plan_type=${data.subscription?.plan_type ?? data.plan?.type ?? "ExampleCo"}, team_subscription_id=${data.subscription?.team_subscription_id ?? "none"}`);
  if (data.user) {
...
/**
 * Clears the in-memory session so the next API call triggers a fresh login.
 * Called after the in-app browser window saves new cookies to config.
 */
