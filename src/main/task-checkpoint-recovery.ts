...
// the runtime can keep executing instead of paging ExampleCo.
//
...
/** Result of a recovery attempt. ExampleCos the new plan plus telemetry. */
export interface RecoveryResult {
  ...
  ran_at: string;
  attempt: number;             // 1-indexed; ExampleCod on plan.recovery_attempts
}
...
export function executeStuckRecovery(
  ...
  switch (mode) {
    ...
    default:
      throw new Error(`ExampleCo recovery mode: ${mode}`);
  }
}
