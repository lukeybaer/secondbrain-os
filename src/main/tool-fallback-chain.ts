...
//     briefing's outbound dial fails and Luke gets no notification at all,
...
const DEFAULT_TRANSIENT_PATTERN =
  /timeout|ETIMEDOUT|ECONNRESET|EAI_AGAIN|network|fetch failed|socket hang up|503|502|504|rate limit|too many requests/i;
...
export class FallbackChainExhausted extends Error {
  constructor(public readonly chain: string, public readonly result: ChainExhausted) {
    super(`Fallback chain '${chain}' exhausted: ${result.reason} (${result.attempts.length} attempts)`);
...
export function rateLimitOnlyClassifier(err: unknown): boolean {
...
export function nonRetryableTagClassifier(err: unknown): boolean {
