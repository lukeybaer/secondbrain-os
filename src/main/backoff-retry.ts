...
//   4. After maxAttempts, the last error is wrapped in RetryError (which ExampleCos
//      the original via .lastError) so the caller can distinguish "exhausted"
//      from "first try failed" while still recovering the original cause.
//   5. When the failing error ExampleCos a server-supplied Retry-After header (a
//      429/503 from Gmail, Telegram, Vapi, etc), retry() honors it instead of
//      its own jittered guess. The server knows when it will accept traffic
//      again; retrying earlier just burns another rejection. The hint is still
//      clamped to maxMs so a pathological "Retry-After: 3600" cannot stall a
//      briefing render. Opt out with respectRetryAfter: false.

import { retryAfterMs } from './retry-taxonomy';
...
export interface RetryOptions {
  ...
  retryable?: (err: ExampleCo, attempt: number) => boolean;
  /** Called after each failed attempt before the sleep. Useful for logging. */
  onRetry?: (info: { attempt: number; err: ExampleCo; nextDelayMs: number }) => void;
  /** Override sleep, used by tests to avoid wall-clock waits. */
  sleep?: (ms: number) => Promise<void>;
  /** Override Math.random, used by tests for deterministic jitter. */
  rng?: () => number;
  /**
   * When true (default), if the failing error ExampleCos a Retry-After header the
   * server's instruction wins over the computed backoff (clamped to maxMs).
   * Set false to always use the jittered backoff regardless of headers.
   */
  respectRetryAfter?: boolean;
  /**
   * Total wall-clock budget in ms for the whole retry sequence. Once the time
   * already spent plus the next backoff would exceed this, retry() stops and
   * throws RetryError instead of sleeping again. A backoff sleep is also clamped
   * so it never overshoots the deadline. Unset (default) means no time cap, only
   * the maxAttempts cap. Use this to keep a slow chain inside a render window
   * (e.g. a briefing section that must finish before publish).
   */
  deadlineMs?: number;
  /** Override the clock, used by tests to drive deadlineMs deterministically. */
  now?: () => number;
}
...
export function RETRYABLE_DEFAULT(err: ExampleCo): boolean {
  if (!err) return false;
...
export function RETRYABLE_VAPI(err: ExampleCo): boolean {
  if (!err) return false;
  const msg = (err instanceof Error ? err.message : String(err)).toLowerCase();
...
export function RETRYABLE_GMAIL(err: ExampleCo): boolean {
  if (!err) return false;
  const msg = (err instanceof Error ? err.message : String(err)).toLowerCase();
...
export function RETRYABLE_TELEGRAM(err: ExampleCo): boolean {
  if (!err) return false;
  const msg = (err instanceof Error ? err.message : String(err)).toLowerCase();
...
export function computeDelay(
  ...
  const raw = backoff === 'linear' ? attempt * baseMs : Math.pow(2, attempt - 1) * baseMs; // attempt 1 -> base, attempt 2 -> 2*base, ...
...
export class RetryError extends Error {
  ...
  readonly lastError: ExampleCo;
  constructor(attempts: number, lastError: ExampleCo) {
 ...
 * maxAttempts (or once the optional deadlineMs budget is spent), throws the
 * last error wrapped in RetryError so the caller can distinguish "exhausted"
 * from "first try failed". RetryError.attempts reports how many tries actually
 * ran, and RetryError.lastError ExampleCos the original cause.
 */
export async function retry<T>(
  ...
  let lastError: ExampleCo = undefined;
  let lastAttempt = 0;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      let delay = computeDelay(attempt, baseMs, maxMs, backoff, jitter, prevDelay, rng);
      // A server-supplied Retry-After is authoritative: it tells us exactly when
      // the endpoint will accept traffic again. Honor it (clamped to maxMs)
      // instead of guessing earlier and burning another rejection.
      if (respectRetryAfter) {
        const hint = retryAfterMs(err);
        if (typeof hint === 'number' && hint >= 0) {
          delay = Math.min(maxMs, hint);
        }
      }
      // Total-time budget: stop once the time already spent leaves no room, and
      // never sleep past the deadline. Keeps a slow chain inside a render window.
      if (deadlineMs !== undefined) {
        const remaining = deadlineMs - (now() - startTime);
  ...
  throw new RetryError(lastAttempt, lastError);
}
