...
export async function renderFromEDL(
  ...
  const recDir = recordingBaseDir(recording);
...
function collectRecordingFiles(recording: StudioRecording): Record<string, string> {
...
function recordingBaseDir(recording: StudioRecording): string {
  const firstFile =
    recording.files['front'] ||
    recording.files['main'] ||
    recording.screenFile ||
    Object.values(recording.files)[0];
  if (!firstFile) throw new Error('No source files available for rendering');
...
async function renderVideo(
  ...
  const cameraFiles = collectRecordingFiles(recording);

  // Sync anchors: each capture process recorded its spawn wall-clock time, so
  // inputs that started later get a per-input offset and the screen audio
  // stays aligned with later-started cameras. Old recordings without
  // syncAnchors get zero offsets everywhere: exactly the legacy behavior.
  const offsetsMs = computeSyncOffsets(recording.syncAnchors);
  const fileOffsetsSec: Record<string, number> = {};
  for (const [position, file] of Object.entries(cameraFiles)) {
    fileOffsetsSec[offsetKey(file)] = (offsetsMs[position] ?? 0) / 1000;
  }

  await validateRenderInputs(edl, cameraFiles, fileOffsetsSec);
...
/**
 * Key used to look up a per-input sync offset regardless of whether the
 * caller resolved the .mkv original or the remuxed .mp4 variant.
 */
function offsetKey(filePath: string): string {
  return filePath.replace(/\.(mkv|mp4)$/i, '');
}
...
async function validateRenderInputs(
  edl: EditDecision[],
  cameraFiles: Record<string, string>,
  fileOffsetsSec: Record<string, number>,
): Promise<void> {
  ...
  for (const decision of edl) {
    if (decision.end <= decision.start) {
      throw new Error(
        `Render blocked: invalid EDL segment for "${decision.camera}" (${decision.start}-${decision.end})`,
      );
    }

    const file = cameraFiles[decision.camera];
    if (!file) {
      throw new Error(`Render blocked: EDL references missing source "${decision.camera}"`);
    }
    if (!fs.existsSync(file)) {
      throw new Error(`Render blocked: source file is missing for "${decision.camera}"`);
    }

    if (!durationCache.has(file)) {
      durationCache.set(file, await probeDuration(file));
    }
    const duration = durationCache.get(file);
    if (duration === null || duration === undefined) continue;

    const localEnd = Math.max(0, decision.end - (fileOffsetsSec[offsetKey(file)] || 0));
    if (localEnd > duration + 0.5) {
      throw new Error(
        `Render blocked: "${decision.camera}" segment needs ${localEnd.toFixed(1)}s but source is ${duration.toFixed(1)}s`,
      );
    }
  }
}
...
function buildFilterComplex(
  ...
  for (const decision of edl) {
    ...
    const file = cameraFiles[camera];
    if (!file) {
      throw new Error(`Render blocked: EDL references missing source "${camera}"`);
    }
  ...
  for (let i = 0; i < edl.length; i++) {
    const d = edl[i];
    const file = cameraFiles[d.camera];
    if (!file) {
      throw new Error(`Render blocked: EDL references missing source "${d.camera}"`);
    }
    const srcIdx = fileIndex[file];

    // Sync offset: a source that started later holds global time T at local
    // time T - offset, so trim values shift down by this input's offset.
    const videoOffsetSec = (file && fileOffsetsSec[offsetKey(file)]) || 0;
    const vStart = Math.max(0, d.start - videoOffsetSec);
    const vEnd = Math.max(vStart, d.end - videoOffsetSec);

    // Video: trim + scale + optional zoom
    let videoFilter = `[${srcIdx}:v]trim=start=${vStart}:end=${vEnd},setpts=PTS-STARTPTS`;
    ...
    if (hasAudio) {
      const audioOffsetSec = (audioFileHint && fileOffsetsSec[offsetKey(audioFileHint)]) || 0;
      const aStart = Math.max(0, d.start - audioOffsetSec);
      const aEnd = Math.max(aStart, d.end - audioOffsetSec);
      const audioFilter = `[${audioSrcIdx}:a]atrim=start=${aStart}:end=${aEnd},asetpts=PTS-STARTPTS[a${i}]`;
