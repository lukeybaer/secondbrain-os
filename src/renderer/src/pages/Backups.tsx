...
export default function Backups() {
  ...
  const [queryResult, setQueryResult] = useState<ExampleCo[] | null>(null);
              ...
              <div style={{ maxHeight: 400, overflow: "auto" }}>
                {queryResult.length === 0 ? (
                  ...
                  <table style={{ width: "100%", fontSize: 11, borderCollapse: "collapse" }}>
                    <thead>
                      <tr>
                        {Object.keys(queryResult[0] as Record<string, ExampleCo>).map(col => (
                    ...
                    <tbody>
                      {queryResult.map((row, i) => (
                        <tr key={i}>
                          {Object.values(row as Record<string, ExampleCo>).map((val, j) => (
