...
const S = {
  ...
  healthDot: (status: string) => ({
    width: 8,
    height: 8,
    borderRadius: '50%',
    background: status === 'recording' ? '#22c55e' : status === 'starting' ? '#fbbf24' : '#ef4444',
    display: 'inline-block',
    marginRight: 8,
  }),
...
export default function Studio() {
  ...
  const [cameraHealth, setCameraHealth] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    ...
    window.api.studio.onCameraHealth((health) => {
      setCameraHealth(health || {});
    });

    return () => {
      window.api.studio.offProgress();
      window.api.studio.offCameraHealth();
  ...
  async function refreshDevices() {
    try {
      setDevices(await window.api.studio.refreshDevices());
    } catch {}
  }
  ...
  function statusColor(status?: string): string {
    if (status === 'recording') return '#4ade80';
    if (status === 'starting') return '#fbbf24';
    if (status === 'died' || status === 'failed') return '#f87171';
    return '#555';
  }
      ...
      >
        ...
        <div style={{ ...S.card, width: '100%', maxWidth: 500 }}>
          ...
          {videoCams.length === 0 ? (
            ...
            videoCams.map((cam, i) => {
              const position =
                config?.cameras?.find((c) => c.name === cam.name)?.position ||
                (['front', 'side', 'overhead', 'extra'] as const)[i] ||
                'extra';
              const health = cameraHealth[position];
              return (
                <div
                  key={i}
                  style={{
                ...
                >
                  <span style={health ? S.healthDot(health) : S.dot(true)} />
                  <span style={{ color: '#e0e0e0', flex: 1 }}>{cam.name}</span>
                  {health && <span style={{ color: statusColor(health) }}>{statusLabel(health)}</span>}
          ...
          {config?.recordScreen && (
            ...
            >
              <span style={cameraHealth.screen ? S.healthDot(cameraHealth.screen) : S.dot(true)} />
              <span style={{ color: '#e0e0e0', flex: 1 }}>Screen Capture</span>
              {cameraHealth.screen && (
                <span style={{ color: statusColor(cameraHealth.screen) }}>
                  {statusLabel(cameraHealth.screen)}
                </span>
              )}
        ...
        {warnings && (
          <div style={{ fontSize: 12, color: '#fbbf24', maxWidth: 400, textAlign: 'center' }}>
            {warnings}
          </div>
        )}
