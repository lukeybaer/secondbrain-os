...
export interface TaskRecord {
  ...
  prompt: string;
  title: string;
  status: 'queued' | 'running' | 'awaiting-review' | 'done' | 'failed' | 'cancelled';
  createdAt: string;
  updatedAt: string;
...
const STATUS_STYLE: Record<Status, { label: string; fg: string; bg: string; border: string }> = {
  ...
  queued: { label: 'Queued', fg: '#fbbf24', bg: '#3a2f0f', border: '#7f5f1d' },
...
export default function Tasks({ active }: { active: boolean }): React.ReactElement {
  ...
  useEffect(() => {
    // Live updates: a pushed task is upserted into the list.
    const onPush = (task: TaskRecord): void => {
      setTasks((prev) => {
