...
export default function Import({ onImported }: { onImported?: () => void }) {
  ...
  function applySort(items: ImportItem[]): ImportItem[] {
    return [...items].sort((a, b) => {
      let cmp = 0;
      if (sortField === "date") cmp = a.date.localeCompare(b.date);
      else if (sortField === "title") cmp = a.title.localeCompare(b.title);
      else if (sortField === "duration") cmp = a.durationMinutes - b.durationMinutes;
  ...
  const filteredOtterItems = search.trim()
    ? otterItems.filter(i =>
  ...
  const filteredChatItems = search.trim()
    ? chatItems.filter(i =>
        i.title.toLowerCase().includes(search.toLowerCase()) ||
        i.date.includes(search)
      )
  ...
  const visibleOtterItems = applySort(filteredOtterItems);
  const visibleChatItems = applySort(filteredChatItems);
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "#111", borderBottom: "1px solid #1e1e1e" }}>
                <Th width={40} />
                <SortTh field="title" label="Title" align="left" sortField={sortField} sortDir={sortDir} onSort={toggleSort} />
                <SortTh field="date" label="Date" width={100} sortField={sortField} sortDir={sortDir} onSort={toggleSort} />
                <SortTh field="duration" label="Duration" width={70} sortField={sortField} sortDir={sortDir} onSort={toggleSort} />
                <Th width={90}>Type</Th>
...
function SortTh({ field, label, width, align = "center", sortField, sortDir, onSort }: {
  field: SortField; label: string; width?: number; align?: string;
