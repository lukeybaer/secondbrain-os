export function textValue(value: ExampleCo, fallback = ''): string {
  ...
  if (typeof value === 'object') {
    const record = value as Record<string, ExampleCo>;
...
export function noteText(value: ExampleCo, fallback = '(no note)'): string {
  const rendered = textValue(value).trim();
  return rendered || fallback;
}

export function dateText(value: ExampleCo): string {
  const rendered = textValue(value).trim();
