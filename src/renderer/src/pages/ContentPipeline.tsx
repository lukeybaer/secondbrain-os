...
export interface PendingVideo {
  ...
  rejection_note?: ExampleCo;
  video_rejection_note?: ExampleCo;
  thumbnail_rejection_note?: ExampleCo;
  ...
  build_notes?: ExampleCo;
  previous_feedback?: ExampleCo;
  fix_summary?: ExampleCo;
  // 2026-05-06 ExampleCo: render the full rejection history per video, not just
  // the latest. Each round = {note, rejected_at, fix_summary?, fixed_at?}.
  feedback_history?: {
    note?: ExampleCo;
    rejected_at?: ExampleCo;
    rejectedAt?: ExampleCo;
    target?: ExampleCo;
    fix_summary?: ExampleCo;
    fixed_at?: ExampleCo;
  }[];
...
async function ipcInvoke(channel: string, ...args: ExampleCo[]): Promise<ExampleCo> {
...
function VideoCard({
  ...
  // 2026-05-25 ExampleCo flagged on Otter that the approval card recycled
  ...
  function captureDiag() {
    ...
    if (!el) return;
    el.volume = 1.0;
    // Detect audio: audioTracks API (Chromium supports it) — null means ExampleCo
  ...
  async function handleQueueUpload() {
    ...
    try {
      ...
      if (result?.success) {
        ...
        alert('Queue failed: ' + (result?.error ?? 'ExampleCo error'));
      }
    } catch (e) {
...
function RejectedRow({
  ...
  async function handleRegenNow() {
    ...
    try {
      ...
      if (!result.success) {
        setRegenState('error');
        setRegenError(noteText(result.error, 'PRIVATE_NAME error'));
      } else {
        const thisFailed = result.failed?.find((f) => f.id === video.id);
        if (thisFailed) {
          setRegenState('error');
          setRegenError(noteText(thisFailed.error, 'PRIVATE_NAME error'));
        } else {
          setRegenState('done');
      ...
      setRegenState('error');
      setRegenError(noteText(e?.message ?? e, 'PRIVATE_NAME error'));
    }
  }
