...
type Page = "chat" | "import" | "conversations" | "whatsapp" | "calls" | "projects" | "personas" | "content-pipeline" | "settings";
...
const NAV_ITEMS: { id: Page; label: string; hint: string }[] = [
  ...
  { id: "whatsapp", label: "WhatsApp", hint: "Read & send WhatsApp messages" },
  ...
  { id: "content-pipeline", label: "Content Pipeline", hint: "Review and approve videos for upload" },
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#0f0f0f" }}>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
        <div style={show("chat")}><Chat onConversationSaved={refreshCount} /></div>
        <div style={show("import")}><Import onImported={refreshCount} /></div>
        <div style={show("conversations")}><Conversations onCountChange={refreshCount} /></div>
        <div style={show("whatsapp")}><WhatsApp onCountChange={refreshCount} /></div>
        <div style={show("calls")}><Calls active={page === "calls"} pendingCall={pendingCall} autoListen={autoListen} /></div>
        <div style={show("projects")}><Projects onCallNow={handleCallNow} onNavigateTo={(p) => setPage(p as any)} onSetAutoListen={setAutoListen} /></div>
        <div style={show("personas")}><Personas active={page === "personas"} /></div>
        <div style={show("content-pipeline")}><ContentPipeline /></div>
        <div style={show("settings")}><Settings /></div>
      </div>
