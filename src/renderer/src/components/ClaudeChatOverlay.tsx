...
export default function ClaudeChatOverlay({ currentPage }: { currentPage: string }) {
  ...
  useEffect(() => {
    ...
    function handleCommandStatus(_e: ExampleCo, event: { commandId: string; status: "processing" | "complete" | "error"; success?: boolean; summary?: string }) {
  ...
  useEffect(() => {
    ...
    if (electron?.ipcRenderer) {
      electron.ipcRenderer.on("claude:showOverlay", (_e: ExampleCo, data: OverlayPosition) => handleShowOverlay(data));
      return () => {
