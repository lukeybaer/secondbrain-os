  ...
  async function sendCommand(prompt: string, screenshotPath?: string) {
    ...
    try {
      ...
      setResults(prev => prev.map(r => r.id === id
        ? { ...r, status: result?.success ? "queued" : "error", commandId: result?.commandId, error: result?.error }
        : r
      ));
    } catch (err: any) {
      setResults(prev => prev.map(r => r.id === id
                ...
                <div key={r.id} style={{
                  ...
                  {r.status === "queued" && (
                    <div style={{ color: "#6dbf6d" }}>
                      Queued — {r.commandId ? `ID: ${r.commandId.slice(0, 16)}…` : "check Telegram for result"}
                    </div>
                  )}
                  {r.status === "error" && (
