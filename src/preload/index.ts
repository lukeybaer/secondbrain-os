...
const api = {
  ...
  tasks: {
    ...
    run: (input: {
      ...
      prompt: string;
      title?: string;
    }): Promise<any> => ipcRenderer.invoke('tasks:run', input),
    ...
    reply: (id: string, reply: string): Promise<any> => ipcRenderer.invoke('tasks:reply', id, reply),
  ...
  studio: {
    ...
    refreshDevices: (): Promise<any[]> => ipcRenderer.invoke('studio:refreshDevices'),
    ...
    onCameraHealth: (cb: (health: Record<string, string>) => void) => {
      ipcRenderer.on('studio:camera-health', (_e, health) => cb(health));
    },
    offCameraHealth: () => ipcRenderer.removeAllListeners('studio:camera-health'),
  },
