...
async function transcribeRecording(recording) {
  ...
  const existing = [...new Set(candidates)].filter((f) => fs__namespace.existsSync(f));
  ...
  for (const f of existing) {
    ...
    console.log(`[studio-director] Skipping ${path__namespace.basename(f)} , no audio stream`);
  }
  if (!audioSource) {
    throw new Error(
      `No files have an audio stream. Checked: ${existing.map((f) => path__namespace.basename(f)).join(", ")}. Ensure the microphone is recording.`
    );
  }
  let words;
  try {
...
async function runWhisperTranscription(audioPath) {
  return new Promise((resolve, reject) => {
    const scriptPath = path__namespace.join(__dirname, "..", "..", "scripts", "transcribe.py");
    const proc = cp.spawn("python", [scriptPath, audioPath, "--output-format", "json"], {
      cwd: path__namespace.dirname(audioPath),
      env: { ...process.env }
    });
    let stdout = "";
    let stderr = "";
...
async function checkForAudioStream(filePath) {
...
async function runOpenAIWhisperTranscription(audioPath, apiKey) {
  ...
  await new Promise((resolve, reject) => {
    ...
    proc.on(
      "close",
      (code) => code === 0 ? resolve() : reject(new Error(`FFmpeg audio extract failed (code ${code})`))
    );
    proc.on("error", reject);
  });
  try {
    const fileData = fs__namespace.readFileSync(tempWav);
    const blob = new Blob([fileData], { type: "audio/wav" });
    const formData = new FormData();
    formData.append("file", blob, path__namespace.basename(tempWav));
    ...
    try {
      fs__namespace.unlinkSync(tempWav);
    } catch {
    }
  }
}
function detectSections(words) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    const hasLongPause = nextWord && nextWord.start - words[i].end > 2;
...
async function generateEDL(recording, transcript) {
  const config = index.getConfig();
...
async function generateAIEDL(recording, transcript, apiKey) {
  const availableCameras = recording.cameras.map((c) => c.position);
  const hasScreen = recording.screenFile || recording.files["screen"];
  const timestampedText = transcript.words.map((w) => `[${w.start.toFixed(1)}s] ${w.word}`).join(" ");
  const markerInfo = recording.markers.length > 0 ? "\n\nUser markers during recording:\n" + recording.markers.map((m) => `- ${m.type} at ${m.timestamp.toFixed(1)}s${m.label ? `: ${m.label}` : ""}`).join("\n") : "";
...
Remove dead air, filler, and retakes. Keep only the best content. Return valid JSON only.`;
  ...
  const edl = JSON.parse(jsonMatch[0]);
  return edl.filter(
    (d) => typeof d.camera === "string" && typeof d.start === "number" && typeof d.end === "number" && d.end > d.start
  );
}
function generateSimpleEDL(recording, transcript) {
  const edl = [];
...
function generateAlternatingEDL(cameras, start, end) {
  const edl = [];
  const cutInterval = 10;
  let cameraIdx = 0;
  let t = start;
...
function removeRetakes(transcript) {
...
exports.transcribeRecording = transcribeRecording;
