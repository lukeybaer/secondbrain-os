 ...
 *
 * NOTE on a known source bug:
 *   buildSystemPrompt() at calls.ts:152 references `${voicemailSection}` but
 *   never assigns that variable — it only defines the function buildVoicemailSection().
 *   This means initiateCall() WITHOUT a persona always throws
 *   "ReferenceError: voicemailSection is not defined".
 *   Tests that exercise the no-persona path document this behavior and
 *   only invoke initiateCall without expecting it to succeed (or use a persona).
 *
 * Strategy:
...
describe("initiateCall — no-persona path (known source bug)", () => {
  it("throws ReferenceError for voicemailSection when no persona is set (known bug in calls.ts:152)", async () => {
    const { saveConfig } = await import("../src/main/config");
    saveConfig({ vapiApiKey: "vapi-test-key", vapiPhoneNumberId: "pn-test-id" });
    (listPersonas as ReturnType<typeof vi.fn>).mockReturnValue([]);

    vi.spyOn(global, "fetch").mockResolvedValueOnce(
      makeFetchResponse({ id: "call-bug", status: "queued", monitor: {} }) as any,
    );

    // The no-persona code path in buildSystemPrompt references `voicemailSection`
    // which is never declared in that scope.  This surfaces as a ReferenceError.
    await expect(
      initiateCall("+15551234567", "Test no-persona path", ""),
    ).rejects.toThrow(/voicemailSection is not defined/);
  });
});
