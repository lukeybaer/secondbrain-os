...
function makeFetchResponse(body: ExampleCo, ok = true, status = 200) {
  return {
    ok,
    status,
...
describe("initiateCall — successful outbound call (with persona)", () => {
  ...
  it("uses a short 'Hi.' primer firstMessage on outbound (Amy must drive)", async () => {
    // ExampleCo 2026-04-27 PRIVATE_NAME call + 2026-04-29 retest: an empty firstMessage
...
describe("fetchAndSyncInboundCalls", () => {
  ...
  it("also queries the dynamic inbound Vapi phone number when configured", async () => {
    const { saveConfig } = await import("../src/main/config");
    saveConfig({
      vapiApiKey: "vapi-test-key",
      vapiPhoneNumberId: "pn-static-id",
      vapiInboundPhoneNumberId: "pn-live-inbound-id",
    });

    const fetchSpy = vi
      .spyOn(global, "fetch")
      .mockResolvedValueOnce(makeFetchResponse([]) as any)
      .mockResolvedValueOnce(
        makeFetchResponse([
          {
            id: "call-live-inbound",
            type: "inboundPhoneCall",
            customer: { number: "+15550008888" },
            status: "in-progress",
          },
        ]) as any,
      );
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      "https://api.vapi.ai/call?phoneNumberId=pn-static-id&limit=20",
      expect.any(Object),
    );
    expect(fetchSpy).toHaveBeenCalledWith(
      "https://api.vapi.ai/call?phoneNumberId=pn-live-inbound-id&limit=20",
      expect.any(Object),
    );
    expect(loadCallRecord("call-live-inbound")).not.toBeNull();
  });
  ...
  it("uses 'ExampleCo' as phoneNumber when customer.number is absent", async () => {
    const { saveConfig } = await import("../src/main/config");
    saveConfig({ vapiApiKey: "vapi-test-key", vapiPhoneNumberId: "pn-test-id" });
    ...
    expect(record!.phoneNumber).toBe("ExampleCo");
  });
});
...
describe("syncCallbackAssistant", () => {
  ...
  it("PATCHes the signed Vapi webhook server when a webhook secret is configured", async () => {
    const { saveConfig } = await import("../src/main/config");
    saveConfig({
      vapiApiKey: "vapi-test-key",
      callbackAssistantId: "asst-signed",
      vapiPhoneNumberId: "pn-test-id",
      ec2BaseUrl: "https://ea.example.com/prod",
      vapiWebhookSecret: "configured-secret",
    });
    ...
    const patchCall = fetchSpy.mock.calls.find(
      ([url]) => typeof url === "string" && (url as string).includes("/assistant/asst-signed"),
    );
    expect(patchCall).toBeDefined();
    const body = JSON.parse(((patchCall![1]) as RequestInit).body as string);
    expect(body.serverUrl).toBe("https://ea.example.com/prod/vapi/webhook");
    expect(body.server).toEqual({
      url: "https://ea.example.com/prod/vapi/webhook",
      headers: { "x-vapi-secret": "configured-secret" },
    });
  });
  ...
  it("sets firstMessage to neutral Amy greeting when there are incomplete non-owner calls", async () => {
    const { saveConfig } = await import("../src/main/config");
    saveConfig({
      vapiApiKey: "vapi-test-key",
    ...
    const body = JSON.parse(((patchCall![1]) as RequestInit).body as string);
    expect(body.firstMessage).toBe("Hi, this is Amy. Thanks for calling back!");
  });

  it("sets firstMessage to neutral Amy greeting when all non-owner calls are completed", async () => {
    const { saveConfig } = await import("../src/main/config");
    saveConfig({
      vapiApiKey: "vapi-test-key",
    ...
    const body = JSON.parse(((patchCall![1]) as RequestInit).body as string);
    expect(body.firstMessage).toBe("Hi, this is Amy. Good to hear from you.");
  });

  it("sets firstMessage to neutral Amy greeting when no prior non-owner calls exist", async () => {
    const { saveConfig } = await import("../src/main/config");
    saveConfig({
      vapiApiKey: "vapi-test-key",
    ...
    const body = JSON.parse(((patchCall![1]) as RequestInit).body as string);
    expect(body.firstMessage).toBe("Hi, this is Amy speaking, how can I help you?");
  });
