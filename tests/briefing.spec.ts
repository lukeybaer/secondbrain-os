 ...
 *  - Verify Employer and Mortgage articles are capped and deduped against AI/World
 */
...
describe('articleKey dedup logic', () => {
  ...
  it('falls back to title prefix when no URL', () => {
    const a = { title: 'Employer announces Q1 results today', url: undefined } as NewsArticle;
    const key = (a.url || a.title.toLowerCase().slice(0, 50)).trim();
    expect(key).toBe('employer announces q1 results today');
  });
});

describe('dedup caps and limits', () => {
  it('respects max=2 for Employer articles', () => {
    const articles = makeArticles(5, 'Employer');
  ...
  it('filters articles already in globalSeen', () => {
    ...
    // AI article was deduped; only 2 Employer articles remain (exactly the cap)
    expect(result).toHaveLength(2);
    expect(result.every((a) => a.title.startsWith('Employer'))).toBe(true);
  });

  it('Employer and Mortgage share the same seen set (no cross-section dupes)', () => {
    const shared = [makeArticles(1, 'Shared')[0]];
    const globalSeen = new Set<string>();

    // Employer gets the shared article
    ...
    // Should be empty because shared article was already consumed by Employer
...
describe('sendDailyBriefing — integration', () => {
  ...
  it('sends 4 Telegram messages when all sections have content', async () => {
    ...
    // Should have sent at least 3 messages (msg1 AI/Tech, msg2 World, msg3 Employer+Mortgage, msg4 ops)
  ...
  it('Employer section says "no articles found today" when empty', async () => {
    await sendDailyBriefing();
    const mockSend = vi.mocked(sendMessage);
    const allText = mockSend.mock.calls.map((c) => c[1] ?? c[0]).join('\n');
    expect(allText).toContain('Employer GROUP NEWS');
  });
