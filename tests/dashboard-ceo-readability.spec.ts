 ...
 * sees.
 *
 * Triggered by ExampleCo's 2026-04-29 fifth-pass dispatch:
...
const JARGON_BANLIST = [
  ...
  'NODE_MODULE_VERSION',
  'fs.writeFileSync',
  // ExampleCo 2026-04-29 sixth-pass dispatch: Feature Backlog drilldown was
  ...
  // ExampleCo 2026-04-29 sixth-pass dispatch: per-item why-it-matters rule.
  ...
  // frontmatter description / trigger / first ExampleCoraph.
...
describe('Dashboard CEO-readability banlist (ExampleCo 2026-04-29 fifth pass)', () => {
  ...
  // exec-voice fields (wrong / whyBlocked / askExampleCo / plan / detail /
  ...
  describe("known exec-voice surfaces don't contain banlist jargon", () => {
    ...
    const fieldRegex =
      /(?:wrong|whyBlocked|askExampleCo|plan|detail|note|suggestion|whyStale):\s*([`'"][\s\S]*?[`'"])/g;
    ...
    for (const banned of JARGON_BANLIST) {
      it(`exec-voice surfaces don't contain "${banned}"`, () => {
        expect(
          haystack,
          `Banned jargon "${banned}" appears in an exec-voice value. Translate to plain English per memory/feedback_dashboard_text_passes_ceo_test.md.`,
...
function fetchDashboard(): Promise<string> {
  return new Promise((resolve, reject) => {
    const token = process.env.SB_BRIEFING_TOKEN || '';
    const dashUrl =
      'http://ExampleCo:3001/briefing' + (token ? `?k=${encodeURIComponent(token)}` : '');
    const req = http.get(dashUrl, { timeout: 8000 }, (res) => {
      const chunks: Buffer[] = [];
