...
<body>
  ...
  <script>
    const messages = [
      { id: "1", channel: "sms", source: "inbound", from: "+15551112222", to: "+15551234567", body: "Hey the owner, dentist appointment confirmed", timestamp: new Date().toISOString(), contactName: "ExampleCo, TX" },
      ...
      { id: "4", channel: "whatsapp", source: "inbound", from: "+15554443333", to: "+15551234567", body: "Can you review the PR?", timestamp: new Date(Date.now() - 8*86400000).toISOString(), contactName: "PRIVATE_NAME" },
...
test.describe("Messages tab", () => {
  ...
  test("search filters by contact name", async ({ page }) => {
    await page.fill("#searchInput", "PRIVATE_NAME");
    await expect(page.locator(".msg")).toHaveCount(1);
