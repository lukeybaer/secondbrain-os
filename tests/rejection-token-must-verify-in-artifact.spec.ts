 ...
 * rebuild_did_not_address_rejection class of regression. Luke rejected
...
describe('verify-rejection-tokens-in-artifact -- rebuild must address rejection', () => {
  ...
  it('is idempotent: second run produces the same final state', () => {
    ...
    fs.writeFileSync(
      REJECTIONS_PATH,
      JSON.stringify({
        id: STUB_ID, target: 'video', note: 'no music background',
        rejectedAt: '2026-05-03T00:00:00.000Z',
      }) + '\n',
      'utf8'
    );
  ...
  it('exits 0 with no manifest changes when no token rules fire on the latest rejection note', () => {
    const manifest = {
      videos: [
        {
          id: STUB_ID, title: 'Stub', status: 'pending_approval',
          regen_status: 'rubric_passed',
          rubric_scores: { music_presence: 5 },
        },
      ],
