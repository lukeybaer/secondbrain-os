 ...
 *
 * ExampleCo 2026-04-28 #learn: "No linkedin reachouts? where's the QC #learn".
...
describe('LinkedIn outreach QC', () => {
  ...
  it('rejects malformed LinkedIn copy before approval', () => {
    const malformedDrafts = [
      'Given Becoming the operating model for HR leaders, I would value your read on this?',
      'Saw your post on AI transformation for HR . Open to a quick exchange?',
      'Saw your post on AI operations (especially HR workflows. Open to a quick exchange?',
      'Saw your post on AI... operations. Open to a quick exchange?',
    ];

    for (const draftBody of malformedDrafts) {
      const v = qcDraft({ ...goodDraft, draftBody });
      expect(v.passed).toBe(false);
      expect(v.failures.some((f: any) => f.rule === 'textQuality')).toBe(true);
    }
  });
