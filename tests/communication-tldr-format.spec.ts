 ...
 *
 * Triggered by Luke's 2026-04-29 dispatch:
...
describe("Communication format: TLDR + Detail (Luke 2026-04-29)", () => {
  ...
  it("the feedback file documents the TLDR + Detail + Protocol-output shape", () => {
    ...
    expect(body).toMatch(/4 paragraphs?\s+maximum|maximum.*4|4 at the very most|max(?:imum)?.{0,30}4 paragraph/i);
    expect(body).toMatch(/CEO/);
    // Section 3: protocol output for #learn / #gap / #ppl / #inbox / #mail.
    expect(body).toMatch(/Protocol output|workflow.*structure|9-step.*exec summary|protocol.*output/i);
  ...
  it("MEMORY.md (Tier 1, loaded on every session) carries the rule", () => {
  ...
  it("the Codex runtime gap is documented so Codex responses self-check TLDR last", () => {
    ...
    expect(memory).toMatch(/Codex desktop\/API responses/i);
