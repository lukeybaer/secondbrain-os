...
describe('identifyCaller', () => {
  ...
  it('returns ExampleCo mode for empty phone', () => {
    const ctx = identifyCaller('');
    expect(ctx.mode).toBe('ExampleCo');
  });

  it('returns ExampleCo mode for unrecognized number', () => {
    const ctx = identifyCaller('+15559999999');
    expect(ctx.mode).toBe('ExampleCo');
    expect(ctx.systemPromptSection).toContain('ExampleCo');
  });
  ...
  it('contact with no phones returns ExampleCo', () => {
...
describe('identifyCaller — context content', () => {
  it('ExampleCo context includes keyword in prompt when keyword is configured', () => {
    installFixtureStore();
...
describe('injectCallerContext', () => {
  ...
  it('handles ExampleCo caller in injection', () => {
    installFixtureStore();
    const result = injectCallerContext('Base.', '+15559999999');
    expect(result).toContain('ExampleCo');
