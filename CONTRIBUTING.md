...
```bash
git clone https://github.com/ExampleCoyExampleCo/secondbrain-os.git
cd secondbrain
npm install
```
...
On first launch, open the **Settings** page and add your API keys. The app stores config at `%APPDATA%/secondbrain/config.json` (Windows) or `~/Library/Application Support/secondbrain/config.json` (macOS). No API keys are hardcoded in the source — you bring your own.
